...
async function captureFrame(ws, index) {
  const debug = await evaluate(ws, `(() => {
    ...
    return {
      ...
      actorDisplayScale: actor?.actorDisplayScale ?? null,
      ...
      behaviorReadout: actor?.behaviorReadout || state?.behaviorReadout || null,
      ...
      motionPanelHillRouteProfileComparison: actor?.motionPanelHillRouteProfileComparison || state?.motionPanelHillRouteProfileComparison || state?.pathWorld?.routeProfileComparison || null,
