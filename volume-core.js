    ...
    for (let z = 0; z < nextGridSize; z += 1) {
      for (let y = 0; y < nextGridSize; y += 1) {
        for (let x = 0; x < nextGridSize; x += 1) {
          ...
          let initialBodySeed = 0;
          let initialDensitySeed = 0;
          let initialSmokeSeed = 0;
          let initialHeatSeed = 0;
          let initialFireSeed = 0;
          for (const sourcePrimitive of sourcePrimitives) {
            ...
            const inputRadius = Math.max(0.04, sourcePrimitive.radius);
            const inputFlow = Math.max(0, sourcePrimitive.flowRate);
            const sourceFireMix = Math.max(0, Math.min(1, sourcePrimitive.fireSourceMix ?? 1));
            const sourceSmokeMix = Math.max(0, Math.min(1, sourcePrimitive.smokeSourceMix ?? 1));
            const sourceDensityGain = Math.max(0, sourcePrimitive.densityGain ?? 1);
            const sourceFireGain = Math.max(0, sourcePrimitive.fireGain ?? 1);
            const sourceSmokeGain = Math.max(0, sourcePrimitive.smokeGain ?? 1);
            const sourceAnyMix = Math.max(sourceFireMix * sourceFireGain, sourceSmokeMix * sourceSmokeGain) * sourceDensityGain;
            ...
            const localSource = sourcePrimitive.primitiveCenteredBody ? primitiveBodySource : plumeSource;
            const sourceSmokeSeed = localSource * sourceSmokeMix * sourceSmokeGain * sourceDensityGain;
            const sourceFireSeed = localSource * sourceFireMix * sourceFireGain * sourceDensityGain;
            initialBodySeed += localSource * sourceAnyMix;
            initialSmokeSeed += sourceSmokeSeed;
            initialHeatSeed += sourceFireSeed;
            initialFireSeed += sourceFireSeed;
            initialDensitySeed += Math.max(sourceSmokeSeed * 1.08, sourceFireSeed * 0.62);
          }
          initialBodySeed = Math.min(initialBodySeed, 2.5);
          initialDensitySeed = Math.min(initialDensitySeed, 2.5);
          initialSmokeSeed = Math.min(initialSmokeSeed, 2.5);
          initialHeatSeed = Math.min(initialHeatSeed, 2.5);
          initialFireSeed = Math.min(initialFireSeed, 2.5);
          const i = ((x + y * nextGridSize + z * nextGridSize * nextGridSize) * FLUID_COMPONENTS);
          data[i] = -fz * initialBodySeed * 0.11;
          data[i + 1] = initialBodySeed * 0.22;
          data[i + 2] = fx * initialBodySeed * 0.11;
          data[i + 3] = initialDensitySeed * 1.25;
          data[i + 4] = initialSmokeSeed * 0.74;
          data[i + 5] = initialHeatSeed * 1.28;
          data[i + 6] = initialHeatSeed * 1.0;
          data[i + 7] = initialDensitySeed * (0.35 + 0.65 * Math.sin((fx * 18) + (fz * 11)) ** 2);
          data[i + 8] = initialFireSeed * 0.90;
          data[i + 9] = initialFireSeed * 0.42;
          data[i + 10] = initialFireSeed * (0.30 + 0.70 * Math.cos((fx * 13) - (fz * 17)) ** 2);
          data[i + 11] = 0;
          data[i + 12] = initialSmokeSeed * (0.22 + 0.78 * Math.sin((fx * 31) - (fz * 19)) ** 2);
          data[i + 13] = initialDensitySeed * (0.12 + 0.50 * Math.cos((fx * 23) + (fy * 17) - (fz * 29)) ** 2);
          data[i + 14] = initialFireSeed * (0.18 + 0.82 * Math.sin((fy * 27) + (fz * 21)) ** 2);
          data[i + 15] = initialFireSeed * 0.16;
        }
      }
    }
    return data;
