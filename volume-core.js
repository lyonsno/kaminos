...
export const LEGACY_BOUNDARY_FIRE_CLEAN_ENDPOINT = Object.freeze([0.12, 0.42, 1.75]);
export const LEGACY_BOUNDARY_FIRE_SOOT_ENDPOINT = Object.freeze([1.55, 0.86, 0.18]);
const DEFAULT_BOUNDARY_FIRE_CLEAN_COLOR = '#4a86ff';
const DEFAULT_BOUNDARY_FIRE_SOOT_COLOR = '#ffc460';
...
export function boundaryFireSrgbEndpoint(value, fallback, legacyEndpoint) {
  const hex = normalizeHexColor(value, fallback);
  const normalizedFallback = normalizeHexColor(fallback, '#ffffff');
  if (hex === normalizedFallback) return [...legacyEndpoint];
  const linear = [1, 3, 5].map(offset => (
    srgbChannelToLinear(parseInt(hex.slice(offset, offset + 2), 16) / 255)
  ));
  const peak = Math.max(...linear);
  if (peak <= Number.EPSILON) return [...legacyEndpoint];
  const legacyPeak = Math.max(...legacyEndpoint);
  return linear.map(channel => channel / peak * legacyPeak);
}

function writeBoundaryFirePaletteUniform(uniforms, offset, value, fallback, legacyEndpoint) {
  const rgb = boundaryFireSrgbEndpoint(value, fallback, legacyEndpoint);
  uniforms[offset] = rgb[0];
  uniforms[offset + 1] = rgb[1];
  uniforms[offset + 2] = rgb[2];
  uniforms[offset + 3] = 1;
...
const ANALYTIC_EMITTER_FAMILY_MODE = Object.freeze({
  wick: 1,
  nozzle: 2,
  ribbon: 3,
  ring: 4,
...
function normalizeAnalyticEmitterDescriptor(descriptor) {
  ...
  if (!descriptor || typeof descriptor !== 'object' || Array.isArray(descriptor)) {
    throw new Error('analytic emitter descriptor must be an object or null');
  }
  const family = String(descriptor.family || '');
  if (!Object.hasOwn(ANALYTIC_EMITTER_FAMILY_MODE, family)) {
    throw new Error(`unsupported analytic emitter family: ${family || 'missing-family'}`);
  }
  if (descriptor.coordinateSpace !== 'volume-local') {
  ...
  return {
    ...
    chemistry: {
      ...
      fuel: clampFinite(chemistry.fuel, 0, 3, 0),
      flame: clampFinite(chemistry.flame, 0, 4, 0),
...
function analyticEmitterComponentwiseHalfExtent(descriptor, antialiasMargin) {
  const { axis, supportAxis, radius, extent } = descriptor;
  ...
  return axis.map(component => (
    extent * Math.sqrt(Math.max(0, 1 - component * component)) + radius + antialiasMargin
  ));
}
...
fn archiveBoundarySplatHistory(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  if (candidateIndex == 0u) {
    ...
    boundarySplatHistorySlotMetadata[slot].effectiveDrawCount = boundarySplatDraw.sourceCandidateCount;
    ...
    boundarySplatHistorySlotMetadata[slot].initialized = select(0u, 1u, boundarySplatDraw.sourceCandidateCount > 0u);
  }
...
export function createKaminosVolumePrototype({
  ...
  const state = {
    ...
    boundarySplatHistoryHoldover: null,
  ...
  function analyticEmitterReceipt() {
    return {
  ...
  function rebuildFluidBindGroups() {
    ...
    if (analyticEmitterInjectionBindGroupLayout && analyticEmitterInjectionUniformBuffer) {
  ...
  function ensureBoundarySplatBuffers() {
    ...
    device.queue.writeBuffer(
      boundarySplatHistorySlotMetadataBuffer,
      0,
      new Uint32Array(BOUNDARY_SPLAT_HISTORY_SLOTS * BOUNDARY_SPLAT_HISTORY_SLOT_METADATA_STRIDE_U32),
    );
  ...
  function updateUniforms(now) {
    ...
    if (boundarySplatCameraBuffer) {
      ...
      splatCamera.set([camera.position.x, camera.position.y, camera.position.z, 1], 36);
      splatCamera.set([boundarySplatCapacity, requestedInstanceCount, historyDepth, 0], 40);
    ...
    const effectiveFlowKernel = flowKernelEffectiveControls(controlsSnapshot);
    uniforms[324] = effectiveFlowKernel.strength;
    uniforms[325] = effectiveFlowKernel.radiusWorld;
    uniforms[326] = effectiveFlowKernel.coherence;
    uniforms[327] = 0;
    const volumeExposure = clampFinite(controlsSnapshot.volumeExposure, 0, 20, 1);
    uniforms[328] = volumeExposure;
    uniforms[329] = 0;
    uniforms[330] = 0;
    uniforms[331] = 0;
    writeBoundaryFirePaletteUniform(
      uniforms,
      332,
      boundaryFireUniforms.cleanColor,
      DEFAULT_BOUNDARY_FIRE_CLEAN_COLOR,
      LEGACY_BOUNDARY_FIRE_CLEAN_ENDPOINT,
    );
    writeBoundaryFirePaletteUniform(
      uniforms,
      336,
      boundaryFireUniforms.sootColor,
      DEFAULT_BOUNDARY_FIRE_SOOT_COLOR,
      LEGACY_BOUNDARY_FIRE_SOOT_ENDPOINT,
    );
    uniforms.fill(0, 340, 360);
    ...
    uniforms[360] = controlsSnapshot.artisticSwirl === false ? 0 : 1;
    uniforms[361] = controlsSnapshot.phasedSway === false ? 0 : 1;
    uniforms[362] = 0;
    uniforms[363] = 0;
    ...
    state.artisticSwirl = uniforms[360] >= 0.5;
    state.phasedSway = uniforms[361] >= 0.5;
    ...
    state.flowKernelEffective = {
      strength: uniforms[324],
      radiusWorld: uniforms[325],
      coherence: uniforms[326],
    };
  ...
  function encodeBoundarySplats(encoder, hooks = {}) {
    ...
    state.boundarySplatHistoryWriteSlot = historyWriteSlot;
    writeBoundarySplatInstanceDescriptors();
  ...
  async function sampleBoundarySplatHistorySlotMetadata(options = {}) {
    ...
    try {
      const encoder = device.createCommandEncoder({
        label: `kaminos ${BOUNDARY_SPLAT_RENDERER_IDENTITY} history slot metadata readback encoder`,
      });
  ...
  async function sampleBoundarySplatDrawState(options = {}) {
    ...
    encoder.copyBufferToBuffer(boundarySplatIndirectBuffer, 0, readback, indirectCommandOffset, 16);
    device.queue.submit([encoder.finish()]);
    await readback.mapAsync(GPUMapMode.READ);
    const mappedRange = readback.getMappedRange();
    const drawState = new Uint32Array(mappedRange, 0, 12);
      ...
      try {
        ...
        if (state.frameCount !== sourceRenderFrameCountBefore || state.simStepCount !== simStepCountBefore) {
          throw new Error(`boundary-splat-history-holdover-source-advanced:${JSON.stringify({
            sourceRenderFrameCountBefore,
            sourceRenderFrameCountAfter: state.frameCount,
            simStepCountBefore,
            simStepCountAfter: state.simStepCount,
          })}`);
        }
        ...
        const result = {
          identity: 'boundary-splat-history-holdover-draw-v0',
          ok: true,
          authority: 'bounded-completed-live-history-replay-no-independent-simulation-no-learned-prediction',
          flameAuthority: 'bounded-completed-live-history-replay-no-independent-simulation-no-learned-prediction',
          smokeAuthority: boundarySplatHybridRequested()
          ...
          historyAllocationGeneration: slot.historyAllocationGeneration,
          archiveWriteSequence: slot.archiveWriteSequence,
          sourceCandidateGeneration: slot.sourceCandidateGeneration,
          sourceSimStepCount: slot.sourceSimStepCount,
          sourceCandidateCount: slot.sourceCandidateCount,
          effectiveDrawCount: plan.drawCount,
          slot,
          selection,
          plan,
  ...
  return {
    ...
    setAnalyticEmitterDescriptor(descriptor = null) {
      const normalized = normalizeAnalyticEmitterDescriptor(descriptor);
