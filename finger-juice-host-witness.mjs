...
async function main() {
  ...
  try {
    ...
    if (!canvasActivity.childStatePresent) throw new Error(`live Host frame missing child debug state: ${JSON.stringify(canvasActivity)}`);
    if ((canvasActivity.particleCount || 0) <= 0) throw new Error(`live Host frame has no smoke particles: ${JSON.stringify(canvasActivity)}`);
