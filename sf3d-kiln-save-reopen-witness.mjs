...
import { canReuseGeneratedOutput, judgeSf3dSmoke } from './sf3d-host-device.mjs';
...
async function loadAuthoredScene(page, fileName) {
  ...
  await page.evaluate(async name => {
    ...
    const file = new File([await response.text()], name, { type: 'application/json' });
  ...
  await page.waitForFunction(() => {
    ...
    return /^Scene (?:loaded:|load failed:)|^Invalid scene/i.test(status);
  });
  const status = await page.$eval('#info-bar', element => element.textContent);
  if (!/^Scene loaded:/i.test(status)) throw new Error(`Kaminos scene restore failed: ${status}`);
}
let browser;
let page;
save();
try {
  ...
  if (!canReuseGeneratedOutput(prior) || !prior.output.result.presentation?.source) {
    throw new Error('save/reopen requires valid same-device generated output and a reusable source report');
  }
  ...
  const routePresetId = requestedUrl.searchParams.get('settings_preset') || requestedUrl.searchParams.get('preset');
  if (!sourceSceneFile || routePresetId !== prior.expectedScene.presetId) {
  ...
  await loadAuthoredScene(page, original.file);
