...
async function loadBlockWeights(base, weightsByRole) {
  const layers = [];
  for (let layerIndex = 0; layerIndex < 32; layerIndex += 1) {
    const load = suffix => loadFloat(base, weightsByRole.get(`vit-block-stack-layer${layerIndex}-${suffix}`));
    layers.push({
      layerIndex,
...
async function loadNeckWeights(base, weightsByRole, branch) {
  const load = role => loadFloat(base, weightsByRole.get(role));
...
function request(route, requestId, inputs, outputs, routeConfig = {}) {
  return createRouteInvocationRequest(route, { requestId, inputs, outputs, routeConfig });
}

async function runFrameTrunk({ frameIndex, manifest, tensorsByRole, weightsByRole, rgba, patchProjection, prefixWeights, blockWeights, blockWeightsHash, adapter, device, commit, update }) {
  ...
  const preprocessResult = await runSam3ImagePreprocessPhaseProgramRoute({
    ...
    }, { 'pixel-values': { artifactId: `sam31-two-image:frame-${frameIndex}:pixel-values`, shape: [1, preprocessShape.height, preprocessShape.width, 3] } }),
  ...
  parity.pixelValues = maxAbs(pixelValues, await loadFloat(base, tensorsByRole.get(`frame-${frameIndex}-pixel-values`)));
  ...
  const patchResult = await runSam3ImagePatchEmbedPhaseProgramRoute({
    ...
    }, { 'patch-embeddings': { artifactId: `sam31-two-image:frame-${frameIndex}:patch-embeddings`, shape: [1, manifest.shape.patchTokens, manifest.shape.visionHiddenSize] } }),
  ...
  parity.patchEmbeddings = maxAbs(patchEmbeddings, await loadFloat(base, tensorsByRole.get(`frame-${frameIndex}-patch-embeddings`)));
  ...
  const prefixResult = await runSam3ImageVitPrefixPhaseProgramRoute({
    ...
    }, { 'vit-prefix-hidden-states': { artifactId: `sam31-two-image:frame-${frameIndex}:vit-prefix`, shape: [1, manifest.shape.patchHeight, manifest.shape.patchWidth, manifest.shape.visionHiddenSize] } }),
  ...
  parity.vitPrefix = maxAbs(prefixHiddenStates, await loadFloat(base, tensorsByRole.get(`frame-${frameIndex}-vit-prefix-hidden-states`)));
  ...
  const stackResult = await runSam3ImageVitBlockStackPhaseProgramRoute({
    ...
    }, { 'vit-block-stack-hidden-states': { artifactId: `sam31-two-image:frame-${frameIndex}:vit-backbone`, shape: [1, manifest.shape.patchHeight, manifest.shape.patchWidth, manifest.shape.visionHiddenSize] } }),
  ...
  parity.vitBackbone = maxAbs(backboneHiddenStates, await loadFloat(base, tensorsByRole.get(`frame-${frameIndex}-vit-backbone-hidden-states`)));
...
async function runNeck({ branch, frameIndex, manifest, tensorsByRole, weightsByRole, weights, backbone, adapter, device, commit, update }) {
  const base = '/oracle/ingress';
  ...
  const outputs = {};
  for (let level = 0; level < 3; level += 1) outputs[`${outputPrefix}-feature-${level}`] = { artifactId: `sam31-two-image:frame-${frameIndex}:${branch}-feature-${level}`, shape: [1, manifest.shape.fpnLevels[level].height, manifest.shape.fpnLevels[level].width, 256] };
  outputs[`${outputPrefix}-position-2`] = { artifactId: `sam31-two-image:frame-${frameIndex}:${branch}-position-2`, shape: [1, manifest.shape.patchHeight, manifest.shape.patchWidth, 256] };
  ...
  for (let level = 0; level < 3; level += 1) {
    ...
    parity[`feature${level}`] = maxAbs(features[level], await loadFloat(base, tensorsByRole.get(`frame-${frameIndex}-${branch}-feature-${level}`)));
  }
  const position2 = new Float32Array(result.debugReadback[`${branch}Position2`]);
  parity.position2 = maxAbs(position2, await loadFloat(base, tensorsByRole.get(`frame-${frameIndex}-${branch}-position-2`)));
  return { result, route, features, position2, parity };
}

async function runHighResolution({ branch, frameIndex, manifest, tensorsByRole, weightsByRole, weights, neck, adapter, device, commit, update }) {
  const base = '/oracle/ingress';
  ...
  const result = await runSam31DecoderHighResolutionProjectionPhaseProgramRoute({
    request: request(route, `sam31-two-image-high-resolution-${branch}-${frameIndex}-${Date.now()}`, {
      ...
      'sam31-decoder-high-resolution-s0': { artifactId: `sam31-two-image:frame-${frameIndex}:${branch}-high-resolution-s0`, shape: [1, 32, manifest.shape.fpnLevels[0].height, manifest.shape.fpnLevels[0].width] },
      'sam31-decoder-high-resolution-s1': { artifactId: `sam31-two-image:frame-${frameIndex}:${branch}-high-resolution-s1`, shape: [1, 64, manifest.shape.fpnLevels[1].height, manifest.shape.fpnLevels[1].width] },
    }),
    route, adapter, device, queue: device.queue, adapterName: adapter.info?.description || 'browser-webgpu-adapter', browser: navigator.userAgent,
    model: { ...modelFor(manifest), weightsHash }, kernel: route.kernel,
  ...
  return {
    ...
    parity: {
      highResolutionS0: maxAbs(highResolutionS0, await loadFloat(base, tensorsByRole.get(`${rolePrefix}-s0`))),
      highResolutionS1: maxAbs(highResolutionS1, await loadFloat(base, tensorsByRole.get(`${rolePrefix}-s1`))),
    },
  };
}

export async function runSam31TwoImageBackbone({ manifest, adapter, device, errors, commit = null, update = () => {} }) {
  ...
  const rgba = [await loadRgba(base, tensorsByRole.get('frame-0-rgba')), await loadRgba(base, tensorsByRole.get('frame-1-rgba'))];
  ...
  const patchProjection = await loadFloat(base, weightsByRole.get('patch-embed-projection-weight'));
  ...
  const spatialPositions = resolveSam31SpatialPositionEmbeddings({
    values: await loadFloat(base, positionEntry),
  ...
  const prefixWeights = {
    ...
    values: {
      ...
      layerNormWeight: await loadFloat(base, weightsByRole.get('vit-backbone-layernorm-weight')),
      layerNormBias: await loadFloat(base, weightsByRole.get('vit-backbone-layernorm-bias')),
    },
  };
  const blockWeights = await loadBlockWeights(base, weightsByRole);
  const blockWeightsHash = await sha256Text(blockWeightRoles().map(role => `${role}:${weightsByRole.get(role).sha256}`).join('\n'));
  const backbones = [];
  for (let frameIndex = 0; frameIndex < 2; frameIndex += 1) backbones.push(await runFrameTrunk({ frameIndex, manifest, tensorsByRole, weightsByRole, rgba: rgba[frameIndex], patchProjection, prefixWeights, blockWeights, blockWeightsHash, adapter, device, commit, update }));
  blockWeights.layers.length = 0;

  update('load-interactive-neck-weights');
  const interactiveWeights = await loadNeckWeights(base, weightsByRole, 'interactive');
  const interactive = await runNeck({ branch: 'interactive', frameIndex: 0, manifest, tensorsByRole, weightsByRole, weights: interactiveWeights, backbone: backbones[0], adapter, device, commit, update });
  interactiveWeights.levels.length = 0;

  update('load-propagation-neck-weights');
  const propagationWeights = await loadNeckWeights(base, weightsByRole, 'propagation');
  const propagation = [];
  for (let frameIndex = 0; frameIndex < 2; frameIndex += 1) propagation.push(await runNeck({ branch: 'propagation', frameIndex, manifest, tensorsByRole, weightsByRole, weights: propagationWeights, backbone: backbones[frameIndex], adapter, device, commit, update }));
  propagationWeights.levels.length = 0;

  const projectionWeights = {
    s0: { weight: await loadFloat(base, weightsByRole.get('decoder-high-resolution-s0-weight')), bias: await loadFloat(base, weightsByRole.get('decoder-high-resolution-s0-bias')) },
    s1: { weight: await loadFloat(base, weightsByRole.get('decoder-high-resolution-s1-weight')), bias: await loadFloat(base, weightsByRole.get('decoder-high-resolution-s1-bias')) },
  };
  const frame0High = await runHighResolution({ branch: 'interactive', frameIndex: 0, manifest, tensorsByRole, weightsByRole, weights: projectionWeights, neck: interactive, adapter, device, commit, update });
  const frame1High = await runHighResolution({ branch: 'propagation', frameIndex: 1, manifest, tensorsByRole, weightsByRole, weights: projectionWeights, neck: propagation[1], adapter, device, commit, update });
  ...
  const parityPassed = backbones.every(frame => frame.parity.pixelValues <= tolerances.pixelValuesMaxAbsDiff && frame.parity.patchEmbeddings <= tolerances.patchEmbeddingsMaxAbsDiff && frame.parity.vitPrefix <= tolerances.vitPrefixMaxAbsDiff && frame.parity.vitBackbone <= tolerances.vitBackboneMaxAbsDiff)
  ...
  return {
    ...
    receipts, requestedRouteIds, effectiveRouteIds: receipts.map(receipt => receipt.effectiveRouteId), parity: maximums, parityMaximum, routeChainPassed, parityPassed,
