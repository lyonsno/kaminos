...
async function runFrameTrunk({ frameIndex, manifest, tensorsByRole, weightsByRole, rgba, patchProjection, prefixWeights, blockWeights, blockWeightsHash, adapter, device, commit, update, loadFloat32, verificationAttached, invocationTag, requestIds }) {
  ...
  const expectedLayerCheckpoints = verificationAttached && manifest.diagnosticVitLayers?.length
    ? await Promise.all(manifest.diagnosticVitLayers.map(async layerIndex => {
        const role = `frame-${frameIndex}-vit-layer-${layerIndex}-hidden-states`;
        const entry = tensorsByRole.get(role);
        if (!entry) throw new Error(`selected ViT checkpoint tensor missing: ${role}`);
...
export async function runSam31TwoImageBackbone({
  ...
  const { vitLayerDiagnostics: frame0LayerDiagnostics, vitBackboneDiagnostics: frame0BackboneDiagnostics, vitPrefixDiagnostics: frame0PrefixDiagnostics, ...frame0Parity } = backbones[0].parity;
  const { vitLayerDiagnostics: frame1LayerDiagnostics, vitBackboneDiagnostics: frame1BackboneDiagnostics, vitPrefixDiagnostics: frame1PrefixDiagnostics, ...frame1Parity } = backbones[1].parity;
  const layerDiagnostics = {
    frame0: { vitPrefix: frame0PrefixDiagnostics, vitBackbone: frame0BackboneDiagnostics, vitLayers: frame0LayerDiagnostics },
    frame1: { vitPrefix: frame1PrefixDiagnostics, vitBackbone: frame1BackboneDiagnostics, vitLayers: frame1LayerDiagnostics },
  };
  const maximums = {
  ...
  if (!routeChainPassed || !numericalVerification.gatePassed) throw Object.assign(new Error(`two-image backbone evidence failed: ${JSON.stringify({ routeChainPassed, numericalVerification, maximums, diagnostics: layerDiagnostics })}`), { backboneEvidence: { routeChainPassed, numericalVerification, maximums, diagnostics: layerDiagnostics } });
  return {
    ...
    receipts, requestIds, requestedRouteIds, effectiveRouteIds: receipts.map(receipt => receipt.effectiveRouteId), parity: verificationAttached ? maximums : null, parityDiagnostics: verificationAttached ? layerDiagnostics : null, compoundParity, parityMaximum: verificationAttached ? parityMaximum : null, routeChainPassed, parityPassed: numericalVerification.passed, parityState: numericalVerification.state, parityGatePassed: numericalVerification.gatePassed, numericalVerification, verificationAttached,
