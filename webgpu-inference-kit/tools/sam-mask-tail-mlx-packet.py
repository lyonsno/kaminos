...
ROUTE_ID = "sam3.mask-tail.phase-program.webgpu-local.v0"
SCHEMA = "kaminos.sam3-mask-tail-real-boundary-packet.v0"
BOUNDARY = "sam3-detector-mask-tail-phase-program"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Export real MLX SAM3 mask-tail tensors and weights.")
...
def run_reference(model, image, prompt, resolution):
    ...
    mx.eval(hs[-1], pixel_embed, instance_embed, mask_embeddings, pred_logits, presence_logits)
    ...
    return last_hs, pixel, mask_emb, upscaled, logits, binary, selected, scores, np.array(presence_logits, dtype=np.float32).reshape(-1)
...
def main():
    ...
    last_hs, pixel, mask_emb, upscaled, logits, binary, selected, scores, presence = run_reference(model, image, args.prompt, args.resolution)
    shape = {"batch": int(last_hs.shape[0]), "maskTokens": int(last_hs.shape[1]), "channels": int(last_hs.shape[2]), "height": int(pixel.shape[1]), "width": int(pixel.shape[2])}
    params = dict(flatten(model.parameters()))
    weight_specs = [
        ("mask-embedder-layer-0-weight", "detector_model.mask_decoder.mask_embedder.layers.0.weight", "mask-embedder-layer-0-weight.f32.bin"),
        ("mask-embedder-layer-0-bias", "detector_model.mask_decoder.mask_embedder.layers.0.bias", "mask-embedder-layer-0-bias.f32.bin"),
        ("mask-embedder-layer-1-weight", "detector_model.mask_decoder.mask_embedder.layers.1.weight", "mask-embedder-layer-1-weight.f32.bin"),
        ("mask-embedder-layer-1-bias", "detector_model.mask_decoder.mask_embedder.layers.1.bias", "mask-embedder-layer-1-bias.f32.bin"),
        ("mask-embedder-layer-2-weight", "detector_model.mask_decoder.mask_embedder.layers.2.weight", "mask-embedder-layer-2-weight.f32.bin"),
        ("mask-embedder-layer-2-bias", "detector_model.mask_decoder.mask_embedder.layers.2.bias", "mask-embedder-layer-2-bias.f32.bin"),
        ("instance-projection-weight", "detector_model.mask_decoder.instance_projection.weight", "instance-projection-weight.f32.bin"),
        ("instance-projection-bias", "detector_model.mask_decoder.instance_projection.bias", "instance-projection-bias.f32.bin"),
    ]
    tensors = [
        ("last-hs", "last-hs.f32.bin", last_hs, [shape["batch"], shape["maskTokens"], shape["channels"]], "B,T,C"),
        ("pixel-embed", "pixel-embed.f32.bin", pixel, [shape["batch"], shape["height"], shape["width"], shape["channels"]], "B,H,W,C"),
    ...
    tensor_entries = []
    for role, file, array, tensor_shape, layout in tensors:
    ...
    weight_entries = []
    for role, key, file in weight_specs:
        array = np.array(params[key], dtype=np.float32)
        if role == "instance-projection-weight":
            array = array[:, 0, 0, :]
        written = write_array(out_dir / file, array)
        weight_entries.append({"role": role, "file": file, "sha256": written["sha256"], "dtype": "float32", "shape": list(array.shape), "layout": "out,in" if "weight" in role else "out", "byteLength": written["byteLength"]})
    ...
    manifest = {
        ...
        "staticWeights": {"artifactId": f"sam3-weights:{args.model}:mask-tail-reference-upstream", "sha256": weights_sha, "role": "reference-upstream", "reason": "weights exported for browser mask-tail phase-program execution"},
        "shape": shape,
        "claims": {"fullSam3BrowserExecution": False, "upstream": "mlx-vlm-sam3-detector-reference", "browserExecutedStages": ["mask-embedder", "instance-projection", "decode-mask", "threshold-mask"]},
        ...
        "tolerances": {"maskEmbeddingsMaxAbsDiff": 0.0001, "upscaledEmbeddingMaxAbsDiff": 0.0001, "webGpuLogitsMaxAbsDiff": 0.0002, "binaryMismatchCount": 0},
        "tensors": tensor_entries,
        "weights": weight_entries,
    }
    manifest_path = out_dir / "tensor-manifest.json"
    receipt = {"ok": True, "schema": "kaminos.sam3-mask-tail-reference-receipt.v0", "routeId": ROUTE_ID, "mode": manifest["mode"], "boundary": BOUNDARY, "reference": reference, "shape": shape, "outputs": {"tensorManifest": str(manifest_path), "sourceImage": str(source_path)}}
