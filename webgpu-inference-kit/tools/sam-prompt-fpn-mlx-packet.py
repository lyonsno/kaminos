...
ROUTE_ID = "sam3.prompt-fpn.phase-program.webgpu-local.v0"
SCHEMA = "kaminos.sam3-prompt-fpn-real-boundary-packet.v0"
BOUNDARY = "sam3-detector-prompt-fpn-phase-program"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Export real MLX SAM3 prompt-FPN tensors and weights.")
...
def run_reference(model, image, prompt, resolution):
    ...
    mx.eval(*fpn_trimmed, encoded, inputs_embeds, attention_mask, prompt_fpn_feature, *feats_for_fpn, hs[-1], pixel_embed, instance_embed, mask_embeddings, pred_logits, presence_logits)
...
def main():
    ...
    prompt_feature = ref["prompt_fpn_feature"]
    shape = {
        "batch": int(prompt_feature.shape[0]),
        "channels": int(prompt_feature.shape[3]),
        "heads": 8,
        "spatialTokens": int(ref["encoder_hidden_states"].shape[1]),
        "promptTokens": int(ref["prompt_features"].shape[1]),
        "height": int(prompt_feature.shape[1]),
        "width": int(prompt_feature.shape[2]),
        "groups": 8,
    ...
    for spec in prompt_weights:
        add_weight(weight_entries, out_dir, params, *spec)
    for stage in range(len(ref["composed_features"]) - 1):
    ...
    manifest = {
        ...
        "staticWeights": {"artifactId": f"sam3-weights:{args.model}:prompt-fpn-reference-upstream", "sha256": weights_sha, "role": "reference-upstream", "reason": "weights exported for browser prompt-FPN, pixel-decoder, and mask-tail phase-program execution"},
        "shape": shape,
        "claims": {"fullSam3BrowserExecution": False, "upstream": "mlx-vlm-sam3-detector-reference", "browserExecutedStages": ["prompt-cross-attention-fpn", "pixel-decoder", "mask-embedder", "instance-projection", "decode-mask", "threshold-mask"]},
        ...
        "tolerances": {"promptFpnMaxAbsDiff": 0.0003, "pixelEmbedMaxAbsDiff": 0.0005, "maskEmbeddingsMaxAbsDiff": 0.0001, "upscaledEmbeddingMaxAbsDiff": 0.0001, "webGpuLogitsMaxAbsDiff": 0.001, "cpuOracleBinaryMismatchCount": 8, "binaryMismatchCount": 8},
        "tensors": tensor_entries,
        "weights": weight_entries,
    }
    manifest_path = out_dir / "tensor-manifest.json"
    receipt = {"ok": True, "schema": "kaminos.sam3-prompt-fpn-reference-receipt.v0", "routeId": ROUTE_ID, "mode": manifest["mode"], "boundary": BOUNDARY, "reference": reference, "shape": shape, "outputs": {"tensorManifest": str(manifest_path), "sourceImage": str(source_path)}}
