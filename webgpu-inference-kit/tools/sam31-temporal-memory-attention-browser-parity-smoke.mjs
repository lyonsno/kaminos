...
async function inspectScreenshotPixels(webSocket, pngBase64, expectedBorderX, expectedBorderTop) {
  ...
  return evaluate(webSocket, `(async () => {
    ...
    return {
      ...
      maximumChannel,
      expectedBorderX: borderX,
      expectedBorderTop: borderTop,
      borderSignalPixels: borderSignals,
...
async function main() {
  ...
  try {
    ...
    viewportLayout = await evaluate(webSocket, `(() => {
      window.scrollTo(0, 0);
      ...
      const layout = {
        ...
        heading: { left: heading.left, right: heading.right, top: heading.top },
        status: { left: status.left, right: status.right, top: status.top },
      };
      layout.layoutPassed = layout.scrollX === 0 && layout.scrollY === 0
        && layout.scrollWidth <= layout.innerWidth
        && heading.left >= 0 && heading.right <= layout.innerWidth && heading.top >= 0
        && status.left >= 0 && status.right <= layout.innerWidth && status.top >= 0;
      return layout;
    })()`);
    const layoutPassed = viewportLayout.layoutPassed === true;
    if (!layoutPassed) throw new Error(`receipt surface is clipped outside the viewport: ${JSON.stringify(viewportLayout)}`);
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      const pixels = await inspectScreenshotPixels(webSocket, screenshot.data, viewportLayout.status.left + 1, viewportLayout.status.top + 4);
