      ...
      for (const transposed of variant === 'sequential4' ? [0] : [0, 1]) {
        for (const channels of variant === 'sequential4' ? [8, 1024] : [7, 8]) {
          for (const activation of ['identity', 'relu', 'helper']) {
            const activationOptions = activation === 'identity' ? {} : activation === 'relu'
              ? { activationExpression: 'max(value, 0.0)' }
              : { activationExpression: 'user_activation(value)', activationHelpers:
                'fn user_activation(value: f32) -> f32 { if (value < 0.0) { return value * 0.5; } return value * 2.0; }' };
            ...
            const linearExpected = Array.from(expected);
            for (let row = start; row < start + count; row += 1) {
              for (let col = 0; col < columns; col += 1) {
                const index = row * columns + col, value = expected[index];
                if (activation === 'relu') expected[index] = Math.max(value, 0);
                if (activation === 'helper') expected[index] = Math.fround(value < 0 ? value * 0.5 : value * 2);
              }
            }
            const arms = [];
            for (const arm of ['original', 'f32', 'f16-packed-u32']) {
              const code = arm === 'original' ? original : createWebGpuLinearShader({ variant, weightStorage: arm, ...activationOptions });
              const allocations = [];
            ...
            const result = { variant, transposed, rows, columns, channels, start, count, activation, linearExpected,
