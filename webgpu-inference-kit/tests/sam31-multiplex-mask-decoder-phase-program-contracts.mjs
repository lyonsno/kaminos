...
assert.match(decoderSource, /role: 'sam31-multiplex-object-scores'/, 'object-presence scores must be an authenticated route output for memory conditioning');
assert.match(browserSource, /'sam31-multiplex-object-scores'/, 'the browser request must bind the object-score output artifact');
for (const token of ['verifySam31PacketFloat32Bytes', 'runSam31MultiplexMaskDecoderPhaseProgramRoute', 'layer0Queries', 'objectPointers', 'mappedTensorCount === 133', 'branchCoveragePassed']) assert.match(browserSource, new RegExp(token), `browser witness must make ${token} load-bearing`);
