...
const smokeAdapter = readFileSync(new URL('../smokes/sam-mask-island-parity.js', import.meta.url), 'utf8');
...
assert.match(smokeAdapter, /samMaskIslandParitySmokeState = runtime.evidence/, 'browser smoke must expose the host runtime evidence through its existing debug hook');
...
assert.match(smokeJs, /run: runInvocation/, 'runtime must expose a host-callable invocation entry point');
...
assert.match(smokeJs, /const diagnostic =/, 'runtime must expose opt-in full pre-decoder diagnostic readback');
