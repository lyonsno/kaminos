...
const vectorLinearSource = existsSync(vectorLinearSourceUrl) ? readFileSync(vectorLinearSourceUrl, 'utf8') : '';
...
assert.match(vectorLinearSource, /if \(x < -10\.0\) \{ return 0\.0; \}/, 'ViT MLP GELU shader must saturate its negative tail before cubic overflow can produce NaN');
assert.match(vectorLinearSource, /if \(x > 10\.0\) \{ return x; \}/, 'ViT MLP GELU shader must saturate its positive tail before cubic overflow');
assert.match(vectorLinearSource, /fn mlx_erf\(x: f32\)/, 'ViT MLP GELU shader must port the MLX Metal erf implementation used by the reference backend');
assert.match(vectorLinearSource, /fn mlx_expm1f\(x: f32\)/, 'ViT MLP GELU shader must port MLX Metal expm1 rather than substitute a different erf family');
assert.match(vectorLinearSource, /0\.927734375/, 'ViT MLP GELU shader must preserve the MLX Metal erf branch boundary');
assert.doesNotMatch(vectorLinearSource, /0\.044715/, 'ViT block-stack GPU and CPU GELU paths must not retain the tanh approximation');
...
assert.match(routeSource, /vectorLinearDispatch/, 'ViT dense projections must use the shared vector-linear dispatch contract');
...
assert.deepEqual(dispatch1008Local.qProjection, { logicalInvocations: 5_308_416, dispatch: [288, 288] });
