...
const blockedLinearSourceUrl = new URL('../src/sam-blocked-linear-wgsl.js', import.meta.url);
...
const blockedLinearSource = existsSync(blockedLinearSourceUrl) ? readFileSync(blockedLinearSourceUrl, 'utf8') : '';
...
assert.match(blockedLinearSource, /if \(x < -10\.0\) \{ return 0\.0; \}/, 'ViT MLP GELU shader must saturate its negative tail before cubic overflow can produce NaN');
assert.match(blockedLinearSource, /if \(x > 10\.0\) \{ return x; \}/, 'ViT MLP GELU shader must saturate its positive tail before cubic overflow');
assert.match(blockedLinearSource, /fn mlx_erf\(x: f32\)/, 'ViT MLP GELU shader must port the MLX Metal erf implementation used by the reference backend');
assert.match(blockedLinearSource, /fn mlx_expm1f\(x: f32\)/, 'ViT MLP GELU shader must port MLX Metal expm1 rather than substitute a different erf family');
assert.match(blockedLinearSource, /0\.927734375/, 'ViT MLP GELU shader must preserve the MLX Metal erf branch boundary');
assert.doesNotMatch(blockedLinearSource, /0\.044715/, 'ViT block-stack GPU and CPU GELU paths must not retain the tanh approximation');
...
assert.match(routeSource, /blockedLinearDispatch/, 'ViT dense projections must use the shared blocked-linear dispatch contract');
...
const blockedPhases = new Set(['qProjection', 'kProjection', 'vProjection', 'outputProjection', 'mlpFc1', 'mlpFc2']);
for (const [phase, entry] of Object.entries(dispatch1008Global)) {
  ...
  const outputsPerWorkgroup = blockedPhases.has(phase) ? 256 : 64;
