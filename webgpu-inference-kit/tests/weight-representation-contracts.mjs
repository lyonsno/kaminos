...
assert.equal(native.storageDtype, 'f16');
...
assert.throws(
  () => createWebGpuWeightRepresentationPlan({
    sourceDtype: 'fp32', elementCount: 8, candidates: ['f16-packed-u32'], adapterFeatures: [],
  }),
  /unsupported sourceDtype fp32/,
);
assert.throws(
  () => createWebGpuWeightRepresentationPlan({
    sourceDtype: 'fp16', elementCount: 8, candidates: ['f16-native'], adapterFeatures: [],
  }),
  /no requested weight representation is supported/,
);
assert.throws(
  () => createWebGpuWeightRepresentationPlan({
    sourceDtype: 'fp16', elementCount: 8, candidates: ['f32-expanded'],
    adapterFeatures: [], maxStorageByteLength: 16,
  }),
  /exceeds maxStorageByteLength/,
);
assert.throws(
  () => createWebGpuWeightRepresentationPlan({
    sourceDtype: 'fp16', elementCount: 8, candidates: ['fp8-native'], adapterFeatures: ['shader-f16'],
  }),
  /unsupported weight representation/,
);
assert.throws(
  () => createWebGpuWeightRepresentationPlan({
    sourceDtype: 'fp16', elementCount: Number.MAX_SAFE_INTEGER,
    candidates: ['f16-packed-u32'], adapterFeatures: [],
  }),
  /safe byte lengths/,
);
assert.throws(
  () => createWebGpuWeightRepresentationPlan({
    sourceDtype: 'fp16', elementCount: 8, candidates: ['f16-native'], adapterFeatures: 'shader-f16',
  }),
