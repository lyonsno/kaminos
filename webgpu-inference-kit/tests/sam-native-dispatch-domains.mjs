...
} from '../src/sam-vector-linear-wgsl.js';
...
const shaderClasses = {
  ...
  'sam-detr-decoder': {
    ...
    SAM_VECTOR_LINEAR: 'Ref2 SelfQ SelfK SelfV SelfOut TextQ TextK TextV TextOut VisionQ VisionK VisionV VisionOut Mlp2 BoxHead3',
    SAM_VECTOR_LINEAR_RELU: 'Ref1 Mlp1 BoxHead1 BoxHead2', SAM_DECODER_MASKED_ONLINE_ATTENTION: 'SelfAttn TextAttn', SAM_BIASED_ONLINE_ATTENTION: 'VisionAttn',
  ...
  'sam-mask-tail': { SAM_VECTOR_LINEAR_RELU: 'maskEmbedderLayer0 maskEmbedderLayer1', SAM_VECTOR_LINEAR: 'maskEmbedderLayer2', INSTANCE_PROJECTION: 'instanceProjection', MASK_PROJECTION: 'decodeMask', THRESHOLD: 'thresholdMask' },
};
...
function checkProduction(route, source, shape, index = 0) {
  ...
  if (linearHelper) {
    ...
    )(vectorLinearDispatch, vectorLinearDispatchForDevice);
    bindings.linearWorkgroups = (tokens, inputChannels, outputChannels, device) => {
      logicalTotal = tokens * outputChannels;
      vectorDispatch = linearWorkgroups(tokens, inputChannels, outputChannels, device);
      return vectorDispatch;
    };
  }
  ...
  for (const [, nameExpression, kernelExpression, dispatchExpression] of phases) {
    ...
    const vector = expectedShaders[tokens[name]].startsWith('SAM_VECTOR_LINEAR');
    ...
    if (nativeDispatch) {
      ...
      assert.ok(dispatch.reduce((a, b) => a * b, 1) * 64 >= total, `${fullName}: vector tail coverage`);
    } else {
  ...
  for (const [height, width, batch] of [[16, 16, 1], [72, 72, 1], [9, 13, 2]]) {
    ...
    if (route === 'sam-detr-encoder') {
      ...
      const bad = source.replace(
        'linearWorkgroups(spatialTokenCount, shape.channels, shape.mlpHidden, input.device)',
        'linearWorkgroups(spatialTokenCount, shape.channels, shape.channels, input.device)',
      );
      assert.notEqual(bad, source);
