...
- `createKimodoTextToMotionRouteDefinition(input)` and `createKimodoTextToMotionRouteReceipt(input)` define the Kimodo text-prompt to motion route as shipped by the browser port: `soma-joints` output `[frames, 30, 3]` (30-joint SOMA skeleton, duration-dependent frame count at 30 fps) and a `motion-clip` output of `[frames, 369]` motion-feature rows, with an optional filmstrip. The shape law is enforced at every authority boundary: the receipt factory throws through it, request and worker-result validation apply it, and route-bound receipt authority applies it through the artifact-law registry.
- Route-bound receipt authority: `assertAuthoritativeRouteReceipt(receipt)` asserts GENERIC ENVELOPE authority only — it does not know any route's semantic law. Consumers of routes that carry laws (Kimodo) must pass the route: `assertAuthoritativeRouteReceipt(receipt, route)` and `classifyWebGpuRouteReceiptEvidence(receipt, { route })` bind the receipt to the route's identity (mismatch is a legible `route-mismatch`, decided before any law applies) and enforce the route's artifact law. Laws travel as serializable `{ id, version }` descriptors on the route definition and resolve through the trusted local registry (`registerArtifactLaw` / `requireArtifactLaw`); a definition that lost a required descriptor fails `validateRouteDefinition`, and an unresolvable descriptor never confers authority.
