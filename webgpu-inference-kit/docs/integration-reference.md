...
- `createWebGpuAdaptiveCommandDutyPlanner(input)` for exact within-kernel ranges that grow or shrink toward a caller-owned completed-duty duration. The caller supplies the total work, initial range, duration target, and minimum/maximum chunk bounds; optional `adjustmentGain` exponent-damps each measured correction to control oscillation while preserving full-gain behavior at the default `1`. Observations admit completed queue time as `queue-work-done` or a strictly positive in-command-buffer timestamp range as `gpu-timestamp-query`; other timing authorities fail without advancing coverage. Receipts keep the exact timing authority and the raw full-gain recommendation separate from the effective correction. The planner preserves complete coverage and does not pretend to know the final range count until execution finishes.
...
Only one range may be pending at a time. Stale range ids and timing outside the `queue-work-done` or `gpu-timestamp-query` allowlist fail without advancing coverage. Queue duration remains finite and non-negative; GPU timestamp duration must be finite and greater than zero. Every range carries exact completed-items-over-total progress; `actualRangeCount` remains `null` until the final range completes. History is uncapped, and the planner never invents chunk bounds beyond those supplied by the model adapter or higher-level governor.
