  ...
  return withSamPhaseCleanup(runtime, async () => {
    ...
    for (let layerIndex = 0; layerIndex < shape.layerCount; layerIndex += 1) {
      ...
      phases.push(
        ...
        { name: `detr-encoder-self-q-${layerIndex}`, kernel: `${kernelBase}SelfQ`, dispatch: linearWorkgroups(spatialTokenCount, shape.channels, shape.channels, input.device), yieldAfter: true },
        { name: `detr-encoder-self-k-${layerIndex}`, kernel: `${kernelBase}SelfK`, dispatch: linearWorkgroups(spatialTokenCount, shape.channels, shape.channels, input.device), yieldAfter: true },
        { name: `detr-encoder-self-v-${layerIndex}`, kernel: `${kernelBase}SelfV`, dispatch: linearWorkgroups(spatialTokenCount, shape.channels, shape.channels, input.device), yieldAfter: true },
        ...
        { name: `detr-encoder-self-output-linear-${layerIndex}`, kernel: `${kernelBase}SelfOutput`, dispatch: linearWorkgroups(spatialTokenCount, shape.channels, shape.channels, input.device), yieldAfter: true },
        ...
        { name: `detr-encoder-cross-q-${layerIndex}`, kernel: `${kernelBase}CrossQ`, dispatch: linearWorkgroups(spatialTokenCount, shape.channels, shape.channels, input.device), yieldAfter: true },
        { name: `detr-encoder-cross-k-${layerIndex}`, kernel: `${kernelBase}CrossK`, dispatch: linearWorkgroups(promptTokenCount, shape.channels, shape.channels, input.device), yieldAfter: true },
        { name: `detr-encoder-cross-v-${layerIndex}`, kernel: `${kernelBase}CrossV`, dispatch: linearWorkgroups(promptTokenCount, shape.channels, shape.channels, input.device), yieldAfter: true },
        ...
        { name: `detr-encoder-cross-output-linear-${layerIndex}`, kernel: `${kernelBase}CrossOutput`, dispatch: linearWorkgroups(spatialTokenCount, shape.channels, shape.channels, input.device), yieldAfter: true },
        ...
        { name: `detr-encoder-mlp-fc1-relu-${layerIndex}`, kernel: `${kernelBase}MlpFc1Relu`, dispatch: linearWorkgroups(spatialTokenCount, shape.channels, shape.mlpHidden, input.device), yieldAfter: true },
        { name: `detr-encoder-mlp-fc2-linear-${layerIndex}`, kernel: `${kernelBase}MlpFc2`, dispatch: linearWorkgroups(spatialTokenCount, shape.mlpHidden, shape.channels, input.device), yieldAfter: true },
