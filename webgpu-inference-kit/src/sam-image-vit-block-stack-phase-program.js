...
export function createSam3ImageVitBlockStackDispatchPlan(input = {}) {
  ...
  const paddedTokens = shape.batch * layerShape.windowCount * layerShape.windowTokens;
  const linear = (tokens, inputChannels, outputChannels) => ({
    logicalInvocations: tokens * outputChannels,
    dispatch: vectorLinearDispatch(tokens, inputChannels, outputChannels, { maxWorkgroupsPerDimension }),
  });
  return {
    layerNorm1: entry(shape.tokenCount),
    windowPartition: padded(),
    qProjection: linear(paddedTokens, shape.hiddenSize, shape.hiddenSize),
    kProjection: linear(paddedTokens, shape.hiddenSize, shape.hiddenSize),
    vProjection: linear(paddedTokens, shape.hiddenSize, shape.hiddenSize),
    qRope: padded(),
    kRope: padded(),
    attention: {
    ...
    outputProjection: linear(paddedTokens, shape.hiddenSize, shape.hiddenSize),
    windowUnpartition: total(),
    layerNorm2: entry(shape.tokenCount),
    mlpFc1: linear(shape.tokenCount, shape.hiddenSize, shape.intermediateSize),
    mlpFc2: linear(shape.tokenCount, shape.intermediateSize, shape.hiddenSize),
    residualMlp: total(),
  };
}
  ...
  return withSamPhaseCleanup(runtime, async () => {
    ...
    if (input.includeReadback === true) {
      authoritative.debugReadback = {
        ...
        vitBlockStackHiddenStates: sam3Readback(input, new Float32Array(readback.vitBlockStackHiddenStates)),
      };
    }
