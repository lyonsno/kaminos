...
export function createSam3BrowserImageRuntime(options = {}) {
const baseUrl = options.baseUrl || globalThis.location?.href || 'http://localhost/';
const { statusEl, summaryEl, reportEl, canvas, sourceImageEl } = options.elements || {};
let closed = false;
const sourceObjectUrls = new Set();
...
const defaultVerificationMode = options.verificationMode || 'execution-only';
...
const applicationSession = options.inferenceSession || null;
const servingResources = createSam3BrowserServingResources({
  ...
  async acquireExecutionContext() {
    ...
    try {
      await options.onExecutionContext?.(context);
      return context;
    } catch (error) {
...
const diagnostic = ({ tensorName = null, base64Offset = 0, base64Length = null } = {}) => {
...
function setStatus(status, message = status) {
  ...
  options.onStatus?.({ status, message, error: state.error });
  if (reportEl && (defaultVerificationMode !== 'execution-only' || diagnosticReadbackEnabled)) {
...
function resolveManifestFile(file) {
  return resolveSam3BrowserArtifactUrl(file, activeManifestUrl, baseUrl);
}
...
async function loadSourceImageAsset(url, identity) {
  ...
  sourceObjectUrls.add(objectUrl);
  const image = await loadImage(objectUrl);
  if (identity?.encodedResolution && (image.naturalWidth !== identity.encodedResolution[0] || image.naturalHeight !== identity.encodedResolution[1])) {
    throw new Error('SAM3 decoded source dimensions differ from the caller image identity');
  }
  return {
    image,
    objectUrl,
...
async function main(manifestUrl = initialManifestUrl, invocationOptions = {}) {
  ...
  try {
    ...
    if (!verificationAttached) {
      ...
      const sourceImageUrl = new URL(sourceImage.url, baseUrl);
      if (sourceImageUrl.origin !== new URL(baseUrl).origin) {
        throw new Error(`execution-only SAM3 source image must be same-origin: ${sourceImageUrl.origin}`);
      }
      if (!['http:', 'https:', 'blob:'].includes(sourceImageUrl.protocol)) {
        throw new Error(`unsupported SAM3 source image protocol: ${sourceImageUrl.protocol}`);
      }
      manifest.prompt = {
        text: promptText,
    ...
    if (sourceImageAsset) {
      if (sourceImageEl) sourceImageEl.src = sourceImageAsset.objectUrl;
    ...
    if (gpuBinary) {
      ...
      if (selectionKeep) {
        ...
        for (let index = 0; index < selectionKeep.length; index += 1) {
          ...
          instances.push({
            ...
            logits: gpuLogits?.slice(index * maskElementCount, (index + 1) * maskElementCount) || null,
      ...
      visualOutput = {
        ...
        foregroundScheduling: options.foregroundEvidence?.() || null,
    ...
    for (const url of sourceObjectUrls) URL.revokeObjectURL(url);
    sourceObjectUrls.clear();
  }
}

let invocationPromise = null;
const runInvocation = (manifestUrl, invocationOptions = {}) => {
  if (closed) throw new Error('SAM3 image runtime is closed');
...
return {
  run: runInvocation,
  output: () => visualOutput,
  progress: () => ({ status: state.status, error: state.error }),
  evidence: () => JSON.parse(JSON.stringify(state)),
  diagnostic,
  async close() {
    closed = true;
    try { await invocationPromise; } catch { /* Preserve failure in evidence. */ }
    await servingResources.close();
  },
};
}
