...
import {
  SAM_PROMPT_FPN_ONLINE_ATTENTION_WGSL,
...
export async function runSam3PromptFpnPhaseProgramRoute(input = {}) {
  ...
  return withSamPhaseCleanup(runtime, async () => {
    ...
    await runtime.runStage('load-prompt-fpn-tensors', async stage => {
      ...
      tensors = {
        ...
        spatialLinearDims: stage.createUniformBuffer({
          label: 'sam3.prompt-fpn.spatial-linear-dims',
          schema: [{ name: 'input_channels', type: 'u32' }, { name: 'output_channels', type: 'u32' }, { name: 'total_output', type: 'u32' }],
          values: { input_channels: shape.channels, output_channels: shape.channels, total_output: shape.batch * shape.spatialTokens * shape.channels },
        }),
        promptLinearDims: stage.createUniformBuffer({
          label: 'sam3.prompt-fpn.prompt-linear-dims',
          schema: [{ name: 'input_channels', type: 'u32' }, { name: 'output_channels', type: 'u32' }, { name: 'total_output', type: 'u32' }],
          values: { input_channels: shape.channels, output_channels: shape.channels, total_output: shape.batch * shape.promptTokens * shape.channels },
        }),
        weights: {
    ...
    const program = runtime.defineProgram({
      ...
      uniforms: { dims: tensors.dims, spatialLinearDims: tensors.spatialLinearDims, promptLinearDims: tensors.promptLinearDims },
      kernels: {
      ...
      phases: [
        ...
        { name: 'prompt-qkv-q', kernel: 'qLinear', dispatch: linearWorkgroups(shape.batch * shape.spatialTokens, shape.channels, shape.channels, input.device), yieldAfter: true },
        { name: 'prompt-qkv-k', kernel: 'kLinear', dispatch: linearWorkgroups(shape.batch * shape.promptTokens, shape.channels, shape.channels, input.device), yieldAfter: true },
        { name: 'prompt-qkv-v', kernel: 'vLinear', dispatch: linearWorkgroups(shape.batch * shape.promptTokens, shape.channels, shape.channels, input.device), yieldAfter: true },
        ...
        { name: 'prompt-output-linear', kernel: 'outputLinear', dispatch: linearWorkgroups(shape.batch * shape.spatialTokens, shape.channels, shape.channels, input.device), yieldAfter: true },
