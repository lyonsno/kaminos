...
async function interactivePointerInvocation({ inputs, expected, manifest, weights, adapter, device, adapterInfo, errors, commit, userAgent, invocationTag }) {
  ...
  const request = createRouteInvocationRequest(route, {
    ...
    inputs: {
      'source-frame': { artifactId: 'sam31-two-frame:0', sha256: sourceHash, shape: [1] },
      'sam31-binary-mask-inputs': { artifactId: 'sam31-frame-0-binary-mask-inputs', sha256: binaryHash, shape: [manifest.shape.batch, 1, manifest.shape.inputMaskHeight, manifest.shape.inputMaskWidth] },
...
export async function runSam31BrowserTrackerPackageInvocation({
  ...
  const memoryRequest = createRouteInvocationRequest(memoryRoute, {
    ...
    inputs: {
      ...
      'sam31-multiplex-mask-logits': { artifactId: 'sam31-frame-0-memory-input-masks', sha256: memoryInputMaskHash, shape: [episode.shape.multiplexCount, 1, executionGeometry.maskHeight, executionGeometry.maskWidth] },
