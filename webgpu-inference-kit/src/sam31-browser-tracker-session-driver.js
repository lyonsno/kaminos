...
export function deriveSam31BrowserTrackerExecutionContracts({
  ...
  return Object.freeze({
    ...
    decoder: Object.freeze({
      ...
      maskHeight: geometry.decoderMaskHeight,
      maskWidth: geometry.decoderMaskWidth,
    }),
    memory: Object.freeze({
      ...memoryShape,
      featureHeight: geometry.queryHeight,
      featureWidth: geometry.queryWidth,
      maskHeight: geometry.memoryInputMaskHeight,
      maskWidth: geometry.memoryInputMaskWidth,
      resampledMaskHeight: geometry.sourceMaskHeight,
      resampledMaskWidth: geometry.sourceMaskWidth,
    }),
  });
}
...
async function decoderInvocation({ frame, inputs, expected, manifest, weights, weightsHash, adapter, device, adapterInfo, errors, commit, userAgent, invocationTag }) {
  ...
  const request = createRouteInvocationRequest(route, {
    ...
    outputs: {
      ...
      'sam31-multiplex-mask-logits': { artifactId: scopedOutputId(`sam31-two-frame-mask-logits:${frame}`), shape: [manifest.shape.multiplexCount, manifest.shape.maskOutputsPerObject, manifest.shape.maskHeight, manifest.shape.maskWidth] },
      'sam31-multiplex-selected-masks': { artifactId: scopedOutputId(`sam31-two-frame-selected-masks:${frame}`), shape: [manifest.shape.multiplexCount, 1, manifest.shape.maskHeight, manifest.shape.maskWidth] },
      'sam31-multiplex-object-scores': { artifactId: scopedOutputId(`sam31-two-frame-object-scores:${frame}`), shape: [manifest.shape.multiplexCount, 1] },
...
export async function runSam31BrowserTrackerPackageInvocation({
  ...
  const memoryRequest = createRouteInvocationRequest(memoryRoute, {
    ...
    inputs: {
      ...
      'sam31-propagation-feature-2': { artifactId: 'sam31-frame-0-propagation-feature', sha256: featureHash, shape: [episode.shape.batch, executionGeometry.queryHeight, executionGeometry.queryWidth, episode.shape.channels] },
      'sam31-multiplex-mask-logits': { artifactId: 'sam31-frame-0-memory-input-masks', sha256: memoryInputMaskHash, shape: [episode.shape.multiplexCount, 1, executionGeometry.memoryInputMaskHeight, executionGeometry.memoryInputMaskWidth] },
  ...
  const attentionShape = { batch: episode.shape.batch, queryHeight: executionGeometry.queryHeight, queryWidth: executionGeometry.queryWidth, queryTokens: executionGeometry.queryTokens, memorySpatialTokens: executionGeometry.memorySpatialTokens, numObjPtrTokens: executionGeometry.numObjPtrTokens, memoryTokens: executionGeometry.memoryTokens, channels: episode.shape.channels, heads: 8, headDim: 32, mlpHidden: 2048, layerCount: 4 };
  ...
  const persistentStatePassed = preparedTrackerState.stateVersion === 1
    ...
    && pointerDigestPassed;
  const stateTransitionPassed = (!verificationAttached || (episode.stateTransition.frame0AppearingObjectCount > 0 && episode.stateTransition.frame0AbsentObjectCount > 0)) && suppressionPassed && persistentStatePassed && plan.spatialFrames.length === 1 && plan.pointerFrames.length === 1 && bank.memory.length === executionGeometry.memoryTokens * episode.shape.channels;
