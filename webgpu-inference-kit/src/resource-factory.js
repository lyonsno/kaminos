...
export function createWebGpuResourceFactory(input = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      function cancel() {
        ...
        if (activeWaiterCount(flight) === 0 && flight.status === 'active') {
          ...
          if (!flight.creatorStarted && flight.cancellationMode === WEBGPU_RESOURCE_CANCELLATION_MODES.callerImmediate) {
  ...
  function startFlight(flight, request) {
    flight.task = Promise.resolve()
      .then(async () => {
        // Report-bearing creators must settle even when every caller canceled.
        const skipCancelledCreator = flight.cancellationMode === WEBGPU_RESOURCE_CANCELLATION_MODES.callerImmediate
          && (activeWaiterCount(flight) === 0 || flight.controller.signal.aborted);
        if (invalidation || skipCancelledCreator) {
