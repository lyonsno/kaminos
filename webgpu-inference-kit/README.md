...
`runProgram()` opens and closes one invocation automatically. Before every compute or staged-readback command is encoded, the runtime refreshes that invocation to the newest valid scheduler revision, resolves any matching `commandDuty.chunkControl`, and later stores the exact boundary receipt in the submitted duty descriptor. The uncapped application snapshot proves boundary uptake but does not claim submission; the command-duty report separately proves which prepared duties reached `queue.submit()`. Together they name requested and effective revision, yield delay, phase controls, route, invocation, phase, and duty without relabeling old work. Custom adapter loops call `invocation.refreshAtBoundary({ boundaryId, dutyId, phase, position: "before-encode" })`, consume `invocation.getControl(controlId)` to choose the next bounded duty, encode it, and submit it. Nothing here pretends an already submitted command buffer can be split or preempted.
