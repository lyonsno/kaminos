# Live Combustion

These compositions and studies were captured directly from the live browser
runtime while one stateful WebGPU combustion material was being authored. The
material keeps its momentum and history as source geometry, energy, structure,
and color change.

![A live orange flame with a luminous crown and rooted filament structure](assets/conventional-fire-hero.png)

The conventional-fire still establishes the material at a glance. The films
below show the harder part: its structure and momentum remain alive while the
operator drives it through different bodies, colors, source geometries, and
extinction states.

## Complete Composition

[![A low warm burner with a structured crown](assets/composition-ooh-baby.png)](assets/composition-ooh-baby.mp4)

**Ignition, compression, rebirth.** A violet-rooted ignition grows into a warm
column, develops width and lean, constricts toward a needle, then reblooms as a
low burner before withdrawing into darkness. [Watch the complete
composition.](assets/composition-ooh-baby.mp4)

## Authored Transitions

| Phase transition | Warm morphology | Ignition sting |
| --- | --- | --- |
| [![A violet and blue combustion phase transition](assets/composition-phase-transition.png)](assets/composition-phase-transition.mp4) | [![A tall amber combustion body](assets/composition-warm-morphology.png)](assets/composition-warm-morphology.mp4) | [![An orange flame rooted in a blue base](assets/composition-ignition-sting.png)](assets/composition-ignition-sting.mp4) |
| Cool expansion and contraction with a continuous violet interface. [Motion](assets/composition-phase-transition.mp4) | A broad burner collapses toward a jet and rebuilds without resetting. [Motion](assets/composition-warm-morphology.mp4) | A blue base catches, rises, constricts, and leaves the frame. [Motion](assets/composition-ignition-sting.mp4) |

[![A low blue burner with a violet crown](assets/blue-wide-ignition-extinction.png)](assets/blue-wide-ignition-extinction.mp4)

**Blue wide ignition and extinction.** A compact blue body accelerates
laterally into a low burner, carries a violet crown across the growing
footprint, then withdraws through the luminous front and leaves its smoke in
flight. [Watch the complete movement.](assets/blue-wide-ignition-extinction.mp4)

## Material Studies

| Chromatic column | Warm width cycle | Blue-violet width cycle |
| --- | --- | --- |
| [![Chromatic combustion column](assets/chromatic-combustion-column.png)](assets/chromatic-combustion-column.mp4) | [![Warm width cycle](assets/warm-width-cycle.png)](assets/warm-width-cycle.mp4) | [![Blue-violet width cycle](assets/blue-violet-width-cycle.png)](assets/blue-violet-width-cycle.mp4) |
| Orange crown, luminous interface, cool rooted body. [Motion](assets/chromatic-combustion-column.mp4) | A live contraction and expansion through a warm basin. [Motion](assets/warm-width-cycle.mp4) | Connected cool-spectrum control with a changing footprint. [Motion](assets/blue-violet-width-cycle.mp4) |

[![A blue-violet flame moving from a compact body into a tall column](assets/blue-vertical-morphology.png)](assets/blue-vertical-morphology.mp4)

**Blue vertical morphology.** A compact body rises, narrows, and rebuilds into
a taller violet-edged column before withdrawing. [Motion](assets/blue-vertical-morphology.mp4)

| Violet filament drive | Live contraction | Cool oscillation |
| --- | --- | --- |
| [![Violet filament drive](assets/violet-filament-drive.png)](assets/violet-filament-drive.mp4) | [![Live contraction](assets/live-contraction.png)](assets/live-contraction.mp4) | [![Cool oscillation](assets/cool-oscillation.png)](assets/cool-oscillation.mp4) |
| Thin lateral sheets and filament-rich breakup. [Motion](assets/violet-filament-drive.mp4) | A state-history contraction from the original basin tour. [Motion](assets/live-contraction.mp4) | An alternate blue-violet treatment with broad oscillation. [Motion](assets/cool-oscillation.mp4) |

![An earlier live chromatic combustion column moving through orange, violet, and blue](assets/live-webgpu-combustion.gif)

The browser-native screening room lives in [`index.html`](index.html). Serve
the repository with `python3 serve.py 8095`, then open
`http://127.0.0.1:8095/docs/flame-atlas/`.

## Evidence Boundary

The films preserve source timing without optical flow, frame synthesis,
retiming, interpolation, or cadence repair. The public presentation makes no
simulator frame-rate, universal device-performance, or final quality-tier
claim. Exact derivative identities live in
[`capture-manifest.json`](capture-manifest.json); source identities and
inspection receipts remain in the campaign accountability surface.
