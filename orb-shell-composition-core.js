...
function macroContactEnvelopeRadius(assemblage, t) {
  ...
  const width = (body.widthProfile?.mid || 0.16)
    ...
    * curvatureWidthCapScaleAt(promoted, t);
...
function macroPromotedBodyEdgeSamples(assemblage, targetEdge, rowCount = 72) {
  ...
  const samples = centerline.map((center, row) => {
    ...
    const sharedSeam = sharedSocketSeamEffectAt(assemblage, t);
    const roleEffect = lowerSocketFamilyRoleEffectAt(assemblage, t);
    const curvatureCapScale = curvatureWidthCapScaleAt(promoted, t);
    const widthScale = interlock.widthScale * lowerSocket.widthScale * sharedSeam.widthScale * roleEffect.widthScale * curvatureCapScale;
...
function curvatureWidthCapScaleAt(promoted, t) {
  const law = promoted?.curvatureWidthCapLaw;
  if (!law?.capApplied) return 1;
  const terminalRelief = 0.28 + 0.72 * Math.pow(Math.sin(Math.PI * clamp(t, 0, 1)), 0.38);
  return 1 - (1 - law.widthScale) * terminalRelief;
}
