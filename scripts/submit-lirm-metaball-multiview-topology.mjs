...
const prior = existsSync(jobIndexPath)
  ...
  : { schema: 'kaminos.lirm-metaball-multiview-topology-greenroom-jobs.v0', jobs: [] };
...
for (const condition of manifest.conditions) {
  ...
  if (condition.references.length !== 3) {
    throw new Error(`three-reference topology contract violated for ${condition.id}`);
  }
  for (const seed of manifest.fixedGenerator.seeds) {
    ...
    try {
      ...
      writeFileSync(failurePath, `${JSON.stringify({
        schema: 'kaminos.lirm-metaball-multiview-topology-submission-failure.v0',
        status: 'failed',
