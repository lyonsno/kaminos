...
async function cdpFetch(port, path, options) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') throw new Error(`CDP ${path} timed out after ${cdpTimeoutMs}ms`);
    throw error;
  } finally {
    clearTimeout(timer);
  }
...
async function waitForCdp(port) {
  ...
  throw new Error(`Chrome DevTools endpoint did not open on ${port}`);
}

let cdpTimeoutMs = 120000;
...
async function waitForPage(port) {
  for (let attempt = 0; attempt < 120; attempt += 1) {
    const pages = await cdpFetch(port, '/json/list');
...
function launchChrome({ chrome, port, userDataDir, windowSize, url }) {
  return spawn(chrome, [
...
const chunkBytes = Math.max(4, Math.floor(Number(args.get('--chunk-bytes') || (4 * 1024 * 1024)) / 4) * 4);
const port = Number(args.get('--debug-port') || 9617);
...
cdpTimeoutMs = Math.max(5000, Math.floor(Number(args.get('--cdp-timeout-ms') || cdpTimeoutMs)));
...
try {
  ...
  chromeProcess = launchChrome({ chrome, port, userDataDir, windowSize, url });
  ...
  report.failurePhase = report.frames.length ? 'sequence-capture' : 'route-capture';
  ...
  if (chromeProcess && !keepBrowserOpen) chromeProcess.kill('SIGTERM');
}
