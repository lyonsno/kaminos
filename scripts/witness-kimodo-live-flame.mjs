...
await persist();
let browser, page, interval, watchdogInterval, lease;
const abort=createWitnessAbort(),wait=operation=>abort.wait(operation);
...
try{
  ...
  interval=setInterval(()=>{try{execFileSync(greenroom,['lease','renew',leaseId,'--ttl-seconds','900'],{encoding:'utf8'});}catch(error){const failure=new Error(`lease renewal failed: ${error.message}`);report.errors.push(failure.message);abort.abort(failure);}},60000);
  ...
  watchdogInterval=setInterval(()=>{
    ...
    if(expired){
      ...
      abort.abort(new Error(`Witness ${expired.kind} during ${report.phase}`));return;
    }
    if(!page||sampling)return;sampling=true;
    void page.evaluate(()=>{const s=window.__kimodoLiveFlame;return s?{status:s.status,loadProgress:s.loadProgress,telemetry:s.telemetry,lastError:s.lastError}:null;})
      .then(s=>{if(abort.reason)return;report.lastTrustworthy=s;if(report.phase!=='motion-export')watchdog.observe(report.phase,[s?.status,s?.loadProgress?.loaded,s?.telemetry?.currentStage,s?.telemetry?.progress?.step,s?.telemetry?.scheduler?.observedForegroundBoundaryCount],Date.now());})
      .catch(()=>{}).finally(()=>sampling=false);
  },1000);
  page=await wait(browser.newPage());await wait(page.setViewport({width:1440,height:1000,deviceScaleFactor:1}));
  await wait(page.setCacheEnabled(false));
  const responses=[];
  ...
  report.phase='page-load';await wait(page.goto(url,{waitUntil:'domcontentloaded',timeout:60000}));
  await wait(page.waitForFunction(()=>window.__kimodoLiveFlame?.samples.some(s=>s.active&&s.frameCount>5),{timeout:120000}));
  report.phase='weights';await persist();await wait(page.click('#kimodo-load'));
  await wait(page.waitForFunction(()=>['loaded','failed'].includes(window.__kimodoLiveFlame?.status),{timeout:0}));
  if(await wait(page.evaluate(()=>window.__kimodoLiveFlame.status))!=='loaded')throw new Error(await wait(page.evaluate(()=>JSON.stringify(window.__kimodoLiveFlame.lastError))));
  // Fail substituted routes before spending an inference run.
  await wait(Promise.all(responses));
  const loaded=await wait(page.evaluate(()=>window.__kimodoLiveFlame));
  ...
  const loadedPreset=await wait(page.evaluate(()=>window.__kaminosVolumeSettingsPresetReceipt));
  ...
  await wait(new Promise(r=>setTimeout(r,5000)));
  ...
  try{
    await wait(page.waitForFunction(()=>window.__kimodoLiveFlame?.telemetry?.progress?.step>=5 || window.__kimodoLiveFlame?.runs[0]?.status!=='running',{timeout:120000}));
    if(await wait(page.evaluate(()=>window.__kimodoLiveFlame.runs[0].status==='running')))await wait(page.screenshot({path:path.join(output,'during.png')}));
  }catch(error){if(abort.reason)throw abort.reason;report.captureError=error.message;}
  ...
  report.presetReceipt=await wait(page.evaluate(()=>window.__kaminosVolumeSettingsPresetReceipt));
  ...
  await wait(page.screenshot({path:path.join(output,'complete.png')}));
  ...
  const cdp=await wait(browser.target().createCDPSession());await wait(cdp.send('Browser.setDownloadBehavior',{behavior:'allow',downloadPath:path.resolve(downloadDir),eventsEnabled:true}));
  let download;
  ...
  completed.catch(()=>{}); // CDP may fail before click resolves
  if(await wait(page.$eval('#kimodo-motion-download',el=>el.disabled)))throw new Error('No motion export available');
  await wait(page.click('#kimodo-motion-download'));await wait(completed);
  ...
  // Primary failure survives even if all subsequent CDP operations wedge.
  await persist();
  if(page&&!abort.reason){try{
    const evidence=await boundedCleanup(()=>page.evaluate(()=>window.__kimodoLiveFlame??null),2000,'failure diagnostics');
    if(evidence)report.evidence=evidence;
    await boundedCleanup(()=>page.screenshot({path:path.join(output,'failure.png')}),2000,'failure screenshot');
  }catch(error){report.diagnosticError=error.message;}}
}finally{
  clearInterval(interval);
  clearInterval(watchdogInterval);
  if(page&&report.status==='failed')try{await boundedCleanup(()=>page.evaluate(()=>document.getElementById('kimodo-cancel')?.click()),2000,'cancel');}catch(error){report.cancelError=error.message;}
  if(browser)try{await boundedCleanup(()=>browser.close(),5000,'browser.close');}catch(error){
    report.closeError=error.message;report.status='failed';
    // ChildProcess handle belongs to this launch. Never search/kill other Chrome instances.
    try{const child=browser.process();if(child&&child.exitCode===null&&child.signalCode===null){report.browserFallback={pid:child.pid,signal:'SIGKILL',accepted:child.kill('SIGKILL')};}}
    catch(error){report.browserFallbackError=error.message;}
    try{browser.disconnect();}catch(error){report.disconnectError=error.message;}
  }
