...
const DEFAULT_ROUTE = 'http://127.0.0.1:18223/artifacts/pyro-gaussian-footprint-kneecapper-0716/core-skirt-footprint-oracle-state120-r1/index.html';
...
const browserStateExpression = `(() => {
  ...
  return {
    evidenceState: status?.dataset.state || null,
    evidenceText: status?.textContent || null,
    failure: document.getElementById('failure')?.textContent || null,
...
async function waitForVerifiedPage(session) {
  while (true) {
    ...
    if (state.evidenceState === 'failed') throw new Error(state.failure || 'page evidence validation failed');
    if (state.evidenceState === 'verified' && state.imagesComplete) return state;
    await delay(100);
  }
}
...
async function run() {
  ...
  const profile = await mkdtemp(join(tmpdir(), 'kaminos-core-skirt-page-'));
  let failurePhase = 'browser-launch';
  let chrome = null;
  let session = null;
  const report = {
    schema: 'kaminos.volume.core-skirt-footprint-page-witness.v0',
    status: 'running',
    failurePhase,
  ...
  try {
    ...
    await evaluate(session, `(() => {
      ...
      set('camera', '18', 'input');
      set('leftTreatment', 'm075-global', 'change');
      set('rightTreatment', 'm075-ridge100', 'change');
      set('wipe', '55', 'input');
      document.querySelector('#viewMode [data-mode="residual"]').click();
    })()`);
    const desktopState = await waitForVerifiedPage(session);
    const selectedState = await evaluate(session, `({
      camera: document.getElementById('camera').value,
      left: document.getElementById('leftTreatment').value,
      right: document.getElementById('rightTreatment').value,
      wipe: document.getElementById('wipe').value,
      mode: document.querySelector('#viewMode [aria-pressed="true"]').dataset.mode,
    ...
    if (selectedState.camera !== '18' || selectedState.left !== 'm075-global' || selectedState.right !== 'm075-ridge100' || selectedState.mode !== 'residual') {
