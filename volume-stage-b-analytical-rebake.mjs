...
export async function rebakeAnalyticalStageB({ state, controls = {}, width = 320, height = 320 } = {}) {
  ...
  const depositionIdentity = await hashBytes(new Uint8Array(layers.buffer, layers.byteOffset, layers.byteLength));
  const pixels = resolveOpticalLayers(layers, width, height);
  const sourceStateIdentity = await hashJson({
  ...
  const candidateIdentity = await hashJson({ candidateCount, candidateWeightSum, weightedCandidateSum });
  const coefficientIdentity = await hashJson({ candidateCount, coefficientSum, weightedCoefficientSum });
  const covarianceIdentity = await hashJson({
    candidateCount,
