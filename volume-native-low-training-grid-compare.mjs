...
function htmlPage(report) {
  ...
  :root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#09090b;color:#f4f4f5;font:14px system-ui,sans-serif}header{position:sticky;top:0;z-index:2;padding:12px 18px;background:#18181b;border-bottom:1px solid #3f3f46}h1{margin:0 0 4px;font-size:18px;letter-spacing:0}p{margin:0;color:#a1a1aa}.strip{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1px;background:#27272a}figure{min-width:0;margin:0;background:#000}figcaption{height:58px;padding:10px 14px;background:#18181b;display:flex;flex-direction:column;border-bottom:1px solid #3f3f46}figcaption strong{font-size:16px}figcaption span{color:#a1a1aa;margin-top:3px}img{display:block;width:100%;height:auto;image-rendering:auto}a{display:block}.note{padding:12px 18px;color:#a1a1aa;border-top:1px solid #3f3f46}@media(max-width:800px){.strip{grid-template-columns:1fr}}</style><header><h1>Native 96: control vs training-grid transfer</h1><p>Same native state, splat-only, no runtime truth. Click any panel for the original PNG.</p></header><main class="strip">${panels}</main><div class="note">The 128-trained baseline is cross-grid zero-shot. The 96-trained candidate is matched to the deployment grid but was still trained from a filtered high-history pair, not native-phase truth.</div>`;
}
