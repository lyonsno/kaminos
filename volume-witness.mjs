...
async function main() {
  ...
  try {
    ...
    if (expectsVolumeBrickWallSceneContext) {
      ...
      assert.equal(volumeSceneContext?.receiverLight?.fallbackFloorRole, 'guard-not-cadence-author', 'brick-wall screen-space receiver light must demote the visible fallback floor to a guard, not a cadence author');
      assert.equal(volumeSceneContext?.receiverLight?.couplingModel, 'rendered-fire-energy-primary-fallback-floor-guard-v0', 'brick-wall screen-space receiver light must expose rendered-fire-first coupling');
      ...
      assert.ok((volumeSceneContext?.receiverLight?.fallbackGuardEnergy ?? 1) <= Math.max(0.25, (volumeSceneContext?.receiverLight?.fallbackEnergyFloor ?? 0) * 0.22), 'brick-wall screen-space receiver fallback guard is too large to be only a floor');
