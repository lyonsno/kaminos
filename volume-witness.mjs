...
async function main() {
  ...
  try {
    ...
    if (controlledStepSequenceRequested) {
      ...
      for (let controlledFrameIndex = 0; controlledFrameIndex < controlledStepFrames; controlledFrameIndex += 1) {
        const controlledEval = await wsRequest(ws, 'Runtime.evaluate', {
          expression: `window.__kaminosVolumePrototype.controlledStepFrame(${JSON.stringify({
            frameCount: controlledStepFrames,
            controlledStepFrameIndex: controlledFrameIndex,
        ...
        const frameSlug = `frame-${String(frameIndex + 1).padStart(3, '0')}`;
        ...
        for (let index = 0; index < scaleSet.samples.length; index += 1) {
          ...
          const previewPath = resolve(frameDir, `${slug}.preview.png`);
      ...
      const firstStepCapture = firstFrame.controlledStepCapture || {};
      controlledStepSequenceReport = {
        ...
        startFrameCount: firstStepCapture.beforeFrameCount ?? null,
        startSimStepCount: firstStepCapture.beforeSimStepCount ?? null,
