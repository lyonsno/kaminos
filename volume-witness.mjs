...
async function main() {
  ...
  try {
    ...
    if (expectsVolumeBrickWallSceneContext) {
      ...
      assert.equal(volumeSceneContext?.receiverLight?.energyAuthority, 'rendered-volume-canvas-fire-energy-v21-visible-wide-gather-local-probe-no-readback-v0', 'brick-wall screen-space receiver light must disclose rendered-volume texture fire-energy authority');
      assert.equal(volumeSceneContext?.receiverLight?.fallbackEnergyRole, 'debug-metadata-only-not-radiance', 'brick-wall screen-space receiver light must expose the uploaded envelope as debug metadata only');
      assert.equal(volumeSceneContext?.receiverLight?.couplingModel, 'rendered-fire-energy-only-no-envelope-fallback-v0', 'brick-wall screen-space receiver light must expose rendered-fire-only no-fallback coupling');
      assert.equal(volumeSceneContext?.receiverLight?.renderedFireEnergy?.authority, 'gpu-tsl-volume-texture-node-not-cpu-readable-v0', 'brick-wall screen-space receiver light must expose rendered-fire energy as a GPU-only node');
      assert.equal(volumeSceneContext?.receiverLight?.renderedFireEnergy?.primaryCadenceDriver, true, 'brick-wall screen-space receiver light must make rendered fire energy the primary cadence driver');
      assert.equal(volumeSceneContext?.receiverLight?.receiverLightEnergy?.authority, 'gpu-tsl-receiver-energy-node-not-cpu-readable-v0', 'brick-wall screen-space receiver light must expose receiver-light energy as a GPU-only node');
      assert.equal('fallbackGuardEnergy' in (volumeSceneContext?.receiverLight || {}), false, 'brick-wall screen-space receiver light must not expose fallback radiance');
      ...
      assert.ok((volumeSceneContext?.receiverLight?.volumeEnergy?.sampleCount ?? 0) >= 5, 'brick-wall volume texture fire-energy sampling did not expose enough fixed/local samples for the fire body');
    ...
    const hasTransportedFireEvidence =
      ...
      acceptsRawCarrierPyroPaint;
    if (!expectsCanonicalPlumeProof && !expectsFuelStarvedTallPlume && !expectsNoFireVolumeEvidence && !hasTransportedFireEvidence) {
