...
function fieldBoundaryForAssertion(assertion = {}) {
  if (assertion.field === 'phaseChunkSize.spnPatch' || assertion.field === 'spnPatchChunkSize') return 'spn-patch-chunk';
  if (assertion.field === 'phaseChunkSize.vitBlock' || assertion.field === 'vitBlockChunkSize') return 'vit-block-chunk';
  return null;
}

function fieldKeyForAssertion(assertion = {}) {
  if (assertion.field === 'phaseChunkSize.spnPatch' || assertion.field === 'spnPatchChunkSize') return 'spnPatch';
  if (assertion.field === 'phaseChunkSize.vitBlock' || assertion.field === 'vitBlockChunkSize') return 'vitBlock';
  return null;
}
...
function assertionHasMatchingEvent(assertion = {}, events = []) {
  ...
  return events.some(event => event?.boundary === expectedBoundary || event?.phase === expectedBoundary);
}
...
export function createSchedulerVerificationReceipt(input = {}) {
  ...
  const requestedBoundaryKeys = requestedPhaseFields(requestedScheduler).map(({ key }) => key);
  const missingRequestedBoundaryAssertions = requestedBoundaryKeys
    .filter(key => !verifiedAssertions.some(assertion => assertionVerifiesRequestedKey(assertion, key, events)));
