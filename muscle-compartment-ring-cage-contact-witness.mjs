...
<body>
  ...
  <script type="module">
    ...
    function addCage(groups,cage,index){const mesh=cageMesh(cage,colors[index]);groups.surface.add(mesh);groups.wireframe.add(new THREE.LineSegments(new THREE.EdgesGeometry(mesh.geometry,18),new THREE.LineBasicMaterial({color:colors[index],transparent:true,opacity:.62,depthTest:false,depthWrite:false})));groups.surface.add(line(cage.axisNodeIndices.map(nodeIndex=>cage.positions[nodeIndex]),0xffffff,.7));for(const nodeIndex of cage.fixedNodeIndices){const handle=new THREE.Mesh(new THREE.SphereGeometry(diagnosticScale*.72,12,8),new THREE.MeshStandardMaterial({color:0xf5f1e8,roughness:.35}));handle.position.set(...cage.positions[nodeIndex]);groups.surface.add(handle);}}
    ...
    for(const [armIndex,arm] of payload.arms.entries()){
      ...
      for(const phase of ['initial','packed']){
        ...
        for(const [rowIndex,row] of rows.entries()){if(!row.witness?.leftPoint||!row.witness?.rightPoint)continue;const color=row.kind==='skeletal'?0x48c7ff:0xff8a3d;groups.contacts.add(beam([row.witness.leftPoint,row.witness.rightPoint],color,diagnosticScale*(rowIndex===0?1.2:.65)));}
        scene.add(groups.surface,groups.wireframe,groups.contacts);
      }
      for(const [cageIndex,initialCage] of arm.initialCages.entries())for(const nodeIndex of initialCage.axisNodeIndices){const initial=initialCage.positions[nodeIndex],packed=arm.packedCages[cageIndex].positions[nodeIndex];if(new THREE.Vector3(...initial).distanceTo(new THREE.Vector3(...packed))>1e-6)displacements[arm.semanticId].add(beam([initial,packed],colors[cageIndex],diagnosticScale*.35));}
    }
