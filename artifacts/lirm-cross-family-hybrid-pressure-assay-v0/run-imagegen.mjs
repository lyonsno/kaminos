...
import { buildCrossFamilyHybridImagegenPlan } from './imagegen-contract.mjs';
...
const runtimeRoot = resolve(
  process.env.KAMINOS_LIRM_CROSS_FAMILY_IMAGEGEN_RUNTIME_ROOT
    ?? '/Users/noahlyons/.local/state/gpu-greenroom/outputs/kaminos-lirm-cross-family-hybrid-20260719',
);
...
const runnerFailurePath = join(artifactRoot, 'imagegen-runner-failure.json');
...
async function submitPlan() {
  ...
  if (plan.schema !== 'kaminos.lirm-cross-family-hybrid-imagegen-pressure-plan.v0') {
    throw new Error('unsupported cross-family plan schema');
  }
  const submissions = [];
  for (const cell of plan.cells) {
    ...
    await atomicWriteJson(submissionPath, {
      schema: 'kaminos.lirm-cross-family-hybrid-imagegen-submission.v0',
...
async function collectPlan() {
  ...
  for (const cell of plan.cells) {
    ...
    const status = runGreenroom(['status', cell.jobId]);
    ...
    try {
      ...
      if (durableOutput.sha256 !== completion.output.sha256
          || durableOutput.byteSize !== completion.output.bytes) {
        throw new Error(`durable output copy mismatch: ${cell.cellId}`);
      }
      accepted.push({ ...completion, candidateId: cell.candidateId, stance: cell.stance, durableOutput });
    } catch (error) {
      failed.push({
        cellId: cell.cellId,
        jobId: cell.jobId,
...
async function main() {
  ...
  if (command === 'submit') return submitPlan();
  if (command === 'collect') return collectPlan();
...
try {
  ...
  if (process.argv[2] === 'collect' && result.status !== 'complete-uninspected') process.exitCode = 2;
} catch (error) {
  await atomicWriteJson(runnerFailurePath, {
    schema: 'kaminos.lirm-cross-family-hybrid-imagegen-runner-failure.v0',
    status: 'failed',
    failurePhase: `imagegen-${process.argv[2] ?? 'plan'}`,
    errorMessage: error.message,
    lastTrustworthyEvidence: 'hybrid control receipt and any incrementally written plan or submission report',
  });
  throw error;
}
