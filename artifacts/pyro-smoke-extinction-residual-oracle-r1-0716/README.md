...
- **Exact pre-absorption physical body target:** dense R160 step-45 ray integration of `0.74 smokeDensity + 0.42 microdetail + 0.34 interfaceShred + 0.12 detail`. The source manifest does not preserve production `absorptionGain`, so this witness does not claim fully parameterized production-compositor extinction.
...
Raw authority lives in [`source-step45/oracle-report.json`](source-step45/oracle-report.json), the dense teacher reports under `witness/teachers/`, and each role/camera `render-witness-report.json`. The next admissible experiment keeps the carrier equation and frozen control fixed while adding full residual-weighted covariance, ridge-conditioned split/local sheets, and connected or overlapping hierarchy across rigid block boundaries.
