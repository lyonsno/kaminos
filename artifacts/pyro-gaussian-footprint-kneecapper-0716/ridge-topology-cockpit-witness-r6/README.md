# Ridge Topology Cockpit Witness R6
...
R6 witness repair: Every captured mode now includes a same-state renderer RGBA readback after the render-idle barrier. `report.json` records stage bounds, dimensions, stage-only SHA-256, nonblank pixels, color variation, composition, image authority, and before/after simulation step. Baseline versus target and baseline versus cooled distinctness are asserted from stage RGBA, not whole-page UI screenshots.

Route:

- Repo/worktree: `/private/tmp/kaminos-pyro-gaussian-footprint-kneecapper-0716`
- Branch/source head: `cc/pyro-gaussian-footprint-kneecapper-0716` from `213fb246`
- Greenroom job: `54a05ce97a6a`, type `kaminos_ridge_topology_cockpit_witness`
...
- Effective command, route identity, page-image hashes, and stage-only evidence: `report.json`
