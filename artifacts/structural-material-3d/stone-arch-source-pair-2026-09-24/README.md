...
The paired generation is not a controlled mechanical ablation: the edited source also drifted in fine texture and TRELLIS reconstructed each image independently. A useful next experiment is to derive one explicit volume/structural proxy from each mesh, apply the same support and load, and test whether the notch moves stress concentration and fracture onset for geometric reasons. Do not infer a fracture path from the images alone.
