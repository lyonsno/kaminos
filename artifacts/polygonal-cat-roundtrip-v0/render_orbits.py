#!/usr/bin/env python3
"""Render both admitted casts through the same six-view orbit."""
...
def main() -> int:
    ...
    for route_id, row in ledger["routes"].items():
        ...
        if digest(glb) != row["outputSha256"]:
            raise RuntimeError(f"admitted GLB drifted: {route_id}")
        orbit = ROOT / "reconstructions" / route_id / "orbit"
        manifest = orbit.parent / "orbit-manifest.json"
        command = [
            str(BLENDER), "--background", "--python", str(RENDERER), "--",
            "--glb", str(glb), "--expected-sha256", row["outputSha256"],
            "--out-dir", str(orbit), "--manifest", str(manifest),
            "--failure", str(orbit.parent / "orbit-failure.json"),
