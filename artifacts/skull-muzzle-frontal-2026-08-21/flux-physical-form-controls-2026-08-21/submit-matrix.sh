...
for seed in 81432 81433 81434 81435; do
  if (( seed <= 81433 )); then
    wave=wave-1
  else
    wave=wave-2
...
jq -s \
  ...
  '{
    schema: "kaminos.handy_candyman.flux_physical_form_controls.start_receipt.v1",
...
[[ $(jq -r '.job_count' "$receipt_tmp") == 64 ]] || {
  printf 'Expected 64 jobs, found %s\n' "$(jq -r '.job_count' "$receipt_tmp")" >&2
  exit 1
}
[[ $(jq -r '[.jobs[].job_id] | unique | length' "$receipt_tmp") == 64 ]] || {
  printf 'Job IDs are not unique\n' >&2
  exit 1
}

mv "$receipt_tmp" "$atlas_dir/start-receipt.json"
printf 'Recorded 64 unique jobs in %s\n' "$atlas_dir/start-receipt.json"
