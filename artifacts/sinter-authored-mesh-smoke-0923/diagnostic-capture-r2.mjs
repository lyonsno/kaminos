...
const outDir = resolve(args.get('--out-dir') || `${repoRoot}/artifacts/sinter-authored-mesh-smoke-0923/diagnostic-r2`);
  ...
  try {
    ...
    for (const mode of modes) {
      ...
      const state = await waitFor(async () => {
        const current = await cdpValue(ws, 'window.__kaminosVolumePrototype?.debugState?.()');
        const currentAssembly = current?.gpuStructuralCombustionAssembly;
        return currentAssembly?.dispatchCount >= 120
