#!/usr/bin/env python3
"""Collect canonical Greenroom evidence once every FLUX cell is terminal."""
...
LEDGER = ROOT / "flux-ledger.json"
FAILURE_REPORT = ROOT / "flux-failure-report.json"
...
def main() -> int:
    ...
    if set(submissions.get("cells", {})) != expected_ids:
        raise RuntimeError("submission ledger does not cover the frozen 18-cell campaign")
    ...
    if failures:
        write_failure_report(
            FAILURE_REPORT,
            schema="kaminos.latest-envelope-basin-reconstruction.flux-failure.v0",
            phase="greenroom-terminal-status",
            statuses=statuses,
            failures=failures,
        )
        print(f"FLUX campaign has {len(failures)} failed terminal cell(s); wrote {FAILURE_REPORT}", file=sys.stderr)
        return 1

    ledger = {
        "schema": "kaminos.latest-envelope-basin-reconstruction.flux-ledger.v0",
    ...
    for cell in campaign["cells"]:
        ...
        output = ROOT / "flux" / cell_id / "output.png"
        ...
        try:
            ...
            continue
        receipt_dir = ROOT / "receipts" / cell_id
    ...
    if output_failures:
        ...
        print(f"FLUX evidence validation failed for {len(output_failures)} cell(s); wrote {FAILURE_REPORT}", file=sys.stderr)
        return 1
    write_json(LEDGER, ledger)
    FAILURE_REPORT.unlink(missing_ok=True)
    print(f"collected {len(ledger['cells'])} complete FLUX cells into {LEDGER}")
    return 0
