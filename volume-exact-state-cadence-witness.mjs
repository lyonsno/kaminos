...
try {
  ...
  const initialState = await waitForActiveCadence(effectivePageUrl);
  ...
  for (let index = rows.length; index < sampleCount; index += 1) {
    const state = await debugState();
      validateEffectiveState(state, effectivePageUrl, { requireLearnedPopulation: false });
  ...
  const finalState = await waitForActiveCadence(effectivePageUrl);
...
function boundarySplatSourceCandidateCount(state) {
  return Number(state?.boundarySplatSourceCandidateCount ?? state?.boundarySplatCandidateCount);
}

function boundarySplatSelectedCandidateCount(state) {
  return Number(state?.boundarySplatSelectedCandidateCount ?? state?.boundarySplatCandidateCount);
}

function boundarySplatLearnedPopulationReady(state) {
  return (
    Number(boundarySplatSourceCandidateCount(state)) > 0
    && Number(boundarySplatSelectedCandidateCount(state)) > 0
    && Number(state?.boundarySplatInstanceCount) > 0
  );
}

function validateEffectiveState(state, pageUrl, options = {}) {
  const expected = requestedConfigFromUrl(pageUrl);
  const cadence = state?.exactStateCadence;
  const requireLearnedPopulation = options.requireLearnedPopulation !== false;
  const sourceCandidateCount = Number(state?.boundarySplatSourceCandidateCount ?? state?.boundarySplatCandidateCount);
  const selectedCandidateCount = Number(state?.boundarySplatSelectedCandidateCount ?? state?.boundarySplatCandidateCount);
  const mismatches = [];
  ...
  if (expected.boundarySplatMode === 'learned') {
    ...
    if (requireLearnedPopulation && !(sourceCandidateCount > 0)) mismatches.push(['boundarySplatSourceCandidateCount', '>0', state?.boundarySplatSourceCandidateCount ?? state?.boundarySplatCandidateCount]);
    if (requireLearnedPopulation && !(selectedCandidateCount > 0)) mismatches.push(['boundarySplatSelectedCandidateCount', '>0', state?.boundarySplatSelectedCandidateCount ?? state?.boundarySplatCandidateCount]);
    if (requireLearnedPopulation && !(Number(state?.boundarySplatInstanceCount) > 0)) mismatches.push(['boundarySplatInstanceCount', '>0', state?.boundarySplatInstanceCount]);
  }
...
function compactState(state) {
  ...
  return {
    ...
    boundarySplatSourceCandidateCount: Number(boundarySplatSourceCandidateCount(state) || 0),
    boundarySplatSelectedCandidateCount: Number(boundarySplatSelectedCandidateCount(state) || 0),
...
async function captureCanvas(label) {
  const sample = await evaluate(
    "window.__kaminosVolumePrototype.sampleFrame({ advanceSim: false, includeRgba: true, boundarySplatComposition: 'raymarch-under-splats-v0' })",
    true,
  );
  ...
  const visiblePixelGate = {
    metricLitPixels: Number(metrics.litPixels || 0),
    sampleLitPixels: Number(sample?.litPixels || 0),
    sampleMeanLuma: Number(sample?.meanLuma || 0),
    pass: Number(metrics.litPixels || 0) > 200
      || (Number(sample?.litPixels || 0) > 40 && Number(sample?.meanLuma || 0) > 1),
  };
  ...
  if (!visiblePixelGate.pass) {
    throw new Error(`blank-or-partial-cadence-canvas:${JSON.stringify({ metrics, visiblePixelGate })}`);
  }
  return {
    bytes,
    receipt,
...
function validateReadbackSample(sample) {
  ...
  if (sample?.boundarySplatReadbackCompositionEffective !== 'full-raymarch-under-splats-diagnostic-v0') {
    mismatches.push(['boundarySplatReadbackCompositionEffective', 'full-raymarch-under-splats-diagnostic-v0', sample?.boundarySplatReadbackCompositionEffective]);
  }
  if (sample?.boundarySplatReadbackPassReceipt?.raymarchApplied !== true || sample?.boundarySplatReadbackPassReceipt?.splatApplied !== true) {
    mismatches.push(['boundarySplatReadbackPassReceipt', 'raymarchApplied=true,splatApplied=true', sample?.boundarySplatReadbackPassReceipt]);
  }
...
async function waitForPrototype() {
  ...
  for (let attempt = 0; attempt < 240; attempt += 1) {
    ...
    if (!activationAttempted && state && state.active !== true && !state.error) {
      activationAttempted = true;
      lastTrustworthyEvidence.activationAttempt = await activatePrototype();
      await delay(250);
      continue;
    }
    if (state?.error) {
...
async function activatePrototype() {
  return evaluate(`(async () => {
    const prototype = window.__kaminosVolumePrototype;
    if (!prototype?.setActive) return { ok: false, reason: 'volume-prototype-setActive-missing' };
    const before = prototype.debugState?.() || null;
    await prototype.setActive(true);
    const after = prototype.debugState?.() || null;
    return {
      ok: after?.active === true,
      authority: 'owned-witness-prototype-setActive-v0',
      before: {
        active: before?.active ?? null,
        backend: before?.backend || null,
        error: before?.error || null,
        exactStateCadenceRequested: before?.exactStateCadenceRequested ?? null,
        exactStateCadenceEffective: before?.exactStateCadenceEffective || null,
      },
      after: {
        active: after?.active ?? null,
        backend: after?.backend || null,
        error: after?.error || null,
        exactStateCadenceRequested: after?.exactStateCadenceRequested ?? null,
        exactStateCadenceEffective: after?.exactStateCadenceEffective || null,
      },
    };
  })()`, true, 30000, 'prototype-activation-timeout');
}
...
async function forcePresentationUnderflow(effectivePageUrl) {
  ...
  validateEffectiveState(pressure.state, effectivePageUrl, { requireLearnedPopulation: false });
...
async function waitForActiveCadence(effectivePageUrl) {
  const expected = requestedConfigFromUrl(effectivePageUrl);
  for (let attempt = 0; attempt < 240; attempt += 1) {
    const state = await debugState();
    if (
      ...
      && (expected.boundarySplatMode !== 'learned' || boundarySplatLearnedPopulationReady(state))
    ) return state;
