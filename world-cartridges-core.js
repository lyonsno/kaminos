...
function buildSmokeWorkbenchHelper(offer, {
  ...
  if (offer.routeKind === 'host-surface-smoke-offer-route' || offer.targetSurface === 'lerms-moving-timeline-host') {
    return {
      schema: WORLD_CARTRIDGE_SMOKE_WORKBENCH_HELPER_SCHEMA,
      routeKind: offer.routeKind || 'host-surface-smoke-offer-route',
      operatorRoute: offer.route,
      operatorSteps: [
        'open_operator_route',
      ...
      receiptSchema: offer.expectedReceipt || 'kaminos.host-surface.tools-report.v0',
      docs: [
...
export function createWorldCartridgeIndex(manifests = [], options = {}) {
  const rootDir = options.rootDir || null;
  const cartridges = manifests.map(manifest => {
    const manifestPath = options.manifestPath || null;
    const cartridgeRoot = rootDir && manifest?.id ? join(rootDir, manifest.id) : rootDir;
    const normalized = normalizeWorldCartridgeManifest(manifest, {
      manifestPath,
      rootDir: cartridgeRoot,
    });
    return cartridgeSummary(normalized, rootDir || cartridgeRoot || normalized.files.rootDir || null);
  });
