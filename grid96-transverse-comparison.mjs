...
const MATERIAL_SOCKET_IDENTITY = 'sha256:aa870aa6c4e4e7e83eeaff2f43384949fc9aa14880895fabb758a8f6f2d55f3f';
const MATERIAL_SOCKET_SHA256 = 'e563d05ef0bde2c58c185f27c0c47f32acec90a74ec89579a87e39cba835ad3c';
const MATERIAL_BASIS_SHA256 = '6b726c2f0c7efa223aab68eb7c76b42719595b4fc8943203d2671fa6652b5b13';
...
const MATERIAL_VALID_ROWS = 75289;
const MATERIAL_INVALID_ROWS = 294905;
...
const expectedArmCoordinates = new Map([
  ['n1-b0', { normal: 1, binormal: 0, role: 'structure-normal', validRows: VALID_ROWS, invalidRows: INVALID_ROWS, label: 'Structure normal 1.0 / binormal 0.0' }],
  ['n0-b1', { normal: 0, binormal: 1, role: 'structure-normal', validRows: VALID_ROWS, invalidRows: INVALID_ROWS, label: 'Structure normal 0.0 / binormal 1.0' }],
  ['n05-b05', { normal: 0.5, binormal: 0.5, role: 'structure-normal', validRows: VALID_ROWS, invalidRows: INVALID_ROWS, label: 'Structure normal 0.5 / binormal 0.5' }],
  ['n1-b1', { normal: 1, binormal: 1, role: 'structure-normal', validRows: VALID_ROWS, invalidRows: INVALID_ROWS, label: 'Structure normal 1.0 / binormal 1.0' }],
  ['n15-b15', { normal: 1.5, binormal: 1.5, role: 'structure-normal', validRows: VALID_ROWS, invalidRows: INVALID_ROWS, label: 'Structure normal 1.5 / binormal 1.5' }],
  ['material-n0-b1', { normal: 0, binormal: 1, role: 'material-nonridge', validRows: MATERIAL_VALID_ROWS, invalidRows: MATERIAL_INVALID_ROWS, label: 'Material Non-Ridge 0.0 / binormal 1.0' }],
]);
...
function validateReceipt(path, reportPath, row, transverse) {
  ...
  if (transverse) {
    ...
    const roleIndex = tokens.indexOf('--transverse-basis-role');
    if (coordinate.role === 'material-nonridge') {
      require(roleIndex >= 0 && tokens[roleIndex + 1] === coordinate.role, `${row.key} effective route omitted the explicit material role`);
    } else if (roleIndex >= 0) {
      require(tokens[roleIndex + 1] === coordinate.role, `${row.key} effective structure role drifted`);
    }
  } else {
...
function validateTransverse(path, row) {
  ...
  const material = coordinate.role === 'material-nonridge';
  require(socket?.identity === (material ? MATERIAL_SOCKET_IDENTITY : SOCKET_IDENTITY), `${row.key} transverse socket identity drifted`);
  require(socket?.sha256 === (material ? MATERIAL_SOCKET_SHA256 : SOCKET_SHA256), `${row.key} transverse socket hash drifted`);
  require(socket?.basisArtifact?.sha256 === (material ? MATERIAL_BASIS_SHA256 : BASIS_SHA256), `${row.key} transverse basis payload hash drifted`);
  require(socket?.rowCount === ROW_COUNT && socket?.basisValidRowCount === coordinate.validRows && socket?.basisInvalidRowCount === coordinate.invalidRows, `${row.key} transverse basis coverage drifted`);
  ...
  if (material) {
    require(socket?.basisRole === coordinate.role, `${row.key} effective material role drifted`);
    require(socket?.composedWithStructureBasis === false, `${row.key} material role was composed with structure geometry`);
  } else if (socket?.basisRole != null) {
    require(socket.basisRole === coordinate.role && socket.composedWithStructureBasis === false, `${row.key} effective structure role drifted or composed`);
  }
  for (const camera of report.metrics.cameras) {
    ...
    require(placement.basisValidRows === coordinate.validRows && placement.basisInvalidRows === coordinate.invalidRows, `${row.key} camera ${camera.cameraIndex} basis coverage drifted`);
    ...
    if (material) require(placement.basisRole === coordinate.role && placement.composedWithStructureBasis === false && placement.equalPerTreatedParentQuadrature === true, `${row.key} camera ${camera.cameraIndex} material placement role or budget drifted`);
  }
  return report;
}
...
try {
  ...
  require(Array.isArray(cohort.arms) && cohort.arms.length === expectedArmCoordinates.size, 'cohort must contain exactly six arms');
  ...
  const report = {
    ...
    transverseBases: Object.fromEntries(Object.keys(arms).map(key => [key, arms[key].effective.transverseBasis])),
