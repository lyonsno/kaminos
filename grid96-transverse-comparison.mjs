...
const TRANSVERSE_FOOTPRINT = 'flow-tangent-five-tap-plus-world-normal-binormal-symmetric-placement-v0';
...
function pageHtml(rows, summaries, ranking) {
...
try {
  ...
  const ranking = Object.keys(summaries).sort((a, b) => summaries[a].heldOut.mae - summaries[b].heldOut.mae);
  ...
  writeFileSync(join(stage, 'index.html'), pageHtml(rows, summaries, ranking));
  ...
  const report = {
    ...
    sourceIdentity: identity(control), transverseBasis: arms[ranking[0]].effective.transverseBasis,
  ...
  console.log(JSON.stringify({ status: 'complete', report: reportPath, gallery: join(outDir, 'index.html'), identity: report.identity, ranking, verification: finalVerification }, null, 2));
} catch (error) {
