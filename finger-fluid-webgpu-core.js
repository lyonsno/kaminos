...
export function fingerFluidAnalyticalSupportGeometry() {
  return { boundsMin: [...BOUNDS_MIN], boundsMax: [...BOUNDS_MAX],
...
export function validateFingerFluidMovingHillHostFrame(
  ...
  return validateFingerFluidHostFrame(hostFrame, options, {
    schema: KAMINOS_FINGER_FLUID_MOVING_HILL_HOST_FRAME_SCHEMA,
    route: KAMINOS_FINGER_FLUID_MOVING_HILL_HOST_FRAME_ROUTE,
  });
}
...
export function validateFingerFluidLocalHostFrame(hostFrame, options = {}) {
  if (hostFrame?.supportIdentity !== KAMINOS_FINGER_FLUID_ANALYTIC_SUPPORT_CONTACT_ROUTE) {
    fingerFluidMovingHillHostFrameFailure('requires the retained analytical support', { hostFrame, phase:'validate-local-support' });
  }
  const result = validateFingerFluidHostFrame(hostFrame, { ...options, expectedRemapGeneration: 0 }, {
    schema: KAMINOS_FINGER_FLUID_LOCAL_HOST_FRAME_SCHEMA,
    route: KAMINOS_FINGER_FLUID_LOCAL_HOST_FRAME_ROUTE,
  });
  return Object.freeze({ ...result, supportIdentity: hostFrame.supportIdentity });
}

function validateFingerFluidHostFrame(
  hostFrame,
  {
    device,
    extent,
  ...
  if (hostFrame?.schema !== contract.schema) {
    fail(`schema must be ${contract.schema}`);
  }
  ...
  if (
    hostFrame?.route?.requested !== contract.route
    || hostFrame?.route?.effective !== contract.route
...
export async function createWebGPUFingerFluidSolver({
  ...
  const localHostComposition = safePresentationMode === KAMINOS_FINGER_FLUID_LOCAL_PRESENTATION_MODE;
  ...
  if (
    ...
    && !localHostComposition
  ) {
    movingHillSupportFailure('host-frame composition requires moving_hill_consumer presentation');
  }
  if (localHostComposition && (!safeHostFrameComposition || supportContactRoute !== KAMINOS_FINGER_FLUID_ANALYTIC_SUPPORT_CONTACT_ROUTE)) {
    throw new Error('Local analytical presentation requires host-frame composition and retained analytical support');
  }
  ...
  function render({
    ...
    const validatedHostFrame = hostFrame
      ? (localHostComposition
        ? validateFingerFluidLocalHostFrame(hostFrame, hostValidationOptions)
        : validateFingerFluidMovingHillHostFrame(hostFrame, hostValidationOptions))
      : null;
    ...
    if (
      (safePresentationMode === KAMINOS_FINGER_FLUID_MOVING_HILL_PRESENTATION_MODE || localHostComposition)
    ...
    renderData.set([
      ...
      (safePresentationMode === KAMINOS_FINGER_FLUID_MOVING_HILL_PRESENTATION_MODE || localHostComposition) ? 1 : 0,
    ], 52);
    renderData.set(
    ...
    if (drawDynamicToyMesh) {
      ...
      lastDynamicMeshPresentationMode = localHostComposition
        ? 'suppressed_in_local_analytic_consumer_v0' : 'suppressed_in_moving_hill_consumer_v0';
    } else {
    ...
    if (validatedHostFrame) {
      lastHostFrameCompositionEvidence = Object.freeze({
        schema: localHostComposition ? 'kaminos.finger-fluid.local-analytic-host-frame-encode-evidence.v0' : KAMINOS_FINGER_FLUID_MOVING_HILL_HOST_FRAME_ENCODE_EVIDENCE_SCHEMA,
        ...
        supportIdentity: localHostComposition ? validatedHostFrame.supportIdentity : KAMINOS_FINGER_FLUID_MOVING_HILL_SUPPORT_CONTACT_ROUTE,
  ...
  function getDebugState() {
    return {
      ...
      presentationEvidence: {
        ...
        route: localHostComposition ? KAMINOS_FINGER_FLUID_LOCAL_HOST_FRAME_ROUTE
          : safePresentationMode === KAMINOS_FINGER_FLUID_MOVING_HILL_PRESENTATION_MODE
