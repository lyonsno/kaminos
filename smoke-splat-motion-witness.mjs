...
const heldCurrentState = requestedRoute === HELD_SMOKE_ASSAY_ROUTE_IDENTITY;
const expectedTemporalAuthority = heldCurrentState ? HELD_SMOKE_ASSAY_TEMPORAL_AUTHORITY : TEMPORAL_AUTHORITY;
...
try {
  ...
  if (!heldCurrentState) assert.ok(uniqueFrameDigests.size >= 2, 'frameDigest sequence is static or cached');
  ...
  const report = {
    ...
    motionEvidence: {
      ...
      runtimeFrameCounts,
      heldCurrentState,
      heldStateStable: heldCurrentState ? uniqueFrameDigests.size === 1 : null,
      staticOrCached: heldCurrentState ? null : false,
    },
    frames,
    falseClosureChecks: {
      ...
      rejectsStaticOrCachedFrames: !heldCurrentState,
      distinguishesHeldCurrentStateFromCachedMotion: true,
      rejectsTruncatedOrMassLosingProducts: true,
      preservesFailurePhaseBeforePrimaryOutput: true,
    },
    claimBoundary: heldCurrentState
      ? 'Checksum-bound static held-state A/B raster evidence. This does not prove temporal smoke decode or final flame-smoke depth composition.'
      : 'Standalone WebGPU smoke representation and raster motion evidence. This does not prove final flame-smoke depth composition or live recurrent smoke decode.',
  };
...
function validateState(state) {
  ...
  assert.equal(state.effectiveRoute, requestedRoute);
  assert.equal(state.fallbackReason, null);
  assert.match(state.backend || '', /^WebGPU:/);
  assert.equal(state.temporalAuthority, expectedTemporalAuthority);
  ...
  if (heldCurrentState) {
    assert.deepEqual(state.products.map(product => product.routeCell), ['A', 'B']);
    assert.equal(state.products.some(product => product.producerKind === 'learned-sparse-residual-plus-conserved-coarse'), true);
  } else {
