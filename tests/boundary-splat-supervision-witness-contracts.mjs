...
assert.match(witness, /supervisionMethod\s*=\s*prototype\?\.captureBoundarySplatSupervisionFrame[\s\S]*supervisionMethod\.call\(prototype,\s*\{[\s\S]*expectedRaySteps:\s*boundarySplatSupervisionExpectedRayStepsRequested/, 'witness passes its caller-owned teacher identity into the core capture before runtime-quality routing');
assert.match(witness, /supervision method surface[\s\S]*captureBoundarySplatSupervisionFrame/, 'witness failure names the available prototype method surface when the supervision API is missing');
