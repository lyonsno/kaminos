...
assert.match(raymarchLoop, /if \(flowKernelReconstructionActive\) \{[\s\S]*reconstructed = sampleWorldFlowReconstruction\(p\)[\s\S]*\} else \{[\s\S]*reconstructed = directSample\.reconstructed/, 'raymarch consumes the authored kernel bundle or the equivalent fused direct-cell bundle');
