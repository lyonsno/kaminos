...
assert.match(page, /<option value="140">140×280×140<\/option>/, 'the cockpit exposes the exact vertically expanded 140-cell assay grid');
...
assert.match(
  footprintAudit,
  /boundary-splat-footprint-audit-capacity-growth-failed[\s\S]*boundary-splat-footprint-audit-partial-source-candidates/,
...
assert.match(compactor, /shape = vec4<f32>\(effectiveMajorRadius, effectiveMinorRadius, packedKernelTangent\)/, 'the fixed candidate row carries the declared major/minor footprint and packed tangent');
...
assert.match(vertex, /kernelTangent = boundarySplatOctahedralDecode\(splat\.shape\.zw\)/, 'orientation is compactly stored with the candidate and reused across camera views');
