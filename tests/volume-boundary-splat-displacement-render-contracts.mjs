...
assert.match(core, /splatDisplacementEnabled:\s*survivalOnly\s*\?\s*0\s*:\s*clampFinite\(snapshot\.oracleActivitySplatDisplacement,\s*0,\s*1,\s*0\)/, 'displacement is explicit, bounded, inert by default, and disabled for survival-only cue authority');
