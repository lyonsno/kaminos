...
requirePatterns(index, 'receiver light', [
  ...
  [/window\.kaminosTier2ReceiverLightSetWitnessSurfaceMask/, 'witnesses can expose the same depth-derived receiver coverage used by the light pass'],
...
requirePatterns(witness, 'witness', [
  ...
  [/receiver-light requested route is inactive/, 'an incomplete operator URL fails instead of borrowing the witness route'],
  ...
  [/kaminosTier2ReceiverLightSetWitnessSurfaceMask\?\.\(true\)/, 'paired receiver proof captures explicit receiver coverage instead of inferring it from material color'],
  [/receiverMaskPath/, 'receiver proof writes the coverage-mask artifact used by the assay'],
  [/finally\s*\{[\s\S]*kaminosTier2ReceiverLightSetWitnessSurfaceMask\?\.\(false\)/, 'paired receiver proof restores normal output even when mask capture fails'],
  ...
  [/finally\s*\{[\s\S]*kaminosTier2ReceiverLightSetWitnessHudMute\?\.\(false\)/, 'paired receiver proof restores viewport chrome even when capture fails'],
...
requirePatterns(receiverMetrics, 'receiver delta metrics', [
  ...
  [/receiver-coverage-mask-missing/, 'missing explicit receiver coverage fails loud'],
  [/receiver-delta-escaped-surface-mask/, 'detached warm overlays fail across the full frame'],
