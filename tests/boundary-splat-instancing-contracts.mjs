...
assert.match(
  page,
  /function resolveVolumeCameraFrame\(\)[\s\S]*applyBoundarySplatFieldLayoutCamera[\s\S]*if \(composedCamera\) return null[\s\S]*function frameVolumeCamera\(\)[\s\S]*resolveVolumeCameraFrame\(\)[\s\S]*if \(!frame\) return/,
  'generic smoke framing must not overwrite the composed-field route camera',
);
