...
assert.match(coreSource, /archiveBoundarySplatHistory[\s\S]*candidateIndex < boundarySplatDraw\.sourceCandidateCount[\s\S]*boundarySplatNestedSourceIndex/, 'every live candidate must enter the existing history pass so camera-driven tier promotion cannot expose zeroed older prefixes');
