...
assert.match(page, /data-composition="splat-only-v0"/, 'operator page exposes splat-only attribution renderer composition');
assert.match(page, /data-composition="full-raymarch-under-splats-diagnostic-v0"/, 'operator page labels the old full hybrid as diagnostic');
...
assert.match(witness, /requestedPresetView === 'splat-only'[\s\S]*'splat-only-v0'/, 'continuous witness understands the explicit short preset splat view');
...
assert.match(witness, /isPresetLoader[\s\S]*requestedParams\.get\('view'\) \|\| 'splat-only'/, 'visual witness treats the operator preset loader default as explicit splat-only');
