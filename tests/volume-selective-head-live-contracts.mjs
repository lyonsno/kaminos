...
const replayAnchorStart = core.indexOf('async function loadSelectiveHeadLiveReplayAnchor');
const replayAnchorStop = core.indexOf('\n  function ensureBoundarySidecarBuffer', replayAnchorStart);
const replayAnchorLoader = core.slice(replayAnchorStart, replayAnchorStop);
...
assert.ok(
  /const expectedFluidBytes = fluidBufferBytes\(gridSize\);\s*const expectedFrontBytes = frontFieldBufferBytes\(gridSize\);/.test(replayAnchorLoader),
  'exact replay validates against the live expanded grid buffers instead of accepting cubic payloads into rectangular fields',
);
