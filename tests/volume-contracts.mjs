...
assert.match(core, /fn boundarySidecarSegmentMaxSample/, 'Raymarch has an explicit baked-sidecar ray-segment straddle sampler');
assert.match(core, /fn boundarySidecarSegmentMetaMaxSample/, 'Raymarch has an explicit baked-sidecar ray-segment meta sampler');
...
assert.match(core, /boundaryFireSamplingImportance/, 'Boundary-fire path exposes a scalar sampling-importance signal');
assert.match(core, /boundaryFirePreSkipImportance/, 'Boundary-fire path samples baked sidecar support before coarse majorant skip decisions');
assert.match(core, /boundarySidecarSegmentProbeSpan[\s\S]*boundarySidecarSegmentMaxSample/, 'Boundary-fire pre-skip guard samples sidecar support across the ray segment, not only at p');
assert.match(core, /boundaryFireSegmentSampleForShading/, 'Boundary-fire shading consumes the segment straddle sample for reconstruction');
assert.match(core, /boundaryProtectedOccupancy/, 'Boundary-fire sampling importance protects occupancy skip from starving transparent flame sheets');
assert.match(core, /raymarchInterest\([\s\S]*\+ boundaryFireSamplingImportance/, 'Boundary-fire sampling importance is added to adaptive ray interest before step sizing');
assert.match(core, /boundaryFireRenderActive[\s\S]*bonfireSpatialRayDephase/, 'Boundary-fire mode dampens old screen-space ray dephase before marching');
assert.match(core, /boundaryFireProceduralPhaseDamping[\s\S]*verticalPhaseBreak[\s\S]*verticalPuffBreak/, 'Boundary-fire mode dampens old render-only vertical phase noise before it can crawl over the sheet');
...
assert.match(bakedOnlyBranch[1], /boundarySidecarSegmentMaxSample\(p, rd, boundaryFireSegmentProbeSpanForShading\)/, 'Baked sidecar mode samples the baked structure field across the current ray segment');
assert.match(bakedOnlyBranch[1], /boundarySidecarSegmentMetaMaxSample\(p, rd, boundaryFireSegmentProbeSpanForShading\)/, 'Baked sidecar mode samples baked proximity/normal meta across the current ray segment');
...
assert.match(core, /async function sampleFrame\(\)[\s\S]*encodeMajorant\(encoder, \{ force: majorantRaymarchActive\(controlsSnapshot\) \}\)/, 'sampleFrame readback only forces a majorant bake when majorant traversal is active');
assert.match(core, /majorantOutputBricks/, 'sim-cost ledger distinguishes majorant output bricks from source-cell visits');
...
assert.match(core, /fn boundarySidecarDdaIntervalSupport/, 'Boundary-fire raymarch has a coarse forward support DDA for transparent-sheet sampling protection');
assert.match(core, /boundarySidecarDdaIntervalSupport\(\s*p,\s*rd,\s*dtBase,\s*adaptiveRays/, 'Boundary-fire DDA samples ahead from the current ray position using the active adaptive budget');
assert.match(core, /boundaryFireDdaSupport[\s\S]*boundaryProtectedOccupancy/, 'Boundary-fire DDA support protects occupancy skip from starving lower transparent flame sheets');
assert.match(core, /boundaryFireDdaSupport[\s\S]*raymarchInterest/, 'Boundary-fire DDA support feeds adaptive interest before step sizing');
assert.doesNotMatch(core, /boundaryCandidate\s*=\s*max\([^;]*boundaryFireDdaSupport/, 'Boundary-fire DDA support is sampling-only and must not directly become visible fire opacity');
assert.doesNotMatch(core, /boundaryFireLookPreservationMask/, 'Boundary-fire adaptive path must not reintroduce the hero-look-coupled preservation mask');
...
assert.match(core, /supportDdaIdentity:\s*'boundary-sidecar-coarse-support-dda-v0'/, 'Debug state exposes the active coarse support DDA identity');
assert.match(index, /id="volume-boundary-sidecar-dda-gate"/, 'Boundary-fire cockpit exposes the coarse support DDA gate slider');
assert.match(index, /id="volume-boundary-sidecar-dda-span"/, 'Boundary-fire cockpit exposes the coarse support DDA span slider');
assert.match(index, /id="volume-boundary-sidecar-dda-threshold"/, 'Boundary-fire cockpit exposes the coarse support DDA threshold slider');
...
assert.match(core, /const sampleLookFreeze = normalizeLookFreeze\(controlsSnapshot\.lookFreeze\)[\s\S]*if \(!sampleLookFreeze\) \{[\s\S]*encodeSim\(encoder\)[\s\S]*encodeMajorant\(encoder, \{ force: majorantRaymarchActive\(controlsSnapshot\) \}\)/, 'sampleFrame must not advance sim while look freeze is active and must not force majorant when traversal is disabled');
...
assert.match(witness, /occupiedBricks/, 'witness inspects coarse majorant brick occupancy');
...
assert.match(witness, /expectedMajorantRaymarchActive/, 'witness derives expected majorant traversal state from the route');
assert.match(witness, /expectedMajorantBuildRequested/, 'witness accepts disabled majorant construction when majorant traversal is off');
assert.match(witness, /coarse majorant field was built even though traversal was disabled/, 'witness fails loud if majorant work comes back while majorant skip is zero');
assert.match(witness, /GPU majorant readback shows coarse occupancy work even though traversal was disabled/, 'witness GPU readback accepts a disabled majorant only when the coarse field stayed empty');
assert.match(witness, /acceptsBoundaryFireRenderEmission/, 'witness distinguishes boundary-fire render emission from old transported sim radiance');
assert.match(witness, /fire radiance or boundary-fire render-emission evidence/, 'witness fails loud when neither old radiance nor boundary-fire evidence is present');
assert.match(witness, /fireFuelOverlapRatio <= 0\.01 && !acceptsBoundaryFireRenderEmission/, 'witness does not require old transported fire/fuel overlap when boundary-fire render emission is proven');
assert.match(witness, /acceptsBoundaryFireVisualSignal/, 'witness visual gate accepts pale boundary-fire frames through route-specific volume evidence');
assert.match(witness, /boundaryFireVisualSignalPixels/, 'witness reports the pale boundary-fire visual signal score when the old orange-fire gate fails');
