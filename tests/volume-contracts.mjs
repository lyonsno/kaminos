...
const nativeLowSelectiveComposePath = join(root, 'volume-native-low-selective-compose.py');
assert.ok(existsSync(nativeLowSelectiveComposePath), 'native-low selective compose application exists');
const nativeLowSelectiveCompose = readFileSync(nativeLowSelectiveComposePath, 'utf8');
const nativeLowSelectiveWitnessPath = join(root, 'volume-native-low-selective-witness.mjs');
assert.ok(existsSync(nativeLowSelectiveWitnessPath), 'native-low selective visual witness exists');
const nativeLowSelectiveWitness = readFileSync(nativeLowSelectiveWitnessPath, 'utf8');
const fullGridFieldExport = readFileSync(join(root, 'volume-full-grid-field-export.mjs'), 'utf8');
...
assert.match(nativeLowSelectiveCompose, /failurePhase/, 'native-low application writes a durable failure phase');
assert.match(nativeLowSelectiveWitness, /nativeLowControl/, 'native-low witness includes the untouched native simulator control');
assert.match(nativeLowSelectiveWitness, /nativeLowSelectivePredicted/, 'native-low witness includes the frozen-model treatment');
assert.match(nativeLowSelectiveWitness, /splat-only-v0/, 'native-low witness requests splat-only composition for the first discriminant');
assert.match(nativeLowSelectiveWitness, /boundarySplatCompositionEffective/, 'native-low witness verifies effective renderer composition');
assert.match(nativeLowSelectiveWitness, /sameNativeStateIdentity/, 'native-low witness records the shared native input identity');
assert.match(fullGridFieldExport, /kaminos\.volume\.native-low-selective-composition\.v0/, 'full-grid importer explicitly admits native-low learned compositions');
assert.match(fullGridFieldExport, /native-low-simulator-state-no-synthetic-downsample-v0/, 'full-grid importer verifies native-low application authority');
