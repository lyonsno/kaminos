...
assert.match(index, /hand_pose_fixture/, 'Volume route exposes an explicitly non-live hand-pose adapter fixture smoke');
assert.match(index, /setHandPoseFrame/, 'Volume route can seed hand-pose frames through the public adapter API');
...
assert.match(core, /normalizeHandPoseEmitters/, 'volume module normalizes hand-pose frames into external emitter segments');
assert.match(core, /setHandPoseFrame/, 'volume prototype exposes a public hand-pose frame adapter');
assert.match(core, /requestedHandPoseBackend/, 'debug state records requested hand-pose backend identity');
assert.match(core, /effectiveHandPoseBackend/, 'debug state records effective hand-pose backend identity');
assert.match(core, /handPoseEvidenceKind/, 'debug state distinguishes live, captured, fallback, and synthetic hand-pose evidence');
assert.match(core, /handPoseStale/, 'debug state fails loud when hand-pose input is stale');
assert.match(core, /handPoseFrameId/, 'debug state records hand-pose frame identity');
assert.match(core, /handPoseHandCount/, 'debug state records effective detected hand count');
assert.match(core, /handPoseSegmentCount/, 'debug state records emitted hand-trail segment count');
assert.match(core, /handPoseAdapterWarnings/, 'debug state preserves hand-pose adapter warnings instead of silently accepting bad payloads');
...
assert.doesNotMatch(core, /mix\(0\.18, 0\.78, explicitWindAuthority\)/, 'bonfire no-wind velocity damping must not retain hidden lateral motion');
assert.match(core, /bonfireAdvectionLateralDamping = mix\(1\.0, explicitWindAuthority, bonfireScene\)/, 'bonfire lateral advection authority comes only from explicit wind');
assert.match(core, /bonfireDetailLateralDamping = mix\(1\.0, explicitWindAuthority, bonfireScene\)/, 'bonfire detail-force lateral authority comes only from explicit wind');
...
assert.match(witness, /hand_pose_fixture/, 'witness can drive the explicit hand-pose adapter fixture route');
assert.match(witness, /expectedHandPoseEvidenceKind/, 'witness verifies hand-pose evidence kind for adapter routes');
assert.match(witness, /requestedHandPoseBackend/, 'witness records requested hand-pose backend identity');
assert.match(witness, /effectiveHandPoseBackend/, 'witness records effective hand-pose backend identity');
assert.match(witness, /handPoseEvidenceKind/, 'witness records hand-pose evidence kind');
assert.match(witness, /handPoseStale/, 'witness records whether hand-pose input was stale');
assert.match(witness, /handPoseSegmentCount/, 'witness records emitted hand-pose segment count');
...
assert.match(sweep, /requestedHandPoseBackend/, 'sweep aggregate preserves requested hand-pose backend identity');
assert.match(sweep, /effectiveHandPoseBackend/, 'sweep aggregate preserves effective hand-pose backend identity');
assert.match(sweep, /handPoseEvidenceKind/, 'sweep aggregate preserves hand-pose evidence kind');
assert.match(sweep, /handPoseStale/, 'sweep aggregate preserves hand-pose stale/fresh status');
assert.match(sweep, /handPoseSegmentCount/, 'sweep aggregate preserves hand-pose emitted segment count');
