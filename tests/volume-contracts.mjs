...
assert.match(index, /exploding_jellow_fireball_basin_0706/, 'Volume tab pins the operator-found exploding jellow fireball Pyro look by stable public-safe route identity');
...
assert.match(serve, /\.f32/, 'Local server /api/read serves learned dense cue f32 arrays as binary artifacts');
...
assert.match(volumeWitness, /capture replay screenshot missing visible volume/, 'Capture replay refuses to present a blank screenshot as proof');
...
assert.match(index, /<option value="learned">learned<\/option>/, 'Boundary Fire structure source exposes route-loadable learned dense sidecar cues');
assert.match(index, /volume_learned_boundary_sidecar_manifest_url/, 'Basin URLs preserve learned dense sidecar cue manifest source');
assert.match(index, /volume_learned_boundary_sidecar_variant/, 'Basin URLs preserve learned dense sidecar cue variant selection');
assert.match(index, /volume_learned_boundary_sidecar_classifier_threshold/, 'Basin URLs preserve learned ridge-classifier probability threshold');
assert.match(index, /volume_learned_boundary_sidecar_role/, 'Basin URLs preserve learned dense sidecar role routing');
assert.match(index, /volume_learned_boundary_sidecar_flame_mask/, 'Basin URLs preserve learned flame geography mask routing');
assert.match(index, /volume_learned_boundary_sidecar_smoke_mask/, 'Basin URLs preserve learned smoke geography mask routing');
assert.match(index, /volume_learned_boundary_sidecar_smoke_gain/, 'Basin URLs preserve learned smoke receiver gain');
assert.match(index, /volume_learned_boundary_sidecar_smoke_warm_reject/, 'Basin URLs preserve learned smoke warm-body rejection');
assert.match(index, /normalizeLearnedBoundarySidecarRole/, 'Page route normalizes learned sidecar role aliases before receiver upload');
assert.match(index, /normalizeLearnedBoundarySidecarFlameMask/, 'Page route normalizes learned flame geography mask aliases before receiver upload');
assert.match(index, /normalizeLearnedBoundarySidecarSmokeMask/, 'Page route normalizes learned smoke geography mask aliases before receiver upload');
assert.match(index, /normalizeLearnedBoundarySidecarSmokeGain/, 'Page route normalizes learned smoke receiver gain before upload');
assert.match(index, /normalizeLearnedBoundarySidecarSmokeWarmReject/, 'Page route normalizes learned smoke warm-body rejection before upload');
assert.match(core, /LEARNED_BOUNDARY_SIDECAR_ROLE_IDENTITY\s*=\s*'learned-boundary-sidecar-smoke-flame-role-v0'/, 'Renderer names learned smoke/flame role routing identity');
assert.match(core, /LEARNED_BOUNDARY_SIDECAR_MASK_IDENTITY\s*=\s*'learned-boundary-sidecar-geography-mask-v0'/, 'Renderer names learned smoke/flame geography mask routing identity');
assert.match(core, /LEARNED_BOUNDARY_SIDECAR_RECEIVER_IDENTITY\s*=\s*'learned-boundary-sidecar-smoke-receiver-valve-v0'/, 'Renderer names learned smoke receiver gain/rejection identity');
assert.match(core, /function normalizeLearnedBoundarySidecarRole/, 'Renderer normalizes learned sidecar role aliases');
assert.match(core, /function learnedBoundarySidecarRoleValue/, 'Renderer maps learned sidecar role into explicit uniforms');
assert.match(core, /function normalizeLearnedBoundarySidecarFlameMask/, 'Renderer normalizes learned flame geography mask aliases');
assert.match(core, /function normalizeLearnedBoundarySidecarSmokeMask/, 'Renderer normalizes learned smoke geography mask aliases');
assert.match(core, /function normalizeLearnedBoundarySidecarSmokeReceiverControls/, 'Renderer clamps learned smoke receiver controls before uniforms/debug');
assert.match(core, /function learnedBoundarySidecarFlameMaskValue/, 'Renderer maps learned flame geography masks into explicit uniforms');
assert.match(core, /function learnedBoundarySidecarSmokeMaskValue/, 'Renderer maps learned smoke geography masks into explicit uniforms');
assert.match(core, /learned_boundary_smoke_controls:\s*vec4<f32>/, 'Raymarch uniforms carry learned smoke receiver controls separately from sidecar thresholds');
assert.match(core, /learnedBoundarySidecarRole/, 'Debug state reports the effective learned sidecar role');
assert.match(core, /learnedBoundarySidecarFlameMask/, 'Debug state reports the effective learned flame geography mask');
assert.match(core, /learnedBoundarySidecarSmokeMask/, 'Debug state reports the effective learned smoke geography mask');
assert.match(core, /learnedBoundarySidecarSmokeReceiverIdentity/, 'Debug state reports the learned smoke receiver control identity');
assert.match(core, /learnedBoundarySidecarSmokeGain/, 'Debug state reports the effective learned smoke receiver gain');
assert.match(core, /learnedBoundarySidecarSmokeWarmReject/, 'Debug state reports the effective learned smoke warm-body rejection');
assert.match(core, /learnedSmokeRoleActive/, 'Raymarch has an explicit smoke/plume-only learned sidecar gate');
assert.match(core, /learnedFlameRoleActive/, 'Raymarch has an explicit flame/body learned sidecar gate');
assert.match(core, /learnedFlameGeographyMask/, 'Raymarch has an explicit learned flame injection geography mask');
assert.match(core, /learnedSmokeGeographyMask/, 'Raymarch has an explicit learned smoke injection geography mask');
assert.match(core, /learnedFlameLiveBoundaryMask/, 'Raymarch can restrict learned flame to low-grid live boundary authority');
assert.match(core, /learnedSmokeUpperPlumeFireRejectMask/, 'Raymarch can restrict learned smoke to upper plume with fire rejection');
assert.match(core, /learnedSmokeWarmRejectMask/, 'Raymarch has a learned smoke warm-body rejection mask');
assert.match(core, /learnedSmokeCue\s*\*\s*learnedSmokeGain\s*\*\s*learnedSmokeWarmRejectMask\s*\*\s*learnedSmokeGeographyMask/, 'Learned smoke receiver controls materially modulate learned smoke opacity before compositing');
assert.match(core, /learnedSmokeCueAlpha/, 'Smoke-only learned sidecar role materially affects smoke/plume opacity instead of being a label-only route');
assert.match(index, /loadLearnedBoundarySidecarCueFromRoute/, 'Volume route loads learned dense sidecar cue packs without DevTools injection');
...
assert.match(phaseAlignedProbeWitness, /kaminos\.volume\.phase-aligned-probe-witness\.v0/, 'phase-aligned witness writes a stable schema identity');
assert.match(phaseAlignedProbeWitness, /boundary_fire_bonfire_a_la_ruffles_0709/, 'phase-aligned witness targets the landed Boundary Fire basin');
assert.match(phaseAlignedProbeWitness, /requestedGrid/, 'phase-aligned witness records the requested receiver/input grid');
assert.match(phaseAlignedProbeWitness, /effectiveGrid/, 'phase-aligned witness records the effective renderer grid');
assert.match(phaseAlignedProbeWitness, /referenceGrid/, 'phase-aligned witness records the richer reference grid separately from the default');
assert.match(phaseAlignedProbeWitness, /defaultGrid:\s*128/, 'phase-aligned witness preserves 128 as the landed default grid');
assert.match(phaseAlignedProbeWitness, /referenceGrid:\s*160/, 'phase-aligned witness preserves 160 as the richer operator reference');
assert.match(phaseAlignedProbeWitness, /bakeDataAvailable:\s*false/, 'phase-aligned witness refuses to claim missing bake data exists');
assert.match(phaseAlignedProbeWitness, /bakeDataIdentity:\s*null/, 'phase-aligned witness records null bake identity until bake data lands');
assert.match(phaseAlignedProbeWitness, /downsampleOperatorIdentity/, 'phase-aligned witness records the downsample-operator identity placeholder');
assert.match(phaseAlignedProbeWitness, /pending-vivisector-phase-aligned-corpus-downsample-operator-v0/, 'phase-aligned witness names the corpus-owner placeholder downsample operator');
assert.match(phaseAlignedProbeWitness, /reactionFrontAtlas/, 'phase-aligned witness captures reaction-front atlas evidence');
assert.match(phaseAlignedProbeWitness, /topologyShellIdentity/, 'phase-aligned witness records topology shell identity');
...
assert.match(index, /exploding-jellow-fireball-basin/, 'Source-backed look library exposes the operator-saved exploding jellow Pyro look under a public-safe name');
...
assert.match(core, /LEARNED_BOUNDARY_SIDECAR_CUE_IDENTITY\s*=\s*'dense-learned-sparse-cue-pack-v0'/, 'Renderer preserves learned dense cue pack identity separately from baked truth sidecars');
assert.match(core, /LEARNED_BOUNDARY_SIDECAR_UPLOAD_AUTHORITY\s*=\s*'teacher-domain-phase-aligned-learned-sidecar-cue-upload-v0'/, 'Renderer marks learned sidecar uploads as teacher-domain evidence, not product/native-low closure');
assert.match(core, /function normalizeLearnedBoundarySidecarVariant/, 'Renderer normalizes learned dense sidecar variant selection');
assert.match(core, /normalizeLearnedBoundarySidecarClassifierThreshold/, 'Renderer normalizes learned ridge-classifier probability threshold');
...
const bakedOrFlameLearnedBranch = boundaryFireBranch[0].match(/if \(\(boundarySidecarSource > 0\.5 && boundarySidecarSource <= 1\.5\) \|\| \(boundarySidecarSource > 2\.5 && learnedFlameRoleActive\)\) \{([\s\S]*?)\} else \{/);
assert.ok(bakedOrFlameLearnedBranch, 'Baked sidecar and flame-authorized learned sidecar modes share the sidecar-only branch distinct from live/mix/smoke-only');
assert.match(bakedOrFlameLearnedBranch[1], /sampleWorldBoundarySidecar\(p\)/, 'Baked/flame-learned sidecar mode samples the structure field');
assert.match(bakedOrFlameLearnedBranch[1], /sampleWorldBoundarySidecarMeta\(p\)/, 'Baked/flame-learned sidecar mode samples the proximity/normal meta field');
assert.match(bakedOrFlameLearnedBranch[1], /learnedFlameMaskMode > 0\.5 && learnedFlameMaskMode <= 1\.5[\s\S]*liveBoundarySupportAt\(p, boundarySupportWeights\)/, 'Flame-learned sidecar mode pays live boundary support only for the explicit live_boundary geography mask');
assert.match(bakedOrFlameLearnedBranch[1], /learnedFlameMaskMode > 1\.5 && learnedFlameMaskMode <= 2\.5[\s\S]*learnedFlameFireAuthorityMask/, 'Flame-learned sidecar mode has a cheaper fire-authority geography mask distinct from live boundary sampling');
assert.match(boundaryFireBranch[0], /if \(boundarySidecarSource > 1\.5 && boundarySidecarSource <= 2\.5\) \{[\s\S]*mix\(boundarySupport, boundarySidecarSupportThresholded, 0\.5\)/, 'Mix sidecar mode is the explicit expensive live+baked comparison path with calibrated sparse cue thresholds');
...
assert.match(core, /uniforms\.set\(previousViewProj\.elements,\s*328\)/, 'previous view-projection matrix shifts after Bite-stack, flow, palette, topology-shell lab, boundary-fire, boundary sidecar reconstruction, sidecar display, learned smoke receiver, and scalar activity receiver uniforms');
...
assert.match(witness, /exploding_jellow_fireball_basin_0706/, 'witness recognizes the 2026-07-06 operator-found exploding jellow fireball Pyro basin under a public-safe id');
...
assert.match(witness, /if \(\s*!expectsCanonicalPlumeProof\s*&&\s*!expectsFuelStarvedTallPlume\s*&&\s*!expectsNoFireVolumeEvidence\s*&&\s*!expectsBoundaryFireEvidence\s*&&\s*\(!Number\.isFinite\(sample\.simReadback\.fireLayerMean\) \|\| sample\.simReadback\.fireLayerMean <= 0\.0005\)\s*&&\s*!acceptsRawCarrierPyroPaint\s*\)/, 'no-fire and boundary-fire evidence routes do not require positive transported fire-layer readback');
assert.match(witness, /!expectsFuelStarvedTallPlume\s*&&\s*!expectsNoFireVolumeEvidence\s*&&\s*!expectsBoundaryFireEvidence[\s\S]*radianceMean/, 'no-fire and boundary-fire evidence routes do not require positive fire radiance readback');
assert.match(witness, /boundary-fire/, 'witness accepts a boundary-fire inspect evidence mode');
assert.match(witness, /boundary-fire-sidecar-signal/, 'witness reports boundary-fire sidecar visual evidence identity');
assert.match(witness, /expectsBoundaryFireEvidence/, 'witness separates boundary-fire inspect evidence from stock fire-volume gates');
assert.match(witness, /buildBoundaryFireEvidence/, 'witness builds explicit boundary-fire sidecar evidence');
assert.match(witness, /boundary-fire inspect route did not expose active sidecar evidence/, 'boundary-fire witness fails loud when sidecar route authority is missing');
assert.match(witness, /else if \(\s*!expectsBoundaryFireEvidence\s*&&\s*\([\s\S]*fireFuelOverlapRatio <= 0\.01/, 'boundary-fire evidence routes do not require stock tall-plume live fuel/fire overlap evidence');
...
for (const role of ['hot-core', 'fire-body', 'smoke-body', 'edge-breakup', 'radiance-gain', 'confidence-alpha']) {
  assert.match(pyroRgbIntermediateDecoderExport, new RegExp(role), `Pyro RGB intermediate decoder exporter emits ${role}`);
}
