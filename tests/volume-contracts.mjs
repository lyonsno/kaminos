...
const fireLightProbeSource = index.slice(fireLightProbeStart, fireLightProbeEnd);
assert.match(index, /volume-measured-fire-light-probe-v0/, 'brick-wall fire-light proxy must name measured sim/frame light authority');
assert.match(fireLightProbeSource, /measuredAuthorityIdentity/, 'brick-wall fire-light probe must use the stable measured light authority identity');
assert.match(fireLightProbeSource, /measuredFireLight/, 'brick-wall fire-light proxy must expose its measured sim/frame light summary');
assert.match(fireLightProbeSource, /simReadback/, 'brick-wall fire-light proxy must use sim readback as light evidence');
assert.match(fireLightProbeSource, /measuredSimAvailable/, 'brick-wall fire-light proxy must mark optional sim readback availability instead of presenting zeros as authority');
assert.doesNotMatch(fireLightProbeSource, /controlsState\.plumeHeight|const\s+plumeHeight|plumeHeight\s*\*/, 'brick-wall fire-light proxy must not treat plume height as measured light reach or centroid authority');
