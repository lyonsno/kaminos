...
for (const plumeControl of [
  ['smokePlumeStemFocus', 'volume-smoke-plume-stem-focus', 'volume_smoke_plume_stem_focus'],
  ['smokePlumeEntrainment', 'volume-smoke-plume-entrainment', 'volume_smoke_plume_entrainment'],
  ['smokePlumeCapRoll', 'volume-smoke-plume-cap-roll', 'volume_smoke_plume_cap_roll'],
  ['smokePlumeAgeContrast', 'volume-smoke-plume-age-contrast', 'volume_smoke_plume_age_contrast'],
]) {
  const [key, id, param] = plumeControl;
  assert.match(index, new RegExp(`id="${id}"[^>]+min="0"[^>]+max="1"`), `${key} explosion-plume slider exposes the full zero-to-one assay range`);
  assert.match(index, new RegExp(`${param}`), `${key} explosion-plume slider is routable for exact operator smoke replay`);
  assert.match(index, new RegExp(`${key}:\\s*parseFloat\\(document\\.getElementById\\('${id}'\\)\\.value\\)`), `${key} reaches readVolumeControls`);
  assert.match(index, new RegExp(`${id}-val`), `${key} has a visible live value readout`);
}
assert.match(index, /data-volume-control-section="explosion-plume-dynamics"[\s\S]*volume-smoke-plume-stem-focus[\s\S]*volume-smoke-plume-age-contrast/, 'explosion plume controls sit in their own top-of-volume section');
assert.match(index, /EXPLOSION_PLUME_CONTROL_FIELDS[\s\S]*volume_smoke_plume_stem_focus[\s\S]*volume_smoke_plume_age_contrast/, 'explosion plume routes and listeners are table-driven');
for (const receiverControl of [
  ['farSmokeReceiverRange', 'volume-far-smoke-receiver-range', 'volume_far_smoke_receiver_range'],
  ['farSmokeReceiverOverlap', 'volume-far-smoke-receiver-overlap', 'volume_far_smoke_receiver_overlap'],
  ['farSmokeReceiverCarrier', 'volume-far-smoke-receiver-carrier', 'volume_far_smoke_receiver_carrier'],
  ['farSmokeReceiverRimCurl', 'volume-far-smoke-receiver-rim-curl', 'volume_far_smoke_receiver_rim_curl'],
]) {
  const [key, id, param] = receiverControl;
  assert.match(index, new RegExp(`id="${id}"[^>]+min="0"[^>]+max="1"`), `${key} far-smoke receiver slider exposes the full zero-to-one assay range`);
  assert.match(index, new RegExp(`${param}`), `${key} far-smoke receiver slider is routable for exact operator smoke replay`);
  assert.match(index, new RegExp(`${key}:\\s*parseFloat\\(document\\.getElementById\\('${id}'\\)\\.value\\)`), `${key} reaches readVolumeControls`);
  assert.match(index, new RegExp(`${id}-val`), `${key} has a visible live value readout`);
}
...
for (const transportControl of [
  ['farSmokeTransportLift', 'volume-far-smoke-transport-lift', 'volume_far_smoke_transport_lift'],
  ['farSmokeTransportCarry', 'volume-far-smoke-transport-carry', 'volume_far_smoke_transport_carry'],
  ['farSmokeTransportSourceDepth', 'volume-far-smoke-transport-source-depth', 'volume_far_smoke_transport_source_depth'],
  ['farSmokeTransportGhostKill', 'volume-far-smoke-transport-ghost-kill', 'volume_far_smoke_transport_ghost_kill'],
]) {
  const [key, id, param] = transportControl;
  assert.match(index, new RegExp(`id="${id}"[^>]+min="0"[^>]+max="1"`), `${key} far-smoke transport slider exposes the full zero-to-one assay range`);
  assert.match(index, new RegExp(`${param}`), `${key} far-smoke transport slider is routable for exact operator smoke replay`);
  assert.match(index, new RegExp(`${key}:\\s*parseFloat\\(document\\.getElementById\\('${id}'\\)\\.value\\)`), `${key} reaches readVolumeControls`);
  assert.match(index, new RegExp(`${id}-val`), `${key} has a visible live value readout`);
}
assert.match(index, /FAR_SMOKE_TRANSPORT_CONTROL_FIELDS[\s\S]*volume_far_smoke_transport_lift[\s\S]*volume_far_smoke_transport_ghost_kill/, 'far smoke transport routes and listeners are table-driven');
assert.match(index, /id="volume-far-smoke-isolation-mode"[\s\S]*value="composite"[\s\S]*value="near-only"[\s\S]*value="far-grid-only"[\s\S]*value="receiver-only"/, 'far-smoke isolation exposes composite, near-only, far-grid-only, and receiver-only smoke modes');
assert.match(index, /FAR_SMOKE_ISOLATION_MODE_VALUES[\s\S]*composite[\s\S]*near-only[\s\S]*far-grid-only[\s\S]*receiver-only/, 'far-smoke isolation route values are normalized through an explicit allowlist');
assert.match(index, /volume_far_smoke_isolation/, 'far-smoke isolation mode is routable for exact operator smoke replay');
assert.match(index, /farSmokeIsolationMode:\s*normalizeFarSmokeIsolationMode\(document\.getElementById\('volume-far-smoke-isolation-mode'\)\.value\)/, 'far-smoke isolation reaches readVolumeControls as a normalized mode');
assert.match(index, /volume-far-smoke-isolation-mode-val/, 'far-smoke isolation has a visible live value readout');
...
assert.match(core, /async function sampleFrame\(\)[\s\S]*encodeSim\(encoder\)[\s\S]*encodeFarSmokeGrid\(encoder\)[\s\S]*encodeMajorant\(encoder, \{ force: true \}\)[\s\S]*encodeBoundarySidecar\(encoder\)[\s\S]*encodeDraw\(encoder, frameTexture\.createView\(\), 'kaminos volume one-off readback pass'/, 'sampleFrame readback advances sim, far-smoke grid, and boundary sidecar before drawing evidence frames');
...
assert.match(core, /EXPLOSION_PLUME_SMOKE_DYNAMICS_IDENTITY\s*=\s*'explosion-plume-smoke-dynamics-v0'/, 'volume core names the explosion plume smoke dynamics receipt');
...
assert.match(core, /FAR_SMOKE_SOURCE_SEED_IDENTITY\s*=\s*'far-smoke-live-source-seed-v0'/, 'far-smoke grid names the live source-seed bridge receipt');
assert.match(core, /far_smoke_grid_controls:\s*vec4<f32>/, 'fluid uniforms carry far-smoke grid controls explicitly');
assert.match(core, /FAR_SMOKE_TRANSPORT_IDENTITY\s*=\s*'far-smoke-connected-transport-v0'/, 'volume core names the far-smoke connected transport receipt');
assert.match(core, /FAR_SMOKE_CARRY_IDENTITY\s*=\s*'far-smoke-buoyant-entrainment-carry-v0'/, 'volume core names the far-smoke buoyant entrainment carry receipt');
assert.match(core, /far_smoke_transport_controls:\s*vec4<f32>/, 'fluid uniforms carry far-smoke transport controls explicitly');
assert.match(core, /FAR_SMOKE_ISOLATION_IDENTITY\s*=\s*'far-smoke-evidence-isolation-v0'/, 'volume core names the far-smoke isolation evidence receipt');
assert.match(core, /far_smoke_isolation_controls:\s*vec4<f32>/, 'fluid uniforms carry far-smoke isolation controls explicitly');
assert.match(core, /const uniforms = new Float32Array\(368\)/, 'adding far-smoke isolation controls shifts the uniform buffer deliberately');
assert.match(core, /uniforms\.set\(previousViewProj\.elements,\s*352\)/, 'previous view-projection matrix starts after the far-smoke isolation controls');
...
assert.match(core, /fn explosionPlumeSmokeForce/, 'fluid shader has an explosion-plume force separate from generic smoke morphology');
...
assert.match(core, /fn farSmokeOverlapInjection[\s\S]*u\.primitive_source\.y/, 'far-smoke grid overlap injection samples from the live primitive source height instead of a stale positive receiver band');
assert.match(core, /fn farSmokeGridSourceSeed[\s\S]*u\.primitive_source\.y/, 'far-smoke grid has a live primitive-source seed for empty upper-grid birth');
assert.match(core, /fn farSmokeGridConnectedSupport/, 'far-smoke grid has an explicit connected-support estimator for ghost suppression');
assert.match(core, /fn farSmokeGridEntrainmentVelocity/, 'far-smoke grid has an explicit buoyant entrainment velocity term');
assert.match(core, /fn farSmokeGridSurvivalSupport/, 'far-smoke grid has an explicit survival-support term to keep connected scalar mass alive');
assert.match(core, /plumeBodyRadius[\s\S]*plumeEnvelope[\s\S]*containment = sideFade \* topFade \* bottomFade \* plumeEnvelope \* upperLife/, 'far-smoke grid applies a radial plume envelope so connected support cannot fill the whole upper-volume box');
assert.match(core, /let connected = clamp\(max\(max\(connectedSupport \* 0\.86, injected\.w \* 0\.94\), advected\.w \* mix\(0\.965, 0\.990, transport\.y\)\)/, 'far-smoke grid decays connected support instead of preserving an immortal full-volume support channel');
assert.match(core, /farSmokeTransportCarry/, 'far-smoke grid transport names the near-grid carry term explicitly');
assert.match(core, /farSmokeTransportGhostKill/, 'far-smoke grid transport names the disconnected ghost-kill term explicitly');
assert.match(core, /farSmokeTransportSourceDepth/, 'far-smoke grid transport names the source-depth term explicitly');
assert.match(core, /nativeVolumeVisibility\s*=\s*farSmokeIsolationNearVisibility/, 'far-smoke isolation has an explicit native-volume visibility gate');
assert.match(core, /let fireAlpha\s*=\s*\([\s\S]*stockFireAlpha[\s\S]*shellAlpha[\s\S]*inspectAlpha[\s\S]*\)\s*\*\s*nativeVolumeVisibility/, 'far-smoke non-near isolation modes suppress native fire alpha');
assert.match(core, /farSmokeIsolationNearVisibility[\s\S]*farSmokeIsolationReceiverVisibility[\s\S]*farSmokeIsolationGridVisibility/, 'raymarch shader splits near, receiver, and far-grid visibility for isolation smoke modes');
assert.match(core, /farReceiverProbe = farSmokeReceiverProbe\(p\) \* farSmokeIsolationReceiverVisibility/, 'far-smoke receiver probe is hidden in non-receiver isolation modes');
assert.match(core, /farGridProbe = farSmokeGridProbe\(p\) \* farSmokeIsolationGridVisibility/, 'far-smoke grid probe is hidden in non-grid isolation modes');
assert.match(core, /farReceiver = farSmokeReceiverSample\(p,\s*u\.cameraPos_time\.w\) \* farSmokeIsolationReceiverVisibility/, 'far-smoke receiver contribution is hidden in near-only and far-grid-only smoke modes');
assert.match(core, /farGridSmoke[\s\S]*\* farSmokeIsolationGridVisibility/, 'far-smoke grid contribution is hidden in near-only and receiver-only smoke modes');
assert.match(core, /nearDensity[\s\S]*\* farSmokeIsolationNearVisibility/, 'native near volume density is hidden in far-grid-only and receiver-only smoke modes');
...
assert.match(core, /vel\s*=\s*vel\s*\+\s*explosionPlumeForce\s*\*\s*activityVorticityGate/, 'explosion plume dynamics inject into velocity through the existing activity vorticity gate');
...
assert.match(core, /sourceSeedIdentity:\s*FAR_SMOKE_SOURCE_SEED_IDENTITY/, 'debug state reports the far-smoke source-seed bridge identity');
assert.match(core, /farSmokeTransport:\s*\{[\s\S]*identity:\s*FAR_SMOKE_TRANSPORT_IDENTITY[\s\S]*lift[\s\S]*ghostKill/, 'debug state reports far-smoke connected transport controls');
assert.match(core, /carryIdentity:\s*FAR_SMOKE_CARRY_IDENTITY/, 'debug state reports the far-smoke buoyant entrainment carry identity');
assert.match(core, /label: `kaminos \$\{FAR_SMOKE_GRID_IDENTITY\} \$\{FAR_SMOKE_GRID_SIZE\}\^3 \$\{i\}`[\s\S]*usage: GPUBufferUsage\.STORAGE \| GPUBufferUsage\.COPY_DST \| GPUBufferUsage\.COPY_SRC/, 'far-smoke grid storage buffers are copy sources so CPU readback evidence cannot silently no-op');
assert.match(core, /const buildFarSmokeGridReadback = \(farSmokeData, readIndex\) => \{[\s\S]*identity:\s*'far-smoke-grid-readback-v0'[\s\S]*backend:\s*'cpu-far-smoke-buffer-readback'/, 'sim readback computes far-smoke grid scalar stats from a named storage buffer');
assert.match(core, /const farSmokeBufferReadbacks = farSmokeDatas\.map\(\(farSmokeData, readIndex\) => buildFarSmokeGridReadback\(farSmokeData, readIndex\)\)/, 'sim readback records both far-smoke ping-pong buffers so zero active-buffer evidence cannot hide a write-index mismatch');
assert.match(core, /bufferReadbacks:\s*farSmokeBufferReadbacks/, 'far-smoke readback payload exposes both ping-pong buffer readbacks');
assert.match(core, /farSmokeGridReadback,/, 'sim readback returns the far-smoke grid scalar stats payload');
assert.match(core, /farSmokeIsolation:\s*\{[\s\S]*identity:\s*FAR_SMOKE_ISOLATION_IDENTITY[\s\S]*requestedMode[\s\S]*effectiveMode[\s\S]*modeValue/, 'debug state reports far-smoke isolation requested/effective mode and numeric uniform value');
...
assert.match(core, /explosionPlumeSmokeDynamics[\s\S]*stemFocus[\s\S]*entrainment[\s\S]*capRoll[\s\S]*ageContrast/, 'debug state reports effective explosion plume dynamics gains');
assert.match(core, /farSmokeReceiver[\s\S]*range[\s\S]*overlap[\s\S]*carrier[\s\S]*rimCurl/, 'debug state reports effective far-smoke receiver gains');
...
assert.match(volumeWitness, /farSmokeTransport/, 'volume witness preserves far-smoke transport receipts in reports');
assert.match(volumeWitness, /farSmokeIsolation/, 'volume witness preserves far-smoke isolation receipts in reports');
assert.match(volumeWitness, /'far-smoke-isolation'/, 'volume witness exposes a focused far-smoke isolation evidence mode');
assert.match(volumeWitness, /expectsFarSmokeIsolationEvidence/, 'volume witness must name far-smoke isolation evidence mode explicitly');
assert.match(volumeWitness, /expectedFarSmokeIsolationVisibility\(expectedFarSmokeIsolationMode\)/, 'far-smoke isolation witness validates expected channel visibility');
assert.match(volumeWitness, /farSmokeIsolationEvidenceMode:\s*expectsFarSmokeIsolationEvidence \? 'far-smoke-channel-split-signal' : null/, 'far-smoke isolation witness report preserves the split evidence receipt');
assert.match(volumeWitness, /'far-smoke-readback'/, 'volume witness exposes a focused far-smoke readback evidence mode');
assert.match(volumeWitness, /expectsFarSmokeReadbackEvidence/, 'volume witness must name far-smoke readback evidence mode explicitly');
assert.match(volumeWitness, /const farSmokeGridReadback = sample\.simReadback\?\.farSmokeGridReadback/, 'far-smoke readback witness records the active far-grid scalar payload');
assert.match(volumeWitness, /farSmokeGridReadback\?\.identity,\s*'far-smoke-grid-readback-v0'/, 'far-smoke readback witness validates the active far-grid scalar payload identity');
assert.match(volumeWitness, /farSmokeGridReadback\?\.bufferReadbacks[\s\S]*length === 2/, 'far-smoke readback witness requires both ping-pong buffer readbacks');
assert.match(volumeWitness, /farSmokeSignalBuffers[\s\S]*far smoke grid readback found no nonzero storage-buffer signal/, 'far-smoke readback witness fails loud on all-zero storage-buffer evidence');
assert.match(volumeWitness, /farSmokeGridReadback\.smokeMax > 0\.002/, 'far-smoke readback witness rejects finite zero smoke evidence');
assert.match(volumeWitness, /farSmokeReadbackEvidenceMode:\s*'far-smoke-grid-buffer-signal'/, 'far-smoke readback witness report preserves the buffer evidence receipt');
...
assert.match(core, /uniforms\.set\(previousViewProj\.elements,\s*352\)/, 'previous view-projection matrix shifts after Bite-stack, flow, palette, topology-shell lab, boundary-fire, boundary sidecar reconstruction, sidecar display, scalar activity receiver, activity-tier, smoke artifact assay, explosion-plume, far-smoke receiver, far-smoke grid, far-smoke transport, and far-smoke isolation uniforms');
