...
assert.match(index, /fireReceiverVolumeEnergyTexture = new THREE\.CanvasTexture/, 'main renderer owns a reusable Three.js CanvasTexture for the native volume canvas');
assert.match(index, /sourceTexture\.image = sourceCanvas/, 'main renderer binds the native volume canvas into the reusable Three.js texture');
...
assert.match(index, /volume_scene_context/, 'URL route can request a truthful Three.js scene context behind the volume');
...
assert.match(index, /volume_receiver_light_isolate/, 'URL route can isolate the receiver-light layer for wall-light witness capture');
assert.match(index, /receiver-wall-wash-only-production-energy-black-scene-witness-v1/, 'URL route can isolate the production-energy wall-wash receiver light separately from the fire/contact receiver lobe');
...
assert.match(index, /gpuWallWashAuthority/, 'brick-wall scene context must expose GPU wall-wash authority');
...
assert.match(index, /rendered-volume-canvas-fire-energy-v21-visible-wide-gather-local-probe-no-readback-v0/, 'screen-space fire receiver light must disclose rendered-volume visible wide-gather local-probe texture fire-energy authority');
assert.match(index, /rendered-volume-canvas-visible-wide-gather-local-energy-route-gain-v27-bounded-positive-fire-energy-expansion/, 'screen-space fire receiver light must drive scalar intensity from route gain and bounded positive-fire expanded wide-gather rendered-volume texture energy, not the uploaded control proxy');
...
assert.match(index, /receiver-wide-gather-local-offset-fire-probe-v2/, 'screen-space fire receiver light must disclose the wide-gather receiver-local rendered-fire probe used when fixed UV samples are dark');
...
assert.match(index, /sampleCenter:\s*\[0\.61,\s*0\.61\]/, 'brick-wall rendered-fire receiver primary sample window must target the visible Pyro flame body');
assert.match(index, /sampleRadius:\s*0\.19/, 'brick-wall rendered-fire receiver blur must stay tight enough to avoid averaging mostly dark canvas around the flame');
assert.match(index, /sampleFootprints:\s*\[[\s\S]*\[0\.55,\s*0\.66\][\s\S]*\[0\.61,\s*0\.61\][\s\S]*\[0\.66,\s*0\.56\][\s\S]*\[0\.59,\s*0\.50\]/, 'brick-wall rendered-fire receiver must sample the visible Pyro flame body and upper tongue instead of the old high/side footprint');
assert.match(index, /rendered-fire-visible-multi-footprint-plus-wide-gather-receiver-local-screen-energy-v7/, 'screen-space fire receiver light must sample fixed source footprints and wide-gather receiver-local fire probes before projecting onto receiver surfaces');
assert.match(index, /receiver-surface-depth-soft-normal-mask-v3-split-contact-wall-wash-honest-production-mask/, 'screen-space fire receiver light must disclose the split contact/wall-wash production mask that keeps brick surfaces eligible for fire light');
assert.match(index, /receiver-layer-only-black-scene-witness-v0/, 'screen-space fire receiver light must disclose a receiver-only black-scene witness mode');
assert.match(index, /fireReceiverLightIsolateNode/, 'screen-space fire receiver light must route a GPU-side receiver-only isolation switch into the render pipeline');
assert.match(index, /receiver-isolate-suppresses-foreground-volume-canvas-v0/, 'receiver-only witness must disclose that it suppresses the foreground WebGPU volume canvas overlay');
assert.match(index, /#viewport\.volume-receiver-light-isolate-active #kaminos-volume-canvas\.active \{[^}]*opacity:\s*0;[^}]*\}/, 'receiver-only witness must hide the separate foreground native volume canvas so the visual artifact is actually receiver-only');
assert.match(index, /viewport\.classList\.toggle\('volume-receiver-light-isolate-active', group\.visible && receiverIsolateMode >= 0\.5\)/, 'receiver-only witness route must suppress the foreground canvas from the isolate request itself, including zero-light negative witnesses');
assert.match(index, /fireReceiverLightIsolateNode\.value = group\.visible \? receiverIsolateMode : 0/, 'receiver-only witness route must select the black-backed receiver output from the isolate request itself, including zero-light negative witnesses');
assert.match(index, /const receiverContactLobe = float\(1\.0\)\.sub\(smoothstep\(fireReceiverLightRadiusNode\.mul\(0\.18\), fireReceiverLightRadiusNode, receiverDistance\)\)/, 'screen-space fire receiver light must keep the compact contact lobe as one receiver component');
assert.match(index, /const receiverWallWashCenter = vec2\(0\.43,\s*0\.58\)/, 'screen-space fire receiver light must anchor the wall wash near the observed flame-wall contact instead of the asset center');
assert.match(index, /const receiverWallWashDelta = receiverUv\.sub\(receiverWallWashCenter\)\.mul\(vec2\(2\.45,\s*2\.0\)\)/, 'screen-space fire receiver light must use a tight anisotropic wall-wash distance so the throw spreads from the flame without becoming a wall-sized disk');
assert.match(index, /const receiverWallWashLobe = float\(1\.0\)\.sub\(smoothstep\(float\(0\.04\), float\(0\.27\), receiverWallWashDistance\)\)\.mul\(float\(1\.1\)\)/, 'screen-space fire receiver light must use a bounded lower-wall elliptical falloff instead of a broad asset-sized wash');
assert.doesNotMatch(index, /receiverWallWashBandX|receiverWallWashBandY/, 'screen-space fire receiver light must not keep the old broad rectangular wall-wash plateau');
assert.match(index, /const receiverLobe = receiverContactLobe\.add\(receiverWallWashLobe\)/, 'screen-space fire receiver light must combine the contact lobe with the bounded wall-wash lobe before depth and normal masking');
assert.match(index, /const receiverHasGeometry = smoothstep\(float\(0\.0005\), float\(0\.015\), receiverDepth\)/, 'screen-space fire receiver light must reject no-geometry background depth before drawing receiver-only witness light');
assert.match(index, /const receiverSurfaceMask = receiverVerticalFalloff\.mul\(receiverDepthMask\)\.mul\(receiverHasGeometry\)/, 'screen-space fire receiver light must gate both contact and wall wash to actual scene geometry');
assert.match(index, /const receiverContactMask = receiverContactLobe\.mul\(receiverSurfaceMask\)\.mul\(receiverSoftNormalMask\)/, 'screen-space fire receiver light must keep compact contact lighting under the ordinary soft-normal surface mask');
assert.match(index, /const receiverWallWashNormalMask = mix\(float\(0\.74\), float\(1\.0\), receiverNormalMask\)/, 'screen-space fire receiver light must keep broad brick wall wash eligible even when the packed-normal prepass is weak');
assert.match(index, /const receiverWallWashMask = receiverWallWashLobe\.mul\(receiverSurfaceMask\)\.mul\(receiverWallWashNormalMask\)/, 'screen-space fire receiver light must split wall wash from the compact contact lobe before foreground-volume attenuation');
assert.match(index, /const receiverWallWashVisibility = mix\(float\(0\.72\), float\(1\.0\), foregroundReceiverVisibility\)/, 'screen-space fire receiver light must not let foreground smoke erase the broad brick wall wash entirely');
assert.match(index, /const receiverLightMask = receiverContactMask\.mul\(foregroundReceiverVisibility\)[\s\S]{0,120}\.add\(receiverWallWashMask\.mul\(receiverWallWashVisibility\)\)/, 'screen-space fire receiver light must use an explicit production light mask combining contact and wall-wash visibility');
assert.match(index, /const receiverWallWashOnlyLight = vec3\(1\.0, 0\.36, 0\.07\)\.mul\(/, 'wall-wash-only witness must have a separate light term from the production receiver layer');
assert.match(index, /receiverWallWashMask\.mul\(receiverWallWashVisibility\)\.mul\(fireReceiverLightGainNode\)\.mul\(receiverEnergy\)/, 'wall-wash-only witness must use the same expanded rendered-fire receiver energy as the production receiver layer');
assert.match(index, /receiverWallWashMask\.mul\(receiverWallWashVisibility\)\.mul\(fireReceiverLightGainNode\)\.mul\(receiverEnergy\)\.mul\(float\(2\.35\)\)/, 'wall-wash-only witness must use the same bounded but operator-visible receiver multiplier as the production receiver layer');
assert.doesNotMatch(index, /receiverWallWashMask\.mul\(receiverWallWashVisibility\)\.mul\(fireReceiverLightGainNode\)\.mul\(fixedVolumeFireEnergy\)/, 'wall-wash-only witness must not use a separate fixed-fire-energy diagnostic path that can lie about production receiver light');
assert.match(index, /const volumeFireEnergyLowerLeft/, 'screen-space fire receiver light must include a lower-left source fire footprint');
assert.match(index, /const volumeFireEnergyLowerCenter/, 'screen-space fire receiver light must include a lower-center source fire footprint');
assert.match(index, /const volumeFireEnergyLowerRight/, 'screen-space fire receiver light must include a lower-right source fire footprint');
assert.match(index, /const volumeFireEnergyUpperTongue/, 'screen-space fire receiver light must include an upper flame-tongue source footprint');
assert.match(index, /smoothstep\(float\(0\.0\),\s*float\(0\.012\)/, 'rendered-fire scalar must use a low enough black point for the visible canvas fire sample to drive wall light');
assert.match(index, /const colorSignal = sampleColor\.r\.mul\(float\(0\.62\)\)\.add\(sampleColor\.g\.mul\(float\(0\.30\)\)\)/, 'rendered-fire scalar must preserve warm visible fire color instead of subtracting smoke into darkness');
assert.match(index, /const fixedVolumeFireEnergy = volumeFireEnergyLowerLeft/, 'rendered-fire scalar must retain fixed source footprints as a diagnostic/supporting signal');
assert.match(index, /const localVolumeFireEnergy = volumeEnergyFromSample\(receiverUv\.add\(vec2\(0\.0,\s*0\.10\)\)\)/, 'rendered-fire scalar must include a receiver-local offset fire probe so wall pixels can sample nearby visible flame');
assert.match(index, /volumeEnergyFromSample\(receiverUv\.add\(vec2\(-0\.085,\s*0\.20\)\)\)/, 'receiver-local rendered-fire scalar must sample below-left into the visible flame body');
assert.match(index, /volumeEnergyFromSample\(receiverUv\.add\(vec2\(0\.0,\s*-0\.10\)\)\)/, 'receiver-local rendered-fire scalar must mirror the vertical probe direction so texture-space orientation does not hide fire energy');
assert.match(index, /volumeEnergyFromSample\(receiverUv\.add\(vec2\(-0\.145,\s*0\.28\)\)\)/, 'receiver-local rendered-fire scalar must include a wider down-left probe for wall pixels beside the flame body');
assert.match(index, /volumeEnergyFromSample\(receiverUv\.add\(vec2\(-0\.145,\s*-0\.28\)\)\)/, 'receiver-local rendered-fire scalar must include a wider mirrored probe for inverted texture-space cases');
assert.match(index, /const wideLocalVolumeFireEnergy = volumeEnergyFromSample\(receiverUv\.add\(vec2\(-0\.20,\s*-0\.16\)\)\)/, 'receiver-local rendered-fire scalar must include a wider left/down gather into the observed flame body');
assert.match(index, /volumeEnergyFromSample\(receiverUv\.add\(vec2\(-0\.34,\s*-0\.08\)\)\)/, 'receiver-local rendered-fire scalar must reach far enough left for broad wall pixels beside the flame');
assert.match(index, /volumeEnergyFromSample\(receiverUv\.add\(vec2\(0\.18,\s*-0\.16\)\)\)/, 'receiver-local rendered-fire scalar must include a mirrored right/down gather for camera-angle variation');
assert.match(index, /const volumeFireEnergy = fixedVolumeFireEnergy\.mul\(float\(1\.35\)\)\.add\(localVolumeFireEnergy\.mul\(float\(1\.05\)\)\)\.add\(wideLocalVolumeFireEnergy\.mul\(float\(1\.95\)\)\)/, 'rendered-fire scalar must combine fixed footprints with the wide-gather receiver-local fire probe without a fallback floor');
assert.match(index, /smoothstep\(float\(0\.0\), float\(0\.012\), luminanceSignal\.add\(warmSignal\.mul\(0\.86\)\)\.add\(colorSignal\.mul\(0\.58\)\)\)/, 'rendered-fire scalar must use a sensitive positive-fire threshold after removing the envelope fallback');
assert.match(index, /const receiverEnergySignal = smoothstep\(float\(0\.0\), float\(0\.18\), volumeFireEnergy\)/, 'receiver-light production energy must expand positive rendered-fire energy without turning tiny samples into a white slab or adding fallback radiance');
assert.match(index, /const receiverEnergy = receiverEnergySignal\.mul\(float\(0\.32\)\)[\s\S]{0,160}\.add\(volumeFireEnergy\.mul\(float\(0\.035\)\)\)[\s\S]{0,160}\.add\(volumeFireEnergy\.mul\(volumeFireEnergy\)\.mul\(float\(0\.003\)\)\)/, 'receiver-light production energy must use a bounded positive-fire response instead of debug-era thousandfold amplification');
assert.doesNotMatch(index, /receiverEnergySignal\.mul\(float\(7\.20\)\)[\s\S]{0,160}\.add\(volumeFireEnergy\.mul\(float\(1600\.0\)\)\)[\s\S]{0,160}\.add\(volumeFireEnergy\.mul\(volumeFireEnergy\)\.mul\(float\(5200\.0\)\)\)/, 'receiver-light production energy must not keep the debug-era thousandfold amplification that saturates the receiver isolate');
assert.match(index, /receiverLightMask\.mul\(fireReceiverLightGainNode\)\.mul\(receiverEnergy\)\.mul\(float\(2\.35\)\)/, 'receiver-light production output must make the rendered-fire-only wall wash operator-visible without saturating the isolate');
assert.match(index, /const receiverProductionIsolate = receiverIsolateBand\(1\.0\)/, 'receiver-only witness mode 1 must isolate the production receiver-light layer');
assert.match(index, /const receiverWallWashIsolate = receiverIsolateBand\(2\.0\)/, 'receiver-only witness mode 2 must isolate wall wash separately from contact/fire silhouette lighting');
assert.match(index, /const receiverOnlyOutput = vec4\(receiverLight\.mul\(receiverProductionIsolate\)\.add\(receiverWallWashOnlyLight\.mul\(receiverWallWashIsolate\)\), 1\)/, 'receiver-only witness must output selected receiver layers without scene beauty or foreground fire');
assert.match(index, /renderPipeline\.outputNode = mix\(sceneOutput\.add\(vec4\(receiverLight, 0\)\), receiverOnlyOutput, receiverProductionIsolate\.add\(receiverWallWashIsolate\)\)/, 'receiver-only witness must switch the final pipeline to a black-backed selected receiver-light layer instead of composited scene beauty');
assert.match(index, /gainScale:\s*0\.78/, 'screen-space fire receiver light must expose enough route gain for visible wall wash after removing the fallback floor');
assert.match(index, /maxGain:\s*2\.8/, 'screen-space fire receiver light must cap route gain high enough for operator-visible wall wash without adding fallback radiance');
assert.match(index, /volumeFireEnergy\)\.mul\(receiverEnergyDebug\)\.mul\(float\(12\.0\)\)/, 'receiver energy debug must amplify sampled rendered-fire energy enough to expose scalar failures visually');
assert.match(index, /localVolumeFireEnergy\)\.mul\(receiverEnergyDebug\)\.mul\(float\(3\.8\)\)/, 'receiver energy debug must expose the wide receiver-local scalar separately from the final masked receiver-light composition');
assert.match(index, /receiver-local-probe-debug-v0/, 'receiver diagnostics must expose an unmasked local-probe debug mode for proving wall-adjacent fire gather');
assert.match(index, /wideLocalVolumeFireEnergy\.mul\(localProbeDebug\)\.mul\(float\(7\.5\)\)/, 'local-probe debug must draw the wide gathered fire energy without relying on the receiver mask');
assert.match(index, /volumeFireEnergyLowerLeft\s*=\s*volumeEnergyFootprint\(vec2\(0\.55,\s*0\.66\)/, 'screen-space fire receiver light implementation must use the recorded lower-left source fire footprint');
assert.match(index, /volumeFireEnergyLowerRight\s*=\s*volumeEnergyFootprint\(vec2\(0\.66,\s*0\.56\)/, 'screen-space fire receiver light implementation must use the recorded lower-right source fire footprint');
assert.match(index, /volumeFireEnergyUpperTongue\s*=\s*volumeEnergyFootprint\(vec2\(0\.59,\s*0\.50\)/, 'screen-space fire receiver light implementation must use the recorded upper flame-tongue source footprint');
assert.match(index, /sampleWindowRingFor\(vec2\(0\.55,\s*0\.66\)/, 'receiver sample-window debug must draw the same lower-left footprint used for energy');
assert.match(index, /sampleWindowRingFor\(vec2\(0\.66,\s*0\.56\)/, 'receiver sample-window debug must draw the same lower-right footprint used for energy');
assert.match(index, /sampleWindowRingFor\(vec2\(0\.59,\s*0\.50\)/, 'receiver sample-window debug must draw the same upper-tongue footprint used for energy');
assert.doesNotMatch(index, /volumeFireEnergyLowerLeft\s*=\s*volumeEnergyFootprint\(vec2\(0\.30,\s*0\.74\)/, 'screen-space fire receiver light must not keep the old stale low-basin lower-left footprint');
assert.doesNotMatch(index, /volumeFireEnergyLowerRight\s*=\s*volumeEnergyFootprint\(vec2\(0\.66,\s*0\.74\)/, 'screen-space fire receiver light must not keep the old stale low-basin lower-right footprint');
assert.doesNotMatch(index, /sampleFootprints:\s*\[[\s\S]*\[0\.44,\s*0\.72\][\s\S]*\[0\.50,\s*0\.72\][\s\S]*\[0\.56,\s*0\.72\][\s\S]*\[0\.51,\s*0\.66\]/, 'brick-wall rendered-fire receiver must not keep the pre-recenter sample footprint metadata');
assert.doesNotMatch(index, /const energyBase = receiverUv/, 'screen-space fire receiver light must not erase wall wash by sampling energy at the receiver pixel');
assert.doesNotMatch(index, /volumeFireEnergyDownwash/, 'source footprint sampling replaces the failed receiver-local downwash estimator');
assert.match(index, /fallbackEnergyRole:\s*'debug-metadata-only-not-radiance'/, 'screen-space fire receiver light must disclose that the old uploaded envelope is debug metadata only');
assert.match(index, /rendered-fire-energy-only-no-envelope-fallback-v0/, 'screen-space fire receiver light must disclose no-fallback rendered-fire-only receiver coupling');
assert.doesNotMatch(index, /uploaded-control-fire-envelope-no-readback-v0/, 'screen-space fire receiver light must not keep the old uploaded envelope as a receiver energy authority');
...
assert.doesNotMatch(index, /fireReceiverLightFallbackEnergyNode/, 'screen-space fire receiver light must not upload fallback energy into the receiver shader');
assert.doesNotMatch(index, /visibleFallbackFloorAuthority/, 'screen-space fire receiver light must not preserve a visible fallback floor authority');
assert.doesNotMatch(index, /fallbackFloorRole/, 'screen-space fire receiver light must not preserve a fallback floor role');
assert.doesNotMatch(index, /fallbackGuardEnergy/, 'screen-space fire receiver light must not keep a bounded fallback radiance term');
assert.doesNotMatch(index, /const receiverEnergy = volumeFireEnergy[\s\S]{0,180}fireReceiverLightFallbackEnergyNode/, 'receiver energy must not add any envelope fallback term to rendered fire energy');
...
assert.match(index, /receiver-energy-debug-v0/, 'screen-space fire receiver light must expose a debug mode for the shader-visible sampled fire energy');
assert.match(index, /receiver-sample-window-debug-v0/, 'screen-space fire receiver light must expose a debug mode for the fixed rendered-fire sample window');
assert.match(index, /receiver-raw-volume-texture-debug-v0/, 'screen-space fire receiver light must expose a debug mode for the raw TSL volume texture sample route');
assert.match(index, /const receiverDebugBand = \(center\) => \{[\s\S]{0,160}abs\(fireReceiverLightDebugNode\.sub\(float\(center\)\)\)[\s\S]{0,160}smoothstep\(float\(0\.06\), float\(0\.16\), delta\)/, 'receiver-light debug routes must use exact mode bands instead of cumulative smoothstep thresholds');
assert.match(index, /const receiverMaskDebug = receiverDebugBand\(1\.0\)/, 'receiver-light debug mode 1 must isolate the receiver production mask');
assert.match(index, /const receiverEnergyDebug = receiverDebugBand\(2\.0\)/, 'receiver-light debug mode 2 must isolate sampled receiver energy');
assert.match(index, /const sampleWindowDebug = receiverDebugBand\(3\.0\)/, 'receiver-light debug mode 3 must isolate the sample window overlay');
assert.match(index, /const rawTextureDebug = receiverDebugBand\(4\.0\)/, 'receiver-light debug mode 4 must isolate raw volume texture sampling');
assert.match(index, /const localProbeDebug = receiverDebugBand\(5\.0\)/, 'receiver-light debug mode 5 must isolate local probe sampling');
assert.doesNotMatch(index, /const receiverMaskDebug = smoothstep\(float\(0\.45\), float\(0\.55\), fireReceiverLightDebugNode\)/, 'receiver-light mask debug must not remain active for every higher-numbered debug mode');
assert.match(index, /foregroundVolumeColor\.mul\(rawTextureDebug\)/, 'receiver raw texture debug must draw the sampled native volume canvas texture directly');
assert.match(index, /routedVolumeReceiverLightDebug\s*=\s*clampVolumeValue\(routeReceiverLightDebug,\s*0,\s*5\)/, 'receiver-light debug route must allow local-probe debug mode 5 instead of clamping diagnostics to raw texture debug');
assert.match(index, /receiverDebugMode\s*=\s*clampVolumeValue\([^;]+,\s*0,\s*5\)/, 'receiver-light per-frame controls must not collapse local-probe debug mode 5 back to earlier debug routes');
assert.match(index, /receiver-surface-depth-soft-normal-mask-v3-split-contact-wall-wash-honest-production-mask/, 'screen-space fire receiver light must disclose its split contact and broad wall-wash receiver surface mask');
...
assert.doesNotMatch(index, /volume-cpu-fire-light-support-state-v0/, 'brick-wall route must not keep CPU support-state lighting around');
assert.doesNotMatch(index, /cpu-readback-three-light-fill-support-v0/, 'brick-wall route must not keep CPU readback fill authority around');
assert.doesNotMatch(index, /async-bounded-readback-support/, 'brick-wall route must not keep async readback support cadence around');
...
const brickWallContextStart = index.indexOf('function createVolumeBrickWallContext');
const brickWallContextEnd = index.indexOf('function createVolumeSceneContextRouter', brickWallContextStart);
assert.ok(brickWallContextStart >= 0 && brickWallContextEnd > brickWallContextStart, 'brick-wall scene context body is extractable for contract checks');
const brickWallContextSource = index.slice(brickWallContextStart, brickWallContextEnd);
assert.doesNotMatch(brickWallContextSource, /sampleFrame\s*\(/, 'brick-wall scene context must not call sampleFrame for lighting');
assert.doesNotMatch(brickWallContextSource, /fireLightReadbackSupport/, 'brick-wall scene context must not keep live fire-light readback support state');
assert.doesNotMatch(brickWallContextSource, /new THREE\.PointLight|flameKey|flameBounce|smokeWarmth/, 'brick-wall scene context must not keep a hidden Three light rig around');
assert.doesNotMatch(brickWallContextSource, /backwashMaterial|contact-wash/, 'brick-wall scene context must not keep the old hidden CPU/Three contact-wash plane around');
...
assert.match(core, /gpuWallWashVisibleEnergy/, 'fluid fragment shader converts live wall-wash energy into a non-debug visible contribution');
assert.match(core, /gpuWallWashBroadThrow/, 'fluid fragment shader carries a broad brick-face receiver so live wall-wash energy cannot vanish into a tiny lobe');
assert.match(core, /gpuWallWashDebugShape/, 'fluid fragment shader exposes a loud debug shape for brick-face receiver placement');
assert.match(core, /gpuWallWashColor/, 'fluid fragment shader applies a visible warm wall-wash color from live GPU fire energy');
assert.match(core, /gpuWallWashDebugColor/, 'fluid fragment shader keeps debug wall-wash color separate from visible wall radiance');
assert.match(core, /color\s*=\s*color\s*\+\s*gpuWallWashColor/, 'brick-wall GPU wash must contribute visible non-debug wall light before exposure');
assert.doesNotMatch(core, /gpuWallWashPostToneWallTint/, 'brick-wall GPU wash must not post-tone tint the volume/fire body');
assert.doesNotMatch(core, /gpuWallWashDirectWallThrow|gpuWallWashSceneProxySignal/, 'brick-wall GPU wash must not keep stale fragment-side wall throw math after moving the visible wash into scene space');
...
assert.match(core, /fragment-shader-live-fire-visible-wall-wash-v2/, 'fluid debug state reports the v2 broad brick-face fragment wash source');
assert.match(core, /gpu-meter-drives-broad-visible-fragment-wall-wash-v2/, 'fluid debug state reports that live GPU energy drives the broad visible wall wash');
assert.match(core, /cpuReadbackAuthority:\s*false/, 'GPU wall-wash debug state must not derive authority from CPU readback');
assert.match(core, /gpu-meter-drives-broad-visible-fragment-wall-wash-v2/, 'GPU wall-wash debug state must disclose the visible broad fragment wall-wash proxy role');
...
assert.match(core, /uniforms\[201\]\s*=\s*pyroRadianceGate/, 'CPU uploads Pyro radiance gate into the lifecycle-shifted WGSL uniform block');
assert.match(core, /uniforms\[202\]\s*=\s*pyroRadianceSpill/, 'CPU uploads Pyro radiance spill into the lifecycle-shifted WGSL uniform block');
assert.match(core, /uniforms\[203\]\s*=\s*pyroRadianceWarmth/, 'CPU uploads Pyro radiance warmth into the lifecycle-shifted WGSL uniform block');
...
assert.match(core, /uniforms\[204\]\s*=\s*pyroBiteHeat/, 'CPU uploads Bite heat into the lifecycle-shifted Pyro color uniform block');
assert.match(core, /uniforms\[205\]\s*=\s*pyroBiteChroma/, 'CPU uploads Bite chroma into the lifecycle-shifted Pyro color uniform block');
assert.match(core, /uniforms\[206\]\s*=\s*pyroRadianceHue/, 'CPU uploads Radiance hue into the lifecycle-shifted Pyro color uniform block');
assert.match(core, /uniforms\[207\]\s*=\s*pyroRadianceChroma/, 'CPU uploads Radiance chroma into the lifecycle-shifted Pyro color uniform block');
...
assert.match(witness, /liveCpuReadback/, 'witness verifies brick-wall lighting has no live CPU readback support');
...
assert.match(witness, /receiver-light-isolate/, 'witness exposes a dedicated receiver-light isolate evidence mode');
assert.match(witness, /receiver-isolate route must show receiver light when fire light is enabled/, 'witness rejects a blank receiver-isolate positive light artifact');
assert.match(witness, /receiver-isolate zero-light route must stay dark/, 'witness rejects stale receiver light in zero-light isolate artifacts');
...
assert.match(witness, /rendered-fire-energy-only-no-envelope-fallback-v0/, 'witness verifies no-fallback receiver-light coupling');
...
assert.match(index, /sceneContextBridgeBypass:\s*volumeSceneContext\?\.debugState\?\.\(\)\?\.visible === true/, 'scene-context routes must disclose the direct native-canvas composition path instead of claiming a unified bridge');
assert.match(index, /mesh\.visible = active && !sceneContextActive/, 'scene-context composition bypasses the fragile Three CanvasTexture bridge and uses the native volume canvas as the screen-blended volume layer');
...
assert.match(index, /mainRendererNeeded[\s\S]*volumeSceneContextActive/, 'scene-context routes force the main renderer to render the backdrop and receiver light beneath the native volume canvas');
...
assert.match(witness, /operator viewport screenshot missing direct-overlay fire volume/, 'scene-context witness must fail if the raw simulator has fire but the operator-visible viewport lost the foreground volume');
