...
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*density:\s*6\.00[\s\S]*fire:\s*3\.50[\s\S]*radiance:\s*0\.00[\s\S]*glow:\s*0\.00/, 'Hostile multi-flame basin preserves the captured transport and no-radiance/no-glow state');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*raySteps:\s*96[\s\S]*adaptiveRays:\s*0\.65[\s\S]*majorantSkip:\s*0\.00[\s\S]*temporalAccum:\s*0\.00[\s\S]*temporalJitter:\s*0\.00[\s\S]*historyClamp:\s*0\.00[\s\S]*renderScale:\s*0\.35[\s\S]*resolution:\s*160[\s\S]*majorantGrid:\s*48/, 'Hostile multi-flame basin preserves the effective 160^3 debug-state grid rather than the stale 96 URL');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*oracleActivityCurlNoise:\s*0\.00[\s\S]*oracleActivityVorticity:\s*3\.00[\s\S]*oracleActivityMaterial:\s*0\.00/, 'Hostile multi-flame basin preserves the force-transfer state that made the basin interesting');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*pressureMode:\s*'activity-tiers'[\s\S]*activityPressureMaxTier:\s*4[\s\S]*activityPressureDispatchStrategy:\s*'coarse'[\s\S]*activityPressureShadowOverhead:\s*false[\s\S]*gpuTiming:\s*true[\s\S]*activityVorticityGate:\s*0\.00[\s\S]*activityDetailGate:\s*0\.00/, 'Hostile multi-flame basin preserves Activity P4 coarse pressure identity and disabled activity gates');
