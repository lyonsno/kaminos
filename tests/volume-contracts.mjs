...
assert.match(index, /id="volume-add-fire"/, 'Volume tab exposes an Add Fire primitive tool');
assert.match(index, /id="volume-add-smoke"/, 'Volume tab exposes an Add Smoke primitive tool');
...
assert.match(index, /id="volume-source-type-select"/, 'Volume tab exposes selected primitive source type editing');
...
assert.match(index, /volume-source-type-taxonomy-v0/, 'Volume tab carries a stable source type taxonomy identity');
assert.match(index, /VOLUME_SOURCE_TYPES/, 'source type taxonomy is declared as a first-class contract');
...
assert.match(core, /sourceTypeHardFireGain/, 'primitive source records hard-zero fire gain when the source type cannot emit fire');
assert.match(core, /sourceTypeHardSmokeGain/, 'primitive source records hard-zero smoke gain when the source type cannot emit smoke');
...
assert.match(core, /paramsOffset \+ 2\]\s*=\s*source\?\.fireSourceMix/, 'primitive source uniforms carry fire source mix');
assert.match(core, /paramsOffset \+ 3\]\s*=\s*source\?\.smokeSourceMix/, 'primitive source uniforms carry smoke source mix');
...
assert.match(core, /volume-primitive-scene-bounds-source-domain-v1/, 'renderer names the scene-bounds source domain that keeps scene-scale primitives moving past the old native wall');
assert.match(core, /VOLUME_PRIMITIVE_SOURCE_SCENE_BOUNDS/, 'renderer source projection uses the shared authored scene movement bounds');
...
assert.match(core, /mapSceneCoordinateToNativeSource/, 'renderer maps scene coordinates continuously instead of clamping directly into the native source cube');
assert.doesNotMatch(core, /clampFinite\(source\[0\]/, 'primitive source x must not hard-clamp at the old native wall');
assert.match(core, /nativeToScene/, 'fluid simulation maps native grid cells into shared scene-space source coordinates');
assert.match(core, /sceneToCell/, 'raymarch sampling maps shared scene-space rays back into the fluid grid');
assert.match(core, /sceneBoxHit/, 'volume raymarch intersects the shared scene bounds instead of the old native cube');
assert.match(core, /Math\.max\(0\.32,\s*primitive\.simulation\.sourceRadius \* 2\.0\)/, 'scene-domain primitive sources keep enough shader radius support on low witness grids');
...
assert.match(core, /primitiveStateSignature/, 'renderer guards unchanged primitive syncs from rebuilding and resetting the fluid sim');
assert.match(core, /device && primitivesChanged/, 'renderer rebuilds fluid state only when primitive state materially changes');
...
assert.match(core, /let sourceCenter = sceneP - sourceVector\.xyz;/, 'fluid source is centered from each authored primitive transform in scene space');
...
assert.match(witness, /volume_smoke_move_channel_probe/, 'witness can sample immediately after moving a Smoke primitive and reject fire-channel flash');
...
assert.match(witness, /sourceTypeTaxonomy/, 'witness verifies source type taxonomy evidence');
assert.match(witness, /fireSourceMix/, 'witness asserts renderer fire source mix evidence');
assert.match(witness, /smokeSourceMix/, 'witness asserts renderer smoke source mix evidence');
...
assert.match(witness, /failureMainRendererScreenshot/, 'witness failure reports preserve a main-renderer screenshot when available');
