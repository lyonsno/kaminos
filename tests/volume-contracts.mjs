...
assert.match(index, /volumePrototype\?\.receiverLightAttachments\?\.\(\)/, 'Tier 2 receiver light requests native attachments from the live volume producer');
...
assert.match(volumeWitness, /nativeAttachmentAccepted/, 'witness reports whether native Tier 2 receiver attachments are live rather than stale');
assert.match(volumeWitness, /stale native receiver-light attachments/, 'witness fails loudly when native Tier 2 receiver attachments do not advance between reads');
assert.match(volumeWitness, /attachmentIdentity !== 'gpu-splat-radiance-coverage-depth-moments-v0'/, 'witness fails loudly when Tier 2 routes do not request native splat attachments');
assert.match(volumeWitness, /effectiveAttachmentIdentity !== 'gpu-splat-radiance-coverage-depth-moments-v0'/, 'witness fails loudly when Tier 2 routes silently lose native splat attachment authority');
assert.match(volumeWitness, /attachmentAuthority !== 'native-shared-device-gpu-texture-only-v0'/, 'witness rejects CPU, canvas, or procedural receiver-light authority');
...
assert.match(index, /source:\s*'native-webgpu-volume-canvas'/, 'main renderer bridge names the native WebGPU volume canvas source');
assert.match(index, /composition:\s*'dom-webgpu-canvas-no-copy'/, 'main renderer bridge avoids a stale CanvasTexture copy path');
...
assert.match(index, /volume-core\.js\?v=native-splat-receiver-clean-0714b/, 'volume prototype import carries the native splat receiver route cache key');
