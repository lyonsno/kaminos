...
assert.match(index, /exploding_yellow_fireball_0706/, 'Volume tab pins the operator-found exploding yellow fireball Pyro look by stable route identity');
...
assert.match(index, /exploding_yellow_fireball_0706:[\s\S]*fire:\s*0\.00/, 'Exploding yellow fireball basin keeps Pyro-owned visible flame authority');
assert.match(index, /exploding_yellow_fireball_0706:[\s\S]*pyroMaterialGain:\s*1\.50/, 'Exploding yellow fireball basin preserves the operator-tuned high material gain');
assert.match(index, /exploding_yellow_fireball_0706:[\s\S]*pyroBiteHotColor:\s*'#ff4000'/, 'Exploding yellow fireball basin preserves the operator-tuned bite hot color');
assert.match(index, /exploding_yellow_fireball_0706:[\s\S]*pyroRadianceCoolColor:\s*'#ff6b6b'/, 'Exploding yellow fireball basin preserves the operator-tuned radiance cool color');
assert.match(index, /exploding_yellow_fireball_0706:[\s\S]*pyroFireMode:\s*'pyro-owned'/, 'Exploding yellow fireball basin preserves Pyro-owned flame display mode');
assert.match(index, /exploding_yellow_fireball_0706:[\s\S]*pressureMode:\s*'global-p3'/, 'Exploding yellow fireball basin preserves the Full P3 pressure solve for training pickup');
...
assert.match(index, /exploding-yellow-fireball/, 'Source-backed look library exposes the operator-saved exploding yellow Pyro look');
...
assert.match(index, /highProjectedGpu/, 'Volume tab can select GPU-native hidden high-source scalar activity cue projection');
assert.match(index, /id="volume-oracle-activity-source-grid"/, 'Volume tab exposes a hidden scalar activity source grid selector for highProjectedGpu');
assert.match(index, /volume_oracle_activity_source_grid/, 'Basin routes preserve hidden scalar activity source grid identity');
...
assert.match(core, /csProjectHighProjectedScalarActivityCue/, 'WGSL exposes a GPU-native high-source-to-low-receiver scalar cue projection pass');
...
assert.match(core, /hiddenScalarActivitySourceState/, 'volume core owns a live hidden source sim state for highProjectedGpu instead of reusing a static cue');
assert.match(core, /GPU_HIGH_PROJECTED_ACTIVITY_CUE_AUTHORITY/, 'volume core records highProjectedGpu source cue authority separately from lowSelfGpu');
assert.match(core, /GPU_HIGH_PROJECTED_SCALAR_ACTIVITY_CUE_PROJECTION_IDENTITY/, 'volume core records highProjectedGpu projection identity separately from same-grid lowSelfGpu');
assert.match(core, /readbackPolicy:\s*GPU_SCALAR_ACTIVITY_CUE_PROJECTION_READBACK_POLICY/, 'GPU scalar cue projection reports the no-readback policy for live cue paths');
assert.match(core, /sourceGrid:\s*hiddenScalarActivitySourceState\.gridSize/, 'highProjectedGpu projection debug records the effective hidden source grid');
assert.match(core, /receiverGrid:\s*gridSize/, 'highProjectedGpu projection debug records the effective visible receiver grid');
...
assert.match(witness, /exploding_yellow_fireball_0706/, 'witness recognizes the 2026-07-06 operator-found exploding yellow fireball Pyro basin');
...
assert.match(rgbAlphaCleanup, /--truth-role/, 'RGB alpha cleanup can target derived manifests with custom truth role names');
assert.match(rgbAlphaCleanup, /--low-role/, 'RGB alpha cleanup can target derived manifests with custom low/baseline role names');
assert.match(rgbAlphaCleanup, /outputs\.get\(role\)/, 'RGB alpha cleanup reads object-style role output manifests as well as list-style outputs');
...
assert.match(rgbPollutionLift, /--truth-role/, 'RGB pollution-vs-lift diagnostic can target derived manifests with custom truth role names');
assert.match(rgbPollutionLift, /--low-role/, 'RGB pollution-vs-lift diagnostic can target derived manifests with custom low/baseline role names');
assert.match(rgbPollutionLift, /outputs\.get\(role\)/, 'RGB pollution-vs-lift diagnostic reads object-style role output manifests as well as list-style outputs');
...
assert.match(rgbPollutionLift, /outsideNoiseMetrics/, 'RGB pollution-vs-lift diagnostic reports outside weak-support noise removal metrics');
assert.match(rgbPollutionLift, /outsideColumnNoiseMetrics/, 'RGB pollution-vs-lift diagnostic reports weak-support cleanup outside the truth-or-low diagnostic column');
assert.match(rgbPollutionLift, /structurePreservationMetrics/, 'RGB pollution-vs-lift diagnostic reports high-norm body preservation metrics');
assert.match(rgbPollutionLift, /bodyNormRetentionRate/, 'RGB pollution-vs-lift diagnostic measures cleanup norm retention on the predicted high-norm body');
assert.match(rgbPollutionLift, /outsideWeakSupportReductionRate/, 'RGB pollution-vs-lift diagnostic measures broad weak-support reduction outside the truth body');
...
const rgbConservativeFirewallPath = join(root, 'volume-render-rgb-conservative-firewall.py');
assert.ok(existsSync(rgbConservativeFirewallPath), 'volume render RGB conservative firewall diagnostic exists');
const rgbConservativeFirewall = existsSync(rgbConservativeFirewallPath) ? readFileSync(rgbConservativeFirewallPath, 'utf8') : '';
assert.match(rgbConservativeFirewall, /kaminos\.volume\.render-rgb-conservative-firewall\.v0/, 'RGB conservative firewall writes a stable manifest schema');
assert.match(rgbConservativeFirewall, /render-space-conservative-firewall-v0/, 'RGB conservative firewall names its evidence identity');
assert.match(rgbConservativeFirewall, /--truth-role/, 'RGB conservative firewall can target derived manifests with custom truth role names');
assert.match(rgbConservativeFirewall, /--low-role/, 'RGB conservative firewall can target derived manifests with custom low/baseline role names');
assert.match(rgbConservativeFirewall, /outputs\.get\(role\)/, 'RGB conservative firewall reads object-style role output manifests as well as list-style outputs');
assert.match(rgbConservativeFirewall, /protected-fire-mask-from-low-pred-energy-v0/, 'RGB conservative firewall derives protected fire mask without truth support at application time');
assert.match(rgbConservativeFirewall, /protectedFireMask/, 'RGB conservative firewall writes a protected fire mask visual');
assert.match(rgbConservativeFirewall, /editableRegionMap/, 'RGB conservative firewall writes an editable region visual');
assert.match(rgbConservativeFirewall, /conservativeFirewall/, 'RGB conservative firewall writes the protected-fire composition');
assert.match(rgbConservativeFirewall, /firewallMetrics/, 'RGB conservative firewall reports protected/editable/firewall metric aggregates');
...
assert.match(volumeWitness, /hiddenScalarActivitySource:\s*sample\.hiddenScalarActivitySource \|\| state\.hiddenScalarActivitySource/, 'volume witness report preserves hidden high-source sim identity for highProjectedGpu');
