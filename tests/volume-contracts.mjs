...
const bakedOnlyBranch = boundaryFireBranch[0].match(/if \(\(boundarySidecarSource > 0\.5 && boundarySidecarSource <= 1\.5\) \|\| boundarySidecarSource > 2\.5\) \{([\s\S]*?)\} else \{/);
assert.ok(bakedOnlyBranch, 'Baked/learned sidecar modes share the sidecar-only branch distinct from live/mix');
