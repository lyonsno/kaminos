...
assert.match(index, /volume-core\.js\?v=flame-life-pressure-label-0628/, 'volume prototype import carries a cache key when tall-plume flame survival or pressure-label semantics change');
...
assert.match(index, /id="volume-flame-life"/, 'Volume tab exposes flame survival/life control independent from source fuel');
assert.match(index, /<span class="slider-label">Flame Life<\/span>\s*<input type="range" id="volume-flame-life"/, 'Flame Life is labeled as a fire survival control rather than a fuel injector');
assert.match(index, /volume_flame_life/, 'URL route can override flame survival without changing fuel injection or smoke height');
assert.match(index, /flameLife/, 'Volume controls carry flame survival into the renderer');
