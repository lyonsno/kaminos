...
assert.match(witness, /if \(\s*!expectsCanonicalPlumeProof\s*&&\s*!expectsFuelStarvedTallPlume\s*&&\s*!expectsNoFireVolumeEvidence\s*&&\s*!expectsReceiverSupportEvidence\s*&&\s*\(!Number\.isFinite\(sample\.simReadback\.fireLayerMean\) \|\| sample\.simReadback\.fireLayerMean <= 0\.0005\)\s*&&\s*!acceptsRawCarrierPyroPaint\s*\)/, 'no-fire and receiver-support evidence routes do not require positive transported fire-layer readback');
assert.match(witness, /!expectsFuelStarvedTallPlume\s*&&\s*!expectsNoFireVolumeEvidence\s*&&\s*!expectsReceiverSupportEvidence[\s\S]*radianceMean/, 'no-fire and receiver-support evidence routes do not require positive fire radiance readback');
assert.match(witness, /else if \(!expectsReceiverSupportEvidence && \([\s\S]*fireFuelOverlapRatio <= 0\.01[\s\S]*\)\) \{/, 'receiver-support evidence routes do not require the legacy tall-plume fire/fuel overlap beauty threshold');
