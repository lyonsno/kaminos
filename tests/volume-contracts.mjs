...
assert.match(index, /kaminosVolumeRouteInitPromise\s*=\s*initKaminosVolumeRoute\(\)[\s\S]*kaminosVolumeRouteInitPromise\.catch/, 'Volume route initializes from the scene-init fallback path for setup diagnostics');
...
assert.match(core, /uniforms\.set\(previousViewProj\.elements,\s*364\)/, 'previous view-projection matrix follows shared presentation, Boundary Fire palette, reserved source extension, and artistic-motion uniforms');
