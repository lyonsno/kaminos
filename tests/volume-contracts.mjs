...
const volumeShellLabWitness = readFileSync(join(root, 'volume-shell-lab-witness.mjs'), 'utf8');
...
const hostileMultiflameCapturePath = join(root, 'fixtures/volume/hostile-multiflame-pressure-force-0711.capture.json');
assert.ok(existsSync(hostileMultiflameCapturePath), 'hostile multi-flame emergency capture fixture exists');
const hostileMultiflameCapture = readFileSync(hostileMultiflameCapturePath, 'utf8');
const rgbUpscaleBasinCapturePath = join(root, 'fixtures/volume/rgb-upscale-basin-0711-c4d3f040.capture.json');
assert.ok(existsSync(rgbUpscaleBasinCapturePath), 'RGB upscale basin live capture fixture exists');
const rgbUpscaleBasinCapture = readFileSync(rgbUpscaleBasinCapturePath, 'utf8');
const rgbUpscaleBasinPngPath = join(root, 'fixtures/volume/rgb-upscale-basin-0711-c4d3f040.png');
assert.ok(existsSync(rgbUpscaleBasinPngPath), 'RGB upscale basin extracted canvas PNG fixture exists');
const rgbUpscaleBasinWitnessPngPath = join(root, 'fixtures/volume/rgb-upscale-basin-0711-c4d3f040.replay-witness.png');
assert.ok(existsSync(rgbUpscaleBasinWitnessPngPath), 'RGB upscale basin replay witness PNG fixture exists for training pickup');
...
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*label:\s*'hostile multiflame pressure force 0711'/, 'Hostile multi-flame pressure/force basin has a stable named route preset');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*sourceCaptureIdentity:\s*'kaminos-volume-basin-emergency-live-dom-debug-capture-v1'[\s\S]*sourceCaptureHash:\s*'3518d1ef'[\s\S]*sourceCapturedAt:\s*'2026-07-11T11:44:47\.198Z'/, 'Hostile multi-flame basin preserves the emergency live full-control capture identity');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*density:\s*2\.6[\s\S]*fire:\s*0[\s\S]*radiance:\s*0[\s\S]*absorption:\s*0\.75[\s\S]*glow:\s*0[\s\S]*smoke:\s*2\.8/, 'Hostile multi-flame basin preserves the live captured transport state, not the stale compact URL crumbs');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*fireScale:\s*0\.96[\s\S]*plumeHeight:\s*2\.1[\s\S]*windStrength:\s*1\.5[\s\S]*windAngle:\s*-65[\s\S]*windHeight:\s*-0\.8[\s\S]*inputRadius:\s*0\.53[\s\S]*flowRate:\s*1\.15/, 'Hostile multi-flame basin preserves the live broad source/flow family used to find the basin');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*raySteps:\s*104[\s\S]*adaptiveRays:\s*1[\s\S]*occupancySkip:\s*1[\s\S]*majorantSkip:\s*0[\s\S]*temporalAccum:\s*0[\s\S]*temporalJitter:\s*0[\s\S]*historyClamp:\s*0[\s\S]*renderScale:\s*0\.7[\s\S]*resolution:\s*160[\s\S]*majorantGrid:\s*48/, 'Hostile multi-flame basin preserves the emergency live ray/render/sim budget');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*oracleActivityCurlNoise:\s*0[\s\S]*oracleActivityVorticity:\s*3[\s\S]*oracleActivityMaterial:\s*0/, 'Hostile multi-flame basin preserves the force-transfer state that made the basin interesting');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*reactionLiveView:\s*'boundary_fire'[\s\S]*boundarySidecarSource:\s*'baked'[\s\S]*boundarySidecarView:\s*'off'/, 'Hostile multi-flame basin preserves the live captured boundary-fire view state');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*reactionBoundaryFireRidge:\s*1\.08[\s\S]*reactionBoundaryFireRidgeCut:\s*0[\s\S]*reactionBoundaryFireLuma:\s*5[\s\S]*boundarySidecarBlur:\s*0[\s\S]*boundarySidecarWidth:\s*0[\s\S]*boundarySidecarRidge:\s*0\.26/, 'Hostile multi-flame basin preserves the live captured boundary/ridge cut state');
assert.match(index, /hostile_multiflame_pressure_force_0711:[\s\S]*pressureMode:\s*'activity-tiers'[\s\S]*pressureStrategy:\s*'activity_tiers'[\s\S]*activityPressureMaxTier:\s*4[\s\S]*activityPressureDispatchStrategy:\s*'coarse-brick-activity-pressure-mask-v0'[\s\S]*activityPressureShadowOverhead:\s*false[\s\S]*gpuTiming:\s*true[\s\S]*activityVorticityGate:\s*0[\s\S]*activityDetailGate:\s*0/, 'Hostile multi-flame basin preserves Activity P4 coarse pressure identity and disabled activity gates');
assert.match(hostileMultiflameCapture, /"identity": "kaminos-volume-basin-emergency-live-dom-debug-capture-v1"/, 'Hostile multi-flame raw emergency capture fixture preserves source identity');
assert.match(hostileMultiflameCapture, /"hash": "3518d1ef"/, 'Hostile multi-flame raw emergency capture fixture preserves source hash');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*label:\s*'rgb upscale basin 0711'/, 'RGB upscale basin has a stable named route preset');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*sourceCaptureIdentity:\s*'kaminos-rgb-upscale-basin-live-capture-v1'[\s\S]*sourceCaptureHash:\s*'c4d3f040'[\s\S]*sourceCapturedAt:\s*'2026-07-11T18:05:39\.977Z'/, 'RGB upscale basin preserves the live capture identity');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*sourceCaptureSha256:\s*'cc5cf502cf3720c8b9d31555f66770d3febb48e279d1c2f12cc194c3b21790d1'/, 'RGB upscale basin preserves the raw capture checksum');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*sourceCanvasFixture:\s*'fixtures\/volume\/rgb-upscale-basin-0711-c4d3f040\.png'[\s\S]*sourceCanvasCaveat:\s*'blank-webgpu-drawing-buffer-capture'[\s\S]*sourceTrainingTarget:\s*'visual-rgb-upscaling'/, 'RGB upscale basin preserves the embedded canvas capture while naming its blank-buffer caveat');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*sourceReplayWitnessFixture:\s*'fixtures\/volume\/rgb-upscale-basin-0711-c4d3f040\.replay-witness\.png'/, 'RGB upscale basin names the replay witness PNG as the training visual target');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*density:\s*6[\s\S]*fire:\s*0[\s\S]*radiance:\s*0[\s\S]*absorption:\s*0\.75[\s\S]*glow:\s*0[\s\S]*smoke:\s*2\.8[\s\S]*curl:\s*4/, 'RGB upscale basin preserves the live transport and curl state');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*raySteps:\s*88[\s\S]*adaptiveRays:\s*1[\s\S]*occupancySkip:\s*1[\s\S]*majorantSkip:\s*0[\s\S]*temporalAccum:\s*0[\s\S]*temporalJitter:\s*0[\s\S]*historyClamp:\s*0[\s\S]*renderScale:\s*0\.3[\s\S]*resolution:\s*128[\s\S]*majorantGrid:\s*24/, 'RGB upscale basin preserves the live ray/render/sim budget');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*pressureMode:\s*'global-p3'[\s\S]*gpuTiming:\s*true[\s\S]*pressureIterations:\s*3[\s\S]*pressureStrategy:\s*'global'/, 'RGB upscale basin preserves the Full P3 pressure identity used for capture');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*oracleActivityCue:\s*1[\s\S]*oracleActivityCurlNoise:\s*0\.4[\s\S]*oracleActivityVorticity:\s*3[\s\S]*oracleActivityMaterial:\s*0/, 'RGB upscale basin preserves the operator force-transfer state');
assert.match(index, /rgb_upscale_basin_0711:[\s\S]*reactionLiveView:\s*'boundary_fire'[\s\S]*boundarySidecarSource:\s*'baked'[\s\S]*boundarySidecarView:\s*'off'/, 'RGB upscale basin preserves the boundary-fire/baked-sidecar route state');
assert.match(volumeWitness, /rgb_upscale_basin_0711:[\s\S]*sourceCaptureHash:\s*'c4d3f040'[\s\S]*pressureMode:\s*'global-p3'/, 'witness knows the RGB upscale basin preset and pressure identity');
assert.match(rgbUpscaleBasinCapture, /"identity": "kaminos-rgb-upscale-basin-live-capture-v1"/, 'RGB upscale raw capture fixture preserves source identity');
assert.match(rgbUpscaleBasinCapture, /"hash": "c4d3f040"/, 'RGB upscale raw capture fixture preserves source hash');
const applyTallPlumeOperatorPresetFn = index.match(/function applyTallPlumeOperatorPreset\(name, options = \{\}\) \{([\s\S]*?)\n\}/)?.[1] || '';
assert.match(applyTallPlumeOperatorPresetFn, /applyVolumeControlsSnapshot\(preset,\s*\{\s*keepBudgetPreset:\s*true\s*\}\)/, 'Tall-plume presets hydrate through the same full basin snapshot path as saved basins');
const applyVolumeControlsSnapshotFn = index.match(/function applyVolumeControlsSnapshot\(controlsSnapshot = \{\}, options = \{\}\) \{([\s\S]*?)\n\}/)?.[1] || '';
assert.match(index, /function hydrateVolumeRouteOwnedControls\(controlsSnapshot = \{\}\)/, 'Volume basin snapshot hydration has one route-owned state helper instead of preset-only one-offs');
assert.match(index, /routedVolumeActivityPressureMaxTier\s*=\s*normalizeVolumeActivityPressureMaxTier\(controlsSnapshot\.activityPressureMaxTier/, 'Shared basin hydration restores route-owned Activity pressure max-tier state');
assert.match(index, /routedVolumeActivityPressureDispatchStrategy\s*=\s*normalizeVolumeActivityPressureDispatchStrategy\(controlsSnapshot\.activityPressureDispatchStrategy/, 'Shared basin hydration restores route-owned Activity pressure dispatch state');
assert.match(applyVolumeControlsSnapshotFn, /hydrateVolumeRouteOwnedControls\(controlsSnapshot\)/, 'Saved basin snapshots restore route-owned state such as Activity pressure dispatch and GPU timing');
assert.match(index, /function captureVolumeBasinCompact\(label = ''\)/, 'Volume bench exposes a compact full-control basin capture for durable root presets');
assert.match(index, /window\.__kaminosCaptureVolumeBasinCompact = captureVolumeBasinCompact/, 'Compact basin capture is available from the browser console');
assert.match(index, /controls:\s*controlsSnapshot[\s\S]*replayUrl:\s*buildVolumeBasinUrl\(controlsSnapshot\)/, 'Compact basin capture carries full readVolumeControls state and a replay URL instead of controls:null');
...
assert.match(index, /volume_gpu_timing/, 'URL route can opt into GPU timestamp timing telemetry');
assert.match(index, /id="volume-gpu-timing-state"/, 'Volume readout exposes GPU timing support/fallback state');
assert.match(index, /id="volume-gpu-frame-ms"/, 'Volume readout exposes timestamp-derived GPU frame timing when available');
assert.match(index, /id="volume-gpu-pressure-ms"/, 'Volume readout exposes timestamp-derived sim+pressure timing when available');
assert.match(index, /id="volume-gpu-pressure-p95"/, 'Volume readout exposes rolling GPU pressure p95 timing');
assert.match(index, /id="volume-gpu-samples"/, 'Volume readout exposes GPU timestamp sample count');
assert.match(core, /GPU_TIMING_IDENTITY\s*=\s*'webgpu-timestamp-query-phase-timing-v0'/, 'GPU timing telemetry has a stable identity distinct from RAF/queue proxies');
assert.match(core, /GPU_TIMING_UNSUPPORTED_REASON\s*=\s*'timestamp-query-feature-unavailable'/, 'GPU timing telemetry reports an explicit unsupported feature reason');
assert.match(core, /GPU_TIMING_PHASE_SAMPLE_KEYS/, 'GPU timing telemetry declares phase keys for rolling sample stats');
assert.match(core, /recordGpuTimingSamples/, 'GPU timing telemetry records rolling phase samples');
assert.match(core, /gpuTimingStats/, 'debug state preserves rolling GPU timing stats');
assert.match(core, /adapter\.features\?\.has\('timestamp-query'\)/, 'GPU timing telemetry only requests timestamp-query when the adapter exposes it');
assert.match(core, /deviceDescriptor\.requiredFeatures\s*=\s*\['timestamp-query'\]/, 'WebGPU device request includes timestamp-query only for the opt-in supported timing route');
assert.match(core, /createQuerySet\(\{[\s\S]*type:\s*'timestamp'/, 'GPU timing telemetry creates a timestamp query set');
assert.match(core, /resolveQuerySet/, 'GPU timing telemetry resolves timestamp queries into a readback buffer');
assert.match(core, /ensureGpuTimingQueriesWritten/, 'GPU timing telemetry writes skipped phase queries before resolving the full query set');
assert.match(core, /gpuPhaseTimings/, 'debug state preserves timestamp-derived phase timings');
assert.match(core, /gpuTimingReadbackPending/, 'debug state reports whether timestamp readback is pending');
assert.match(volumeWitness, /gpuTiming/, 'witness report preserves GPU timing state when present');
assert.match(volumeWitness, /gpuPhaseTimings/, 'witness report preserves timestamp-derived GPU phase timing details');
assert.match(volumeWitness, /gpuTimingStats/, 'witness report preserves rolling GPU timing stats');
...
assert.match(core, /ACTIVITY_PRESSURE_P4_DENSE_STRATEGY\s*=\s*'core-replay-p4-dense-activity-mask-v0'/, 'activity dense pressure tiers name the P4 core-replay mask strategy without claiming active workgroups');
...
assert.match(core, /let inletSpan = max\(4u, FAR_GRID \/ 2u\)/, 'near-domain outlet occupies a centered subset of the larger far-domain footprint');
assert.match(core, /gid\.x < inletMin \|\| gid\.x >= inletMax \|\| gid\.z < inletMin \|\| gid\.z >= inletMax/, 'far-domain inlet leaves lateral cells empty outside the centered near-domain footprint');
assert.match(core, /\(inletCell\.x \* NEAR_GRID \+ NEAR_GRID \/ 2u\) \/ inletSpan/, 'near-to-far inlet samples the center of each near-grid source bin on X');
assert.match(core, /\(inletCell\.y \* NEAR_GRID \+ NEAR_GRID \/ 2u\) \/ inletSpan/, 'near-to-far inlet samples the center of each near-grid source bin on Z');
...
assert.match(core, /fn sampleFarStateTrilinear/, 'far-smoke advection uses trilinear semi-Lagrangian field sampling');
assert.match(core, /if \(any\(p < vec3<f32>\(0\.0\)\) \|\| any\(p > vec3<f32>\(hi\)\)\) \{ return vec4<f32>\(0\.0\); \}/, 'far-smoke advection samples zero beyond its open domain boundaries');
assert.match(core, /select\(0\.0, smokeCandidate, smokeCandidate > 0\.012\)/, 'far-smoke advection retires sub-support interpolation tails instead of filling empty cells indefinitely');
assert.match(core, /smokeDomainFarAdvectedActiveCells/, 'debug state counts occupied far-smoke cells beyond the injection band');
assert.match(core, /smokeDomainFarHighestActiveLayer/, 'debug state exposes the highest occupied far-smoke layer');
assert.match(core, /fsSmokeDomainInspect/, 'volume core has a far-state-only inspect fragment path');
assert.match(core, /smokeDomainInspectPipeline/, 'volume core builds a distinct far-only inspect pipeline');
...
assert.match(volumeShellLabWitness, /smokeDomainFarAdvectedActiveCells/, 'far-only witness fails when smoke has not moved beyond the injection band');
assert.match(volumeShellLabWitness, /smokeDomainTransferLastReadbackFrame >= minFrames/, 'far-only witness requires counters produced after the requested frame floor');
assert.match(volumeShellLabWitness, /smokeDomainFarInputActiveCells < farCellCount/, 'far-only witness rejects a fully saturated far domain');
assert.match(volumeShellLabWitness, /far-smoke-only-max-projection-v0/, 'far-only witness verifies the effective inspect identity before writing output');
...
assert.match(core, /pressureActivityCueAtCell/, 'WGSL has a current sim-side activity cue for pressure tiers');
assert.match(core, /pressureActivityMaskAtCell/, 'WGSL has dilated current sim-side activity masks for pressure tiers');
const pressureActivityBlock = core.match(/fn pressureActivityCueAtCell[\s\S]*?fn csProjectPressure/)?.[0] || '';
...
assert.match(pressureActivityBlock, /csPressureJacobiTieredP4/, 'activity dense masked dispatch has a direct full-grid P4 core replay Jacobi entry point');
...
assert.match(core, /pressureActivityIndirectArgsBuffer/, 'volume core separates active pressure workgroup storage from indirect dispatch args');
...
assert.match(core, /p4Strategy:[\s\S]*activityPressureP4Strategy/, 'activity tier debug state exposes the dispatch-specific P4 refinement strategy');
assert.match(core, /ACTIVITY_PRESSURE_P4_STRATEGY/, 'activity tier debug state can report the indirect P4 refinement strategy');
...
assert.match(sweep, /volume_activity_pressure_dispatch/, 'performance sweep can compare Activity indirect and dense masked dispatch routes');
