...
assert.match(index, /createVolumeSceneContextRouter/, 'volume route uses an explicit scene-context router instead of raw volume-only smoke');
...
assert.match(index, /greenroom-glb-three-meshes-not-image-plate-v0/, 'brick-wall scene context records GLB-backed background truth');
assert.match(index, /BRICK_WALL_VOLUME_CAMERA_FRAME/, 'brick-wall scene context has a dedicated operator-smoke camera frame');
assert.match(index, /resolveVolumeCameraFrame/, 'volume camera framing resolves scene context instead of only the volume scene preset');
assert.match(index, /activeContext\s*===\s*'brick_wall'[\s\S]*BRICK_WALL_VOLUME_CAMERA_FRAME/, 'brick-wall scene context routes Frame/smoke camera to the brick-wall receiver composition');
assert.match(index, /servedUrlCandidates/, 'brick-wall scene context tries explicit served URL candidates instead of assuming one backend');
assert.match(index, /local-assets\/greenroom\/kaminos-trellis-crumbled-brick-wall-fast8-350k-4k-20260701Tasset-probe\/output\.glb/, 'brick-wall scene context has a Python-static local asset route for smoke');
assert.match(index, /apiReadUrl/, 'brick-wall scene context preserves the Greenroom api-read route as provenance/fallback');
assert.match(index, /assetEffectiveServedUrl/, 'brick-wall scene context reports the served URL that actually loaded');
assert.match(index, /assetFailedServedUrls/, 'brick-wall scene context reports failed served URL candidates');
assert.match(index, /window\.__kaminosVolumeSceneContext/, 'volume scene context is exposed for witness/debug truth');
...
assert.match(core, /COMBUSTION_FRONT_RECEIVER_SUPPORT_IDENTITY\s*=\s*'combustion-front-receiver-support-v0'/, 'volume module names the combustion-front receiver-light support export');
assert.match(core, /COMBUSTION_FRONT_RECEIVER_SUPPORT_AUTHORITY\s*=\s*'combustion-front-topology-sidecar-v0\+reaction-front-stage-fields-v0'/, 'receiver support export preserves its combustion-front and reaction-stage source authority');
assert.match(core, /COMBUSTION_FRONT_RECEIVER_SUPPORT_CONSUMER\s*=\s*'tier2-opt-in-receiver-buffer-light-pass-v0'/, 'receiver support export declares the Tier 2 opt-in receiver-buffer light-pass consumer');
assert.match(core, /supportRole:\s*'lighting-input-not-rendered-receiver-light'/, 'receiver support export cannot masquerade as a rendered receiver-light pass');
assert.match(core, /receiverMaskAuthority:\s*'opt-in-receiver-buffer-required-v0'/, 'receiver support export requires an explicit opted-in receiver mask before lighting scene geometry');
assert.match(core, /const receiverSupport = buildCombustionFrontReceiverSupport/, 'sim readback builds the combustion-front receiver support receipt from live support metrics');
...
assert.match(index, /TIER2_RECEIVER_SPLAT_MOMENT_ENVELOPE_SOURCE\s*=\s*'gpu-splat-radiance-coverage-depth-moments-v0'/, 'Tier 2 receiver light can consume Handy GPU splat radiance/coverage/depth moments as its envelope source');
assert.match(index, /requestedEnvelopeSource:\s*requestedReceiverEnvelopeSource/, 'Tier 2 receiver light debug preserves requested envelope source identity');
assert.match(index, /effectiveEnvelopeSource:\s*effectiveReceiverEnvelopeSource/, 'Tier 2 receiver light debug preserves effective envelope source identity');
assert.match(index, /dynamicEnvelopeSource:\s*'pending-envelope-source-resolution'/, 'Tier 2 receiver light does not advertise the legacy procedural envelope before the effective route resolves');
assert.match(index, /state\.dynamicEnvelopeSource\s*=\s*effectiveReceiverEnvelopeSource/, 'Tier 2 receiver light debug updates dynamicEnvelopeSource to the effective splat/procedural envelope authority');
assert.match(index, /splatMomentEnvelopeAccepted/, 'Tier 2 receiver light reports whether the requested splat-moment envelope is actually effective');
...
assert.match(index, /volume_receiver_light_isolate/, 'URL route can request receiver-light isolate evidence without foreground volume beauty');
assert.match(index, /window\.kaminosTier2ReceiverLightDebugState/, 'Tier 2 receiver light exposes a debug state for witness and operator smoke');
...
assert.match(index, /tier2ReceiverLightActive[\s\S]*mainRendererNeeded/, 'active Tier 2 receiver light forces the main renderer to draw during volume routes');
assert.match(volumeWitness, /receiverSupport\.identity !== 'combustion-front-receiver-support-v0'/, 'volume witness fails loudly when receiver support identity is missing or wrong');
assert.match(volumeWitness, /receiverSupport\.supportRole !== 'lighting-input-not-rendered-receiver-light'/, 'volume witness preserves the distinction between support input and rendered receiver light');
assert.match(volumeWitness, /receiverSupport:\s*sample\.simReadback\?\.receiverSupport \?\? null/, 'volume witness report preserves the receiver support receipt for Tier 2 receiver-light debugging');
assert.match(volumeWitness, /reactionFrontAtlas[\s\S]*rgbaLength[\s\S]*rgbaOmitted/, 'volume witness report preserves reaction-front atlas metadata without dumping the inline RGBA bitmap into JSON');
...
assert.match(witness, /receiver-support/, 'witness accepts a combustion-front receiver-support evidence mode');
assert.match(witness, /receiver-support-sim-readback/, 'witness reports receiver-support evidence as sim-readback authority rather than fire beauty');
assert.match(witness, /expectsReceiverSupportEvidence/, 'witness separates Tier 2 receiver-support receipts from ordinary orange-fire gates');
assert.match(witness, /receiverSupportEvidence/, 'witness report preserves a top-level receiver-support evidence receipt');
assert.match(witness, /receiver-light-isolate/, 'witness accepts an isolate evidence mode for the rendered Tier 2 receiver-light pass');
...
assert.match(witness, /receiver-light-brick-wall/, 'witness accepts a non-isolate brick-wall evidence mode for the rendered Tier 2 receiver-light pass');
assert.match(witness, /tier2-receiver-light-brick-wall/, 'witness reports the rendered Tier 2 receiver-light brick-wall visual evidence identity');
assert.match(witness, /expectsReceiverLightBrickWallEvidence/, 'witness separates Tier 2 brick-wall receiver evidence from isolate and legacy fire-volume gates');
assert.match(witness, /__kaminosVolumeSceneContext/, 'witness reads the live volume scene-context debug surface');
assert.match(witness, /volume-scene-context-brick-wall-v0/, 'witness verifies the brick-wall scene-context identity');
assert.match(witness, /trellis-fast8-350k-4k-brick-wall-glb-v0/, 'witness verifies the brick-wall asset identity');
assert.match(witness, /brick-wall-greenroom-local-static-mount-v0/, 'witness creates and reports the local Greenroom static mount before brick-wall smoke');
assert.match(witness, /assetEffectiveServedUrl === BRICK_WALL_STATIC_ROUTE_URL/, 'witness rejects brick-wall context evidence unless the static GLB route actually loaded');
assert.match(witness, /sceneContextCssComposite !== true/, 'witness rejects brick-wall routes where the native volume canvas is not CSS-composited over the GLB scene');
assert.match(witness, /nativeCanvasMixBlendMode !== 'screen'/, 'witness rejects brick-wall routes where the native volume canvas can still opaque-occlude the Three scene');
...
assert.match(witness, /receiver-light rendered evidence overexposed/, 'witness fails loudly when receiver-light evidence blows out the brick-wall scene');
...
assert.match(witness, /requestedEnvelopeSource !== 'gpu-splat-radiance-coverage-depth-moments-v0'/, 'witness fails loudly when rendered Tier 2 receiver-light smoke routes do not request the splat-moment envelope');
assert.match(witness, /effectiveEnvelopeSource !== 'gpu-splat-radiance-coverage-depth-moments-v0'/, 'witness fails loudly when rendered Tier 2 receiver-light smoke routes silently fall back from splat-moment authority');
assert.match(witness, /receiver-light-rendered-evidence[\s\S]*Page\.captureScreenshot[\s\S]*receiverLightEvidence[\s\S]*return;/, 'rendered receiver-light evidence captures and reports the visual artifact before unrelated legacy volume-control assertions can veto the receipt');
assert.match(witness, /if \(\s*!expectsCanonicalPlumeProof\s*&&\s*!expectsFuelStarvedTallPlume\s*&&\s*!expectsNoFireVolumeEvidence\s*&&\s*!expectsReceiverSupportEvidence\s*&&\s*!expectsReceiverLightIsolateEvidence\s*&&\s*!expectsReceiverLightBrickWallEvidence\s*&&\s*\(!Number\.isFinite\(sample\.simReadback\.fireLayerMean\) \|\| sample\.simReadback\.fireLayerMean <= 0\.0005\)\s*&&\s*!acceptsRawCarrierPyroPaint\s*\)/, 'no-fire, receiver-support, receiver-light isolate, and receiver-light brick-wall evidence routes do not require positive transported fire-layer readback');
assert.match(witness, /!expectsFuelStarvedTallPlume\s*&&\s*!expectsNoFireVolumeEvidence\s*&&\s*!expectsReceiverSupportEvidence\s*&&\s*!expectsReceiverLightIsolateEvidence\s*&&\s*!expectsReceiverLightBrickWallEvidence[\s\S]*radianceMean/, 'no-fire, receiver-support, receiver-light isolate, and receiver-light brick-wall evidence routes do not require positive fire radiance readback');
assert.match(witness, /else if \(!expectsReceiverSupportEvidence && !expectsReceiverLightIsolateEvidence && !expectsReceiverLightBrickWallEvidence && \([\s\S]*fireFuelOverlapRatio <= 0\.01[\s\S]*\)\) \{/, 'receiver-support, receiver-light isolate, and receiver-light brick-wall evidence routes do not require the legacy tall-plume fire/fuel overlap beauty threshold');
