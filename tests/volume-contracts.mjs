...
assert.match(index, /receiver-wall-wash-only-black-scene-witness-v0/, 'URL route can isolate the wall-wash receiver light separately from the fire/contact receiver lobe');
...
assert.match(index, /rendered-volume-canvas-visible-wide-gather-local-energy-route-gain-v26-positive-fire-energy-expansion/, 'screen-space fire receiver light must drive scalar intensity from route gain and positive-fire expanded wide-gather rendered-volume texture energy, not the uploaded control proxy');
...
assert.match(index, /const receiverWallWashBandX = smoothstep\(float\(0\.34\), float\(0\.52\), receiverUv\.x\)/, 'screen-space fire receiver light must include a broad horizontal wall-wash receiver band instead of only the contact/rubble lobe');
assert.match(index, /const receiverWallWashBandY = smoothstep\(float\(0\.14\), float\(0\.32\), receiverUv\.y\)/, 'screen-space fire receiver light must include a broad vertical wall-wash receiver band for the brick face');
assert.match(index, /const receiverLobe = receiverContactLobe\.add\(receiverWallWashLobe\)/, 'screen-space fire receiver light must combine the contact lobe with the wall-wash lobe before depth and normal masking');
...
assert.match(index, /receiverWallWashMask\.mul\(receiverWallWashVisibility\)\.mul\(fireReceiverLightGainNode\)\.mul\(fixedVolumeFireEnergy\)/, 'wall-wash-only witness must be driven by fixed fire energy and broad wall mask, not receiver-local fire silhouette probes');
...
assert.match(index, /const receiverEnergySignal = smoothstep\(float\(0\.0\), float\(0\.0025\), volumeFireEnergy\)/, 'receiver-light production energy must expand tiny positive rendered-fire energy without adding a near-dark fallback floor');
assert.match(index, /const receiverEnergy = receiverEnergySignal\.mul\(float\(7\.20\)\)[\s\S]{0,160}\.add\(volumeFireEnergy\.mul\(float\(1600\.0\)\)\)[\s\S]{0,160}\.add\(volumeFireEnergy\.mul\(volumeFireEnergy\)\.mul\(float\(5200\.0\)\)\)/, 'receiver-light production energy must drive from the sensitive expanded rendered-fire scalar with enough visible wall-wash gain');
assert.match(index, /receiverLightMask\.mul\(fireReceiverLightGainNode\)\.mul\(receiverEnergy\)\.mul\(float\(3\.0\)\)/, 'receiver-light production output must make the rendered-fire-only wall wash operator-visible without adding fallback radiance');
