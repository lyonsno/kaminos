...
assert.doesNotMatch(index, /const receiverEnergy = volumeFireEnergy\.mul\(float\([^)]*\)\)\.add\(/, 'receiver energy must not add any envelope fallback term to rendered fire energy');
...
assert.match(core, /gpuWallWashDebugEnergy/, 'fluid fragment shader keeps only meter/debug energy for brick-wall wash after removing the color write');
assert.doesNotMatch(core, /gpuWallWashPostToneWallTint/, 'brick-wall GPU wash must not post-tone tint the volume/fire body');
assert.doesNotMatch(core, /color\s*=\s*color\s*\+\s*gpuWallWashColor/, 'brick-wall GPU wash must not inject wall color into the volume/fire body');
...
assert.match(core, /gpu-meter-drives-scene-space-wall-glow-v0/, 'GPU wall-wash debug state must disclose the scene-space wall-glow proxy role');
