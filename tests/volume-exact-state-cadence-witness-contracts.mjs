...
assert.match(source, /activatePrototype/, 'witness explicitly activates the current Stage B prototype instead of assuming auto-start');
assert.match(source, /owned-witness-prototype-setActive-v0/, 'activation receipt names witness-owned setActive authority');
...
assert.match(source, /sampleFrame\(\{\s*advanceSim:\s*false,\s*includeRgba:\s*true,\s*boundarySplatComposition:\s*'raymarch-under-splats-v0'\s*\}\)/, 'pixels come from explicit Stage B raymarch-under-splats GPU texture readback without an added simulator step');
...
assert.match(
  source,
  /writeFileSync\(path, bytes\);[\s\S]*lastTrustworthyEvidence\[`\$\{label\}CanvasAttempt`\] = receipt;[\s\S]*if \(!visiblePixelGate\.pass\)/,
