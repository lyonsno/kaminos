...
if (mlxPython) {
  ...
  const pairingControlRun = spawnSync(mlxPython, [
    ...
    '--descriptor-group', 'curl-activity-only',
  ], { encoding: 'utf8' });
  ...
  assert.equal(pairingControlReport.settings.descriptorGroup.mode, 'curl-activity-only');
  assert.deepEqual(pairingControlReport.settings.descriptorGroup.activeChannels, ['flow.curlActivity']);
