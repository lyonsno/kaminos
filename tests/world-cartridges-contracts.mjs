...
const internalCoordinationPathPattern = new RegExp(`${['meta', 'dosis'].join('')}\\/|projects\\/`);
assert.doesNotMatch(
  manifestSource,
  internalCoordinationPathPattern,
  'world cartridge manifest uses public docs and role/subsystem keys instead of internal coordination paths',
);
...
assert.equal(normalized.affordanceBindings.some(binding => binding.id === 'actor-motion-agency'), true);
assert.equal(normalized.affordanceBindings.some(binding => binding.id === 'hand-surface-runtime'), true);
...
assert.equal(index.cartridges[0].firstUseTrial.consumerCoverage.every(coverage => !coverage.consumer.includes(' ')), true);
