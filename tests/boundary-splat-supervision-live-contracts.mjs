...
assert.match(core, /captureBoundarySplatSupervisionFrame[\s\S]*selectiveHeadLiveRole:\s*'truthHigh'[\s\S]*selectiveHeadLiveRenderComposition:\s*'splat-only-v0'/, 'candidate capture forces the fresh selective-head role and splat-only composition required by official admission');
assert.match(core, /captureBoundarySplatSupervisionFrame[\s\S]*candidateSample\.selectiveHeadLiveEffectiveRole\s*!==\s*'truthHigh'[\s\S]*candidateSample\.selectiveHeadLiveCompositionEffective\s*!==\s*'splat-only-v0'/, 'candidate capture rejects a renderer that fails to apply the forced selective-head contract');
