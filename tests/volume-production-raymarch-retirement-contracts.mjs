...
assert.match(core, /directSupport = directCellOpticalSupport\(p\);[\s\S]*directCellExitDistance\(p, rd\)[\s\S]*if \(flowKernelReconstructionActive\)/, 'enabled native transported support gates optional reconstruction before any filter work');
