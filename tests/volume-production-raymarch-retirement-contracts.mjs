...
const directCellExitBody = extractWgslFunctionBody(core, 'directCellExitDistance');
const directCellExitUsesRectangularBounds = body => /let q = clamp\(worldToCell\(p\) - vec3<f32>\(0\.5\), vec3<f32>\(0\.0\), vec3<f32>\(f32\(GRID\) - 1\.001, f32\(GRID_Y\) - 1\.001, f32\(GRID\) - 1\.001\)\);/.test(body);
assert.equal(
  directCellExitUsesRectangularBounds(directCellExitBody),
  true,
  'cell-exit traversal uses the same half-cell lattice and rectangular bounds as trilinear reconstruction',
);
const cubicCellExitMutation = directCellExitBody.replace('f32(GRID_Y) - 1.001', 'f32(GRID) - 1.001');
