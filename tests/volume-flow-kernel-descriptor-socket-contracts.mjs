...
assert.match(selectiveBindGroupSource, /splat:[\s\S]*binding:\s*13[^\n]*majorantBuffer/, 'selective-head splat route binds the shared conservative majorant without colliding with instance or history storage');
assert.match(selectiveBindGroupSource, /splat:[\s\S]*binding:\s*14[^\n]*flowKernelDescriptorBuffer/, 'selective-head splat route binds the shared descriptor output without colliding with instance or history storage');
