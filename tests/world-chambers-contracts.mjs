...
const main = readFileSync(join(root, 'kaminos-main.js'), 'utf8');
...
assert.match(main, /world-chambers-core\.js/, 'full editor module imports the world chambers substrate');
assert.match(main, /kaminosWorldChambersDebugState/, 'full editor module exposes a world chamber debug surface');
...
assert.match(main, /world_chamber/, 'URL route can open a world chamber without manual tab clicking');
assert.match(main, /world_chamber_receipt_url/, 'URL route can load an external world chamber receipt by URL');
assert.match(main, /world_chamber_receipt_root/, 'URL route can select a server-backed receipt root');
assert.match(main, /world_chamber_receipt_path/, 'URL route can select a server-backed receipt path');
assert.match(main, /preview_bench_payload_url/, 'URL route can load a generic Preview Bench payload by URL');
assert.match(main, /preview_bench_payload_root/, 'URL route can select a generic server-backed Preview Bench payload root');
assert.match(main, /preview_bench_payload_path/, 'URL route can select a generic server-backed Preview Bench payload path');
