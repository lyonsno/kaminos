...
const currentPackagePath = resolve(dirname(channelPath), channel.current.packageRelativePath);
const firstPackagePath = resolve(basinRoot, receipt.package.relativePath);

assert.equal(channel.schema, 'kaminos.volume.basin-promotion-channel.v1');
assert.equal(channel.handle, handle);
assert.ok(channel.history.length >= 1, 'promoted handle must retain its first accepted revision');
assert.ok(existsSync(currentPackagePath), 'current channel package must exist in the repository');
assert.ok(existsSync(firstPackagePath), 'first revision receipt package must remain in the repository');

const packageBytes = readFileSync(firstPackagePath);
const packageDocument = validateBasinPromotionPackage(JSON.parse(packageBytes));
const packageSha256 = createHash('sha256').update(packageBytes).digest('hex');
const firstHistoryEntry = channel.history.find(entry => entry.revision === receipt.revision);
assert.ok(firstHistoryEntry, 'channel history must retain the revision named by the first receipt');
assert.equal(packageSha256, firstHistoryEntry.packageSha256);
assert.equal(packageDocument.revision, firstHistoryEntry.revision);
assert.equal(packageDocument.stableRef, firstHistoryEntry.stableRef);
assert.equal(packageDocument.sourceCommit, firstHistoryEntry.sourceCommit);
...
assert.equal(receipt.package.relativePath, firstHistoryEntry.packageRelativePath);
