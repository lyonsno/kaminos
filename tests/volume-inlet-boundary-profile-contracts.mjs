...
assert.match(coreSource, /floats\[23\] = descriptor\.edgeEntrainment \* incrementScale;[\s\S]*?floats\[24\] = ANALYTIC_EMITTER_INLET_PROFILE_MODE\[descriptor\.inletProfile\][\s\S]*?floats\[25\] = descriptor\.momentumLinked \? 0 : 1;[\s\S]*?floats\[26\] = descriptor\.effectiveInletVelocity \* incrementScale;[\s\S]*?floats\[27\] = descriptor\.shearWidthCells;[\s\S]*?words\.set\(\[\.\.\.dispatch\.cellMin, dispatch\.grid\], 28\);[\s\S]*?words\.set\(\[\.\.\.dispatch\.cellExtent, 0\], 32\);/, 'CPU uniform packing carries the descriptor-authoritative effective inlet velocity at the inlet-controls boundary');
