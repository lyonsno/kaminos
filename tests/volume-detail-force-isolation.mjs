...
assert.deepEqual(detailForceIsolationReceipt({ volumeScene: 'bonfire_plume', bonfireDetailForces: 0 }).effectiveMask, [0, 0, 0, 0]);
assert.deepEqual(detailForceIsolationReceipt({ volumeScene: 'bonfire_plume', bonfireDetailForces: 0.5 }).effectiveMask, [0.5, 0.5, 0.5, 0.5]);
assert.deepEqual(detailForceIsolationReceipt({ volumeScene: 'bonfire_plume' }).effectiveMask, [1, 1, 1, 1]);
...
assert.match(index, /forceComparison\.sceneGain === 0[\s\S]*?Bonfire detail-force gain is ZERO/);
