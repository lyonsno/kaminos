...
assert.equal(pkg.gestalt.kind, witness.candidates.find(item => item.id === 'lirm-armature-22').bodyPlan.gestalt.kind);
assert.equal(pkg.silhouette.class, witness.candidates.find(item => item.id === 'lirm-armature-22').bodyPlan.silhouette.class);
...
assert.equal(pkg.trellisSource.kind, 'trellis-clay');
assert.equal(pkg.trellisSource.path, 'trellis-source.svg');
assert.equal(pkg.trellisSource.rasterPath, 'trellis-source.png');
assert.equal(pkg.trellisSource.requiredFor.includes('trellis_clay_probe'), true);
assert.equal(pkg.trellisSource.effectiveSource, 'tight-cropped-transparent-implicit-clay');
...
assert.match(pkg.prompt.positive, new RegExp(pkg.gestalt.label));
assert.match(pkg.prompt.positive, new RegExp(pkg.silhouette.class.replaceAll('-', '[- ]')));
...
assert.ok(existsSync(join(outDir, 'lirm-armature-22', 'trellis-source.svg')), 'writer must emit clean Trellis source SVG');
assert.ok(existsSync(join(outDir, 'lirm-armature-22', 'trellis-source.png')), 'writer must emit clean Trellis source PNG');
...
assert.equal(receipt.packages[1].gestalt.kind, pkg.gestalt.kind);
assert.equal(receipt.packages[1].silhouette.class, pkg.silhouette.class);
assert.equal(receipt.outputInventory.packages[1].trellisSource.rasterPath, 'lirm-armature-22/trellis-source.png');
assert.equal(receipt.packages[1].trellisSource.rasterPath, 'trellis-source.png');
