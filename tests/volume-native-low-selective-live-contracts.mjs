...
const native96ExactFrontTeacherManifestPath = join(root, 'models/selective-head-live/latest-happy-bowl-front-only-160-to-96-step96-v0/manifest.json');
const native96ExactFrontTeacherGeneratedPath = join(root, 'models/selective-head-live/latest-happy-bowl-front-only-160-to-96-step96-v0/model.generated.js');
...
assert.ok(existsSync(native96ExactFrontTeacherManifestPath), 'native96 exact front-only teacher manifest is mirrored for browser custody');
assert.ok(existsSync(native96ExactFrontTeacherGeneratedPath), 'native96 exact front-only teacher browser manifest exists');
...
const native96ExactFrontTeacherManifest = JSON.parse(readFileSync(native96ExactFrontTeacherManifestPath, 'utf8'));
assert.equal(native96ExactFrontTeacherManifest.identity, 'latest-happy-bowl-front-only-160-to-96-step96-v0');
assert.equal(native96ExactFrontTeacherManifest.packed?.byteLength, 37396);
assert.equal(native96ExactFrontTeacherManifest.packed?.sha256, '2eb3d311d8964d21ba471bba973b38ac1f32ee25b0a73926a6ee7b43ca78e95b');
assert.equal(native96ExactFrontTeacherManifest.features?.featureCount, 185);
assert.equal(native96ExactFrontTeacherManifest.architecture?.hiddenWidth, 48);
assert.deepEqual(native96ExactFrontTeacherManifest.packagedResidualChannels, ['frontTopology']);
assert.equal(native96ExactFrontTeacherManifest.derivation?.sourcePackedSha256, '97e25caa711395f26e8b39f22c506e38e772bfc1a12cf518d5e048511d2bee08');
...
assert.match(combined, /native96-sparse-front-continuity-v0/, 'route records native96 sparse-front continuity recovery identity');
assert.match(combined, /sparse_front_continuity[\s\S]*native96SparseFrontContinuityEnabled/, 'operator route exposes the sparse-front continuity recovery switch');
assert.match(combined, /latest-happy-bowl-front-only-160-to-96-step96-v0[\s\S]*2eb3d311d8964d21ba471bba973b38ac1f32ee25b0a73926a6ee7b43ca78e95b/, 'route binds exact native96 front-only teacher identity and checksum');
assert.match(combined, /native96ExactFrontTeacherParentCommit[\s\S]*cf15a42d847cb727d5aad4fc4ef212cf6f40c5ce/, 'receipt records parent commit custody for the exact 37KB teacher');
assert.match(combined, /hardZeroOutsideCandidateVisuallyRejected:\s*true[\s\S]*hardMaskTreatmentClaim:\s*false/, 'continuity route rejects the visually failed hard zero-outside-candidate interpretation');
assert.match(combined, /continuityReconstructionMode[\s\S]*(feathered|interpolation|continuous)/, 'continuity route names continuous reconstruction, interpolation, or feathered blending authority');
assert.match(combined, /deterministicVsLearnedEffectParity[\s\S]*preserve-native96-accepted-structural-delta-v0/, 'continuity receipt preserves accepted deterministic-vs-learned structural-delta parity as the target');
assert.match(combined, /exactFrontTeacherEvalGpuMs[\s\S]*continuityReconstructionGpuMs[\s\S]*totalSparseFrontContinuityGpuMs/, 'continuity receipt reports teacher-eval, reconstruction, and total incremental GPU timing');
assert.match(combined, /sourceDeltaAdmissionGpuMs[\s\S]*exactFrontTeacherEvalGpuMs[\s\S]*continuityReconstructionGpuMs[\s\S]*materializationMs[\s\S]*renderMs/, 'continuity timing ledger includes admission, teacher, reconstruction, materialization, and render stages');
assert.match(combined, /sparseFrontContinuityDecisionBands[\s\S]*profitableTargetMs[\s\S]*10[\s\S]*credibleBreakEvenTargetMs[\s\S]*15[\s\S]*outerKillBoundaryMs[\s\S]*24/, 'continuity route reports 10/15/24ms product bands');
assert.match(combined, /uncappedCandidateCount[\s\S]*uncappedCandidateCoverage[\s\S]*hiddenCandidateCap:\s*false/, 'continuity route preserves uncapped candidate work accounting without hidden caps');
assert.match(combined, /fullGridTeacherReferenceRetained:\s*true[\s\S]*sparseContinuityTreatmentRendererConsumed:\s*true/, 'continuity route keeps dense teacher reference while consuming the sparse-continuity treatment visually');
assert.match(combined, /native96Control[\s\S]*deterministicNativeMaterialization[\s\S]*fullExactFrontTeacherReference[\s\S]*sparseFrontContinuityTreatment/, 'continuity operator artifact exposes native control, deterministic materialization, full teacher reference, and sparse continuity treatment roles');
assert.match(witness, /native96SparseFrontContinuity[\s\S]*hardZeroOutsideCandidateVisuallyRejected[\s\S]*totalSparseFrontContinuityGpuMs/, 'witness preserves sparse-front continuity receipt and timing');
assert.doesNotMatch(combined, /hardZeroOutsideCandidateVisuallyRejected:\s*false|hardMaskTreatmentClaim:\s*true/, 'continuity route cannot revive the rejected hard-mask treatment claim');
...
assert.match(route, /modelUrl:\s*cockpitMode\s*\?\s*null[\s\S]*checksum-bound-trained-package-route-registry-v0/, 'cockpit cannot expose the stale static default model URL for a switched package');
...
assert.match(route, /selectedRendererCoverage[\s\S]*sourceDeltaAdmissionCandidateCount[\s\S]*sourceDeltaAdmissionCoverage/, 'cockpit keeps selected-renderer coverage separate from source-delta admission coverage');
...
const frozenCaptureStart = core.indexOf('async function renderFrozenScaleToCanvas');
const frozenCaptureEnd = core.indexOf('async function ensureNativeLowSelectiveSharedRuntime', frozenCaptureStart);
const frozenCapture = core.slice(frozenCaptureStart, frozenCaptureEnd);
assert.match(frozenCapture, /boundarySplatCompositionRequestedRaw\s*===\s*'raymarch-only-v0'[\s\S]*encodeDraw\(encoder,\s*currentTexture\.createView\(\),\s*'kaminos frozen explicit raymarch-only canvas pass'\)[\s\S]*raymarchEncoded\s*=\s*true/, 'explicit Raymarch capture encodes the Raymarch pass regardless of ambient splat state');
assert.match(frozenCapture, /boundarySplatCompositionRequestedRaw\s*===\s*'raymarch-only-v0'[\s\S]*boundarySplatCompositionEffective\s*=\s*'raymarch-only-v0'/, 'explicit Raymarch capture reports Raymarch as the effective composition');
...
assert.match(core, /receiverActive:\s*true[\s\S]*oracleActivityCueSourceValues[\s\S]*resampleScalarActivityCue[\s\S]*forcing-role-disabled-external-cue-restored/, 'forcing route owns the shared receiver while active and restores any prior external cue on exit');
assert.match(core, /native96LearnedActivityForcingState\.receiverActive[\s\S]*native96LearnedActivityForcingState\.cueValid[\s\S]*oracleActivityCueUpload\.status\s*===\s*'uploaded'/, 'uniform routing suppresses uploaded truth authority while learned forcing owns the receiver');
assert.match(witness, /expectedComposition[\s\S]*raymarch-only-v0[\s\S]*effectiveComposition/, 'witness can require and verify an effective Raymarch route');
assert.match(core, /const selectedPresentationRenderReceipt\s*=\s*\{[\s\S]*raymarchEncoded[\s\S]*splatEncoded[\s\S]*raymarchApplied[\s\S]*splatApplied[\s\S]*selectedPresentationRenderReceipt,/, 'shared-device receipt exposes selected renderer pass facts');
assert.match(witness, /expectedComposition\s*===\s*'raymarch-only-v0'[\s\S]*raymarchEncoded,\s*true[\s\S]*raymarchApplied,\s*true[\s\S]*splatEncoded,\s*false[\s\S]*splatApplied,\s*false/, 'witness proves Raymarch-only execution from pass facts rather than composition labels');
assert.match(witness, /cockpitLearnedForcingRequested[\s\S]*setCapturePaused\(true\)[\s\S]*stepCaptureFrame\(\)[\s\S]*modelRefreshDue\s*===\s*true[\s\S]*learnedActivityCueCollapseGpuMs/, 'forcing witness deterministically single-steps to a fresh model-evaluation frame before requiring cue-collapse timing');
assert.match(witness, /forcingRefreshState[\s\S]*setCapturePaused\(false\)[\s\S]*forcingResumedState[\s\S]*status,\s*'running'/, 'forcing witness separately proves that continuous execution resumed after timed single-step acquisition');
assert.match(witness, /forcingRefreshFrame:\s*forcingRefreshFrame\s*\?[\s\S]*nativeLowHeadCostProfile:\s*forcingRefreshFrame\.nativeLowHeadCostProfile[\s\S]*learnedModelRefreshCadence:\s*forcingRefreshFrame\.learnedModelRefreshCadence/, 'successful forcing report preserves the timed refresh frame separately from the end-of-capture reuse frame');
