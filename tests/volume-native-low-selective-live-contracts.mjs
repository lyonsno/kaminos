...
assert.match(combined, /trainedLowGrid[\s\S]*128[\s\S]*effectiveSourceGrid[\s\S]*native-64-native-96-or-native-128-runtime-selected-v0/, '160->128 candidate records trained grid and effective source grid authority');
...
assert.match(route, /primaryRouteId[\s\S]*NATIVE_LOW_TRANSFER_LATEST_HAPPY_BOWL_160_TO_96_ROUTE[\s\S]*NATIVE_LOW_TRANSFER_160_TO_128_ZERO_SHOT_ROUTE[\s\S]*transferRouteId:\s*primaryRouteId[\s\S]*advanceSourceStep:\s*true[\s\S]*transferRouteId:\s*NATIVE_LOW_TRANSFER_160_TO_96_DEPLOYMENT_GRID_ROUTE[\s\S]*advanceSourceStep:\s*false/, 'one manual step advances native96 source once through the selected primary package, then encodes the legacy 96 package without another sim step');
assert.match(route, /sameNativeStateIdentity[\s\S]*sourceStepIdentity[\s\S]*singleNativeSourceStepAdvanced[\s\S]*secondPackageAdvanceSourceStep:\s*false/, 'manual capture returns same source/state identities and exact stepping receipts');
