...
assert.match(
  cockpit,
  /const emitterMorphologyControls = new Set\(\[[\s\S]*?'emitter-assay-family'[\s\S]*?'volume-emitter-inlet-profile'[\s\S]*?'volume-emitter-edge-entrainment'[\s\S]*?'volume-speed'[\s\S]*?\]\)/,
  'only source-boundary and morphology-bearing controls enter the analytic descriptor path',
);
assert.match(
  cockpit,
