...
assert.match(index, /function hybridSplatOverlaySharedDeviceForRenderer\(/, 'Hybrid Renderer validates host WebGPU device limits before sharing Kaminos renderer device');
assert.match(index, /maxComputeWorkgroupStorageSize[\s\S]*32768/, 'Hybrid Renderer refuses shared devices below the splat sorter workgroup-storage requirement');
assert.match(index, /function hybridSplatHostDepthTextureSharingEnabled\(/, 'Hybrid Renderer only publishes host-depth textures when the renderer shares the Kaminos WebGPU device');
...
assert.match(index, /function sceneSplatRecords\(/, 'Hybrid Renderer scene start discovers every reloadable splat in the scene');
assert.match(index, /function hybridSplatRecordIsRendererCapable\(/, 'Hybrid Renderer filters scene membership through renderer-capable splat metadata');
assert.match(index, /sceneSplatRecords\(\)[\s\S]*hybridSplatRecordIsRendererCapable/, 'Hybrid Renderer scene membership excludes point-cloud-only PLY previews from renderer loads');
assert.match(index, /renderability:\s*entry\.renderability \|\| null/, 'Greenroom splat imports preserve asset renderability for scene renderer membership decisions');
assert.match(index, /handle\.loadSceneSplats\(entries\)/, 'Hybrid Renderer scene start loads all scene splats into one renderer-owned scene');
...
assert.match(index, /toggleHybridSplatViewportRenderer\([\s\S]*startHybridSplatSceneRenderer\(\)/, 'Viewport Start Hybrid dispatches through the scene-level renderer path');
assert.match(index, /function syncHybridSplatSceneRendererMembership\(/, 'Hybrid Renderer can resync scene splat membership without forcing operator stop/start');
assert.match(index, /registerSceneObject[\s\S]*scheduleHybridSplatSceneRendererSync\('scene-splat-registered'\)/, 'Hybrid Renderer schedules scene membership sync when a splat is registered while rendering');
assert.match(index, /removeSceneObject[\s\S]*scheduleHybridSplatSceneRendererSync\('scene-splat-removed'\)/, 'Hybrid Renderer schedules scene membership sync when a splat is removed while rendering');
...
assert.match(index, /function kaminosSplatAssetRouteFromSearch\(/, 'Kaminos exposes a first-class splat_root/splat_path boot route for PLY smoke links');
assert.match(index, /function loadKaminosSplatAssetRoute\(/, 'Kaminos PLY smoke links load through an explicit boot function instead of the legacy vague load path');
assert.match(index, /splat_root[\s\S]*splat_path[\s\S]*\/api\/read\?/, 'Kaminos PLY smoke links preserve requested root/path and build an effective /api/read route');
assert.match(index, /function kaminosImageAssetRouteFromSearch\(/, 'Kaminos exposes a first-class image_root/image_path boot route for image smoke links');
assert.match(index, /function loadKaminosImageAssetRoute\(/, 'Kaminos image smoke links load through an explicit boot function instead of a raw file or inert app shell');
assert.match(index, /image_root[\s\S]*image_path[\s\S]*\/api\/read\?/, 'Kaminos image smoke links preserve requested root/path and build an effective /api/read route');
