...
assert.match(index, /const SMOKE_OFFER_SCHEMA\s*=\s*'kaminos\.forge-host\.smoke-offer\.v0'/, 'Preview Bench host declares the Forge Host smoke-offer schema');
assert.match(index, /const PREVIEW_BENCH_SMOKE_OFFER_STATE_SCHEMA\s*=\s*'kaminos\.preview-bench\.smoke-offer-state\.v0'/, 'Preview Bench host declares the mounted smoke-offer state schema');
assert.match(index, /const PREVIEW_BENCH_PAYLOAD_REPORT_SCHEMA\s*=\s*'kaminos\.preview-bench\.payload-report\.v0'/, 'Preview Bench host declares the generic payload-report schema');
assert.match(index, /const PREVIEW_BENCH_PAYLOAD_ROUTE\s*=\s*'kaminos\/preview-bench\/payload-file'/, 'Preview Bench host declares the generic payload-report route');
assert.match(index, /const PREVIEW_BENCH_TERRAIN_SAMPLE_PACKET_SCHEMA\s*=\s*'kaminos\.preview-bench\.terrain-sample-packet\.v0'/, 'Preview Bench host declares the terrain sample packet schema for raw Hill/Big Papa coupling');
assert.match(index, /const PREVIEW_BENCH_TERRAIN_SAMPLE_ROUTE\s*=\s*'kaminos\/preview-bench\/terrain-sample-file'/, 'Preview Bench host declares the terrain sample packet route');
assert.match(index, /function previewBenchSmokeOfferUrlFromRoute\(/, 'Preview Bench smoke offers can be mounted from URL or root/path route parameters');
assert.match(index, /smoke_offer_root/, 'Preview Bench smoke-offer route accepts server-backed root identity');
assert.match(index, /smoke_offer_path/, 'Preview Bench smoke-offer route accepts server-backed path identity');
assert.match(index, /preview_bench_payload_root/, 'Preview Bench payload-report route accepts server-backed root identity');
assert.match(index, /preview_bench_payload_path/, 'Preview Bench payload-report route accepts server-backed path identity');
...
assert.match(index, /terrainSamplePacket/, 'Preview Bench host can extract embedded terrainSamplePacket reports from lane-local witness files');
assert.match(index, /function previewBenchSurfaceLabel\(/, 'Preview Bench surfaces without id still render kind/route/bench identity');
assert.match(index, /surface\.surface/, 'Preview Bench rejected debug surfaces preserve producer surface keys, not only id or route');
assert.match(index, /function mountPreviewBenchSmokeOffer\(/, 'Preview Bench host has an explicit mount function');
assert.match(index, /window\.kaminosPreviewBenchSmokeOfferDebugState/, 'browser witnesses can inspect Preview Bench smoke-offer state without DOM inference');
assert.match(index, /window\.kaminosMountPreviewBenchPayloadReport/, 'agents can mount a generic Preview Bench payload report directly without creating bespoke tabs');
assert.match(index, /window\.kaminosMountPreviewBenchTerrainSamplePacket/, 'agents can mount source-owned terrain sample packets without bespoke tabs or manual JSON paste');
assert.match(index, /window\.kaminosMountPreviewBenchSmokeOffer/, 'agents can mount a smoke offer directly without creating bespoke tabs');
assert.match(index, /id="preview-bench-smoke-offer-list"/, 'Preview Bench tab renders smoke offers through a stable list surface');
assert.match(index, /data-smoke-offer-authority/, 'Preview Bench rows preserve source authority for witness capture');
assert.match(index, /data-smoke-offer-freshness/, 'Preview Bench rows preserve freshness separately from authority');
assert.match(index, /data-smoke-offer-downgrade/, 'Preview Bench rows preserve downgrade/fallback state separately from freshness');
assert.match(index, /data-smoke-offer-rejected-surface/, 'Preview Bench rows expose rejected debug surfaces as first-class evidence');
assert.match(index, /data-terrain-sample-transport/, 'Preview Bench terrain sample rows expose transport encoding for physics consumers');
assert.match(index, /data-terrain-sample-fetch-url/, 'Preview Bench terrain sample rows expose the fetch URL when arrays remain source-owned');
assert.match(index, /data-terrain-sample-sample-checksum/, 'Preview Bench terrain sample rows expose sample checksums for custody');
assert.match(index, /targetSurface/, 'Preview Bench smoke-offer state preserves the requested target surface');
assert.match(index, /acceptanceSurface/, 'Preview Bench smoke-offer state preserves the acceptance surface instead of trusting lane-local debug routes');
