...
assert.match(index, /Pipeline Dock/, 'Pipeline tab is framed as a dock/workbench, not a run-bundle browser');
assert.match(index, /pipeline-source-summary/, 'Pipeline Dock exposes selected source identity');
assert.match(index, /pipeline-stage-list/, 'Pipeline Dock renders source-action-result stages as linear graph porcelain');
assert.match(index, /pipeline-evidence-drawer/, 'Pipeline Dock keeps run bundle/report/sidecar details behind an evidence drawer');
