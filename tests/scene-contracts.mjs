...
assert.match(index, /meshDepthOcclusion:\s*'host-depth-texture'/, 'Hybrid Renderer route reports host-depth texture mesh occlusion when the overlay supports it');
assert.match(index, /function createHybridSplatHostDepthTarget\(/, 'Hybrid Renderer creates a host renderer depth target for arbitrary mesh occlusion');
assert.match(index, /new THREE\.DepthTexture/, 'Hybrid Renderer host depth sharing uses a native depth texture instead of color-encoded linear depth');
assert.match(index, /function hybridSplatHostDepthTextureView\(/, 'Hybrid Renderer extracts a native WebGPU host depth texture view from Three backend state');
assert.match(index, /renderer\?\.backend\?\.get\?\.\(target\.depthTexture\)/, 'Hybrid Renderer extracts the host depth GPU texture from the render target depth texture');
assert.match(index, /setHostDepthTexture/, 'Hybrid Renderer publishes the host depth texture view to the overlay handle');
assert.match(index, /format:\s*'ndc-depth'/, 'Hybrid Renderer host depth texture uses the renderer G-buffer NDC depth convention');
assert.match(index, /depthConvention:\s*'webgpu-reversed-0-to-1'/, 'Hybrid Renderer host depth texture declares the native WebGPU reversed-depth convention');
assert.match(index, /reversedDepth:\s*true/, 'Hybrid Renderer host depth metadata tells the overlay to compare in reversed depth');
assert.match(index, /device:\s*renderer\.backend\.device/, 'Hybrid Renderer creates the overlay on the same WebGPU device as Three');
assert.match(index, /async function createKaminosWebGPUDevice\(/, 'Hybrid Renderer host-depth path creates an explicit shared WebGPU device instead of relying on Three compatibility defaults');
assert.match(index, /maxStorageBuffersPerShaderStage:\s*adapter\.limits\.maxStorageBuffersPerShaderStage/, 'Hybrid Renderer shared device requests enough storage-buffer bindings for the splat compute pipeline');
assert.match(index, /new THREE\.WebGPURenderer\(\{\s*antialias:\s*true,\s*device:\s*webgpuDevice\s*\}\)/, 'Kaminos passes the high-limit shared WebGPU device into Three before the overlay borrows it');
