...
assert.match(index, /function sceneIsEmpty\(\)[\s\S]*sceneObjects\.length === 0 && volumePrimitives\.length === 0 && !persistableFlame && !localLiquidSetup/, 'shared save guard rejects truly empty scenes while allowing volume, flame, and liquid-domain scenes');
