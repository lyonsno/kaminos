...
assert.match(index, /data-tab="library"/, 'Sidebar exposes a dedicated Library tab instead of burying image assets under Greenroom');
assert.match(index, /id="tab-library"/, 'Image assets render in a dedicated Library tab');
assert.match(index, /id="image-library-grid"/, 'Library tab uses an image thumbnail grid instead of a row list');
assert.match(index, /id="image-library-drop-zone"/, 'Library tab exposes a drop target for adding image assets');
assert.match(index, /id="image-library-capture-button"/, 'Library tab exposes viewport capture as a first-class image population path');
assert.match(index, /id="image-library-preview"/, 'Library tab previews selected images in-app instead of only opening a new browser tab');
assert.match(index, /async function refreshImageLibrary\(/, 'Library tab refreshes a declared image asset index instead of relying on scratch folklore');
assert.match(index, /grFetchApi\('assets',[\s\S]*kind:\s*'image'/, 'Image Library UI queries /api/assets?kind=image');
assert.match(index, /function selectImageLibraryAsset\(/, 'Image Library has explicit selected-asset state for detail actions');
assert.match(index, /async function captureViewportImageAsset\(/, 'Image Library can capture the current Kaminos viewport into image-inbox');
assert.match(index, /window\.kaminosImageLibraryDebugState/, 'browser witnesses and agents can inspect image library state without DOM inference');
assert.doesNotMatch(index, /id="tab-greenroom"[\s\S]*id="image-assets-list"/, 'Greenroom should not stack the image asset browser below splats');
