...
assert.match(indexHtml, /data-greenroom-splat-archive/, 'Splat asset history is addressable as an archive instead of dumping under the operator panel');
assert.match(indexHtml, /<details[^>]+id="splat-assets-archive"/, 'Splat asset history is collapsed behind a details archive by default');
assert.doesNotMatch(indexHtml, /<details[^>]+id="splat-assets-archive"[^>]+open/, 'Splat asset history must not be open by default');
assert.match(indexHtml, /data-greenroom-output-archive/, 'GPU Greenroom output history is addressable as an archive instead of dumping under the operator panel');
assert.match(indexHtml, /<details[^>]+id="gpu-greenroom-output-archive"/, 'GPU Greenroom output history is collapsed behind a details archive by default');
