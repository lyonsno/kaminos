...
assert.match(witness, /corrected-splat-frame-pivot/, 'witness supports a corrected splat with a separated authored pivot');
assert.match(witness, /corrected-splat witness requires an explicit server root/, 'corrected splat evidence cannot silently use another checkout');
assert.match(witness, /renderedRootNdc/, 'corrected splat browser witness asserts the rendered root stays in view');
assert.match(witness, /cloudCornerNdc/, 'corrected splat browser witness asserts every rendered cloud-bound corner stays in view');
assert.match(witness, /offscreenCloudCorners\.length/, 'corrected splat witness fails when any rendered cloud bound is cropped');
assert.match(witness, /synthetic PLY did not register as a splat scene object/, 'corrected splat witness requires the PLY to register through the splat path');
assert.match(witness, /separate authored and centroid-shifted pivots/, 'corrected splat witness verifies the authored and rendered pivots are distinct');
assert.match(witness, /precondition failed: authored pivot was already visible before F/, 'corrected splat witness proves F is exercised from an initially hidden authored pivot');
assert.match(witness, /F did not frame the corrected splat authored pivot, rendered root, and complete cloud bounds/, 'corrected splat witness fails if F crops any required framing subject');
assert.match(witness, /fixtureSha256/, 'corrected splat witness records the exact synthetic PLY bytes');
...
assert.match(witness, /selected pivot is outside the view/, 'pivot witness distinguishes an offscreen pivot from a hidden object');
assert.match(witness, /pivot warning overlaps viewport status/, 'pivot witness fails if diagnosis obscures the viewport status');
assert.match(witness, /splat-asset-link/, 'witness supports a direct PLY asset smoke-link scenario');
assert.match(witness, /image-asset-link/, 'witness supports a direct image asset smoke-link scenario');
