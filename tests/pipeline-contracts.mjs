...
try {
  ...
  assert.equal(liveMissingReport.stages[0].effectiveRoute.pipelineScheduler.scheduler.requestedScheduler.mode, 'default');
  assert.equal(liveMissingReport.stages[0].effectiveRoute.pipelineScheduler.scheduler.effectiveScheduler, null);
...
`);
  ...
  assert.equal(liveReport.stages[0].effectiveRoute.adapterReport.pipelineScheduler.verificationState, 'verified');
  ...
  assert.equal(liveReport.stages[0].effectiveRoute.adapterReport.pipelineScheduler.scheduler.requestedScheduler.spnPatchChunkSize, 1);
  assert.equal(liveReport.stages[0].effectiveRoute.adapterReport.pipelineScheduler.scheduler.effectiveScheduler.spnPatchChunkSize, 1);
  assert.deepEqual(liveReport.stages[0].effectiveRoute.adapterReport.pipelineScheduler.scheduler.unsupportedFields, ['vitBlockChunkSize']);
