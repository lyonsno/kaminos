...
for (const [pattern, message] of [
  ...
  [/const timer = timeoutMs === null[\s\S]*setTimeout\(\(\) => reject\(new Error\(`/, 'CDP requests must disable their timer when the caller explicitly selects an uncapped wait'],
  [/phase = 'starting-friendly-firing'[\s\S]*window\.__kaminosWitnessFiringPromise[\s\S]*\}\)\(\)`, fireOperationTimeoutMs\);/, 'The firing-start evaluation must consume the caller-owned deadline, including explicit uncapped mode'],
  [/async function evaluate\(ws, expression, timeoutMs[\s\S]*wsRequest\(ws, 'Runtime\.evaluate',[\s\S]*timeoutMs\)[\s\S]*const browserFiringEvidence = await evaluate\(ws,[\s\S]*fireOperationTimeoutMs\)/, 'Post-firing browser evidence collection must preserve the caller deadline or explicit uncapped wait'],
