...
assert.match(webgpuCoreSource, /function failFingerFluidInitialization\([\s\S]*movingHillSupportProvider\?\.release\?\.\(\);[\s\S]*device\.destroy\(\);[\s\S]*hdrEnvironment = await loadFingerFluidHdrEnvironment\(\);[\s\S]*catch \(error\) \{\s*return failFingerFluidInitialization\(`HDR environment initialization failed:/, 'pre-output HDR initialization failure releases the moving-support provider and acquired WebGPU device');
