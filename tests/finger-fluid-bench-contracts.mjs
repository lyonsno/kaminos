...
assert.match(webgpuCoreSource, /fn analytic_boundary_density_support\(position: vec3<f32>, radiusScale: f32\) -> vec4<f32>/, 'terrain and sphere boundary support share one analytic density/gradient function scaled to particle volume');
assert.match(computeDensityLambdaSource, /let boundarySupport = analytic_boundary_density_support\(position, selfRadiusScale\)[\s\S]*density = density \+ boundarySupport\.w[\s\S]*gradientSelf = gradientSelf \+ boundarySupport\.xyz/, 'analytic boundary density and gradient enter the lambda solve at the represented particle scale');
...
assert.match(webgpuCoreSource, /fn solve_position_delta[\s\S]*let boundarySupport = analytic_boundary_density_support\(position, adaptive_radius_scale\(index\)\)[\s\S]*correction = correction \+ lambda \* boundarySupport\.xyz/, 'analytic boundary support enters position correction at the represented particle scale before collision fallback');
...
assert.match(webgpuCoreSource, /function failFingerFluidInitialization\([\s\S]*failFingerFluidWebGPUInitialization\(\{[\s\S]*provider:\s*movingHillSupportProvider,[\s\S]*device,[\s\S]*reason,[\s\S]*details,[\s\S]*hdrEnvironment = await loadFingerFluidHdrEnvironment\(\);[\s\S]*catch \(error\) \{\s*return failFingerFluidInitialization\(`HDR environment initialization failed:/, 'pre-output HDR initialization failure delegates moving-support and device cleanup to the shared counted primitive');
...
assert.match(webgpuCoreSource, /refractionEvidence:\s*\{[\s\S]*supportDepthRoute:\s*safePresentationMode === KAMINOS_FINGER_FLUID_MOVING_HILL_PRESENTATION_MODE\s*\?\s*null\s*:\s*KAMINOS_FINGER_FLUID_ANALYTIC_SUPPORT_DEPTH_ROUTE[\s\S]*analyticSupportDepthPassCount/, 'refraction evidence names analytic support depth only when that support was actually presented');
assert.match(webgpuCoreSource, /supportPresentationEvidence:\s*\{[\s\S]*route:\s*safePresentationMode === KAMINOS_FINGER_FLUID_MOVING_HILL_PRESENTATION_MODE\s*\?\s*KAMINOS_FINGER_FLUID_MOVING_HILL_PRESENTATION_ROUTE\s*:\s*KAMINOS_FINGER_FLUID_ANALYTIC_SUPPORT_PRESENTATION_ROUTE[\s\S]*colorDepthAuthority:\s*safePresentationMode === KAMINOS_FINGER_FLUID_MOVING_HILL_PRESENTATION_MODE\s*\?\s*'consumer_scene_color_depth_not_bound_v0'\s*:\s*'same_pass_same_analytic_geometry_v0'[\s\S]*particleSupportDrawCount/, 'runtime evidence distinguishes the no-toy moving-Hill consumer route from the analytic support presentation');
...
assert.match(webgpuCoreSource, /geometrySource:\s*safePresentationMode === KAMINOS_FINGER_FLUID_MOVING_HILL_PRESENTATION_MODE\s*\?\s*null\s*:\s*'toyFloorHeight_toyFloorNormal_plus_analytic_obstacle_v0'/, 'the exact solver playground and obstacle are named only for analytic support presentation');
