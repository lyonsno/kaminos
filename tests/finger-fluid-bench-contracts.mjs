...
assert.match(webgpuCoreSource, /computeChemistry:\s*'compute_material_tracer_diffusion'/, 'full-bench solver compiles passive tracer diffusion');
assert.match(webgpuCoreSource, /applyChemistry:\s*'apply_material_tracer_diffusion'/, 'full-bench solver compiles the separate tracer application pass');
assert.match(webgpuCoreSource, /if \(safeRuntimeProfile\.compileChemistry && safeChemistryDiffusion > 0\) \{[\s\S]*dispatch\(pass, pipelines\.computeChemistry, safeParticleCount\);[\s\S]*dispatch\(pass, pipelines\.applyChemistry, safeParticleCount\);[\s\S]*chemistryDiffusionPassCount \+= 2/, 'zero diffusion and live-play profiles dispatch no hidden chemistry work while enabled full-bench diffusion crosses two passes');
...
assert.match(webgpuCoreSource, /clearLiquidFireContacts:\s*'clear_liquid_fire_contact_descriptor'/, 'each full-bench write tick compiles GPU invalidation for the public contact descriptor');
assert.match(webgpuCoreSource, /compactLiquidFireContacts:\s*'compact_liquid_fire_contacts'/, 'full-bench contact-qualified liquid records are compacted into the public ABI on GPU');
assert.match(webgpuCoreSource, /finalizeLiquidFireContacts:\s*'finalize_liquid_fire_contact_descriptor'/, 'a distinct full-bench producer completion dispatch seals the descriptor after compaction');
...
assert.match(webgpuCoreSource, /clearInterface:\s*'clear_interface_counters'/, 'full-bench interface counters are cleared on GPU before compaction');
assert.match(webgpuCoreSource, /compactInterface:\s*'compact_interface_records'/, 'qualifying full-bench interface particles are compacted on GPU');
...
assert.match(webgpuCoreSource, /predict:\s*'predict_positions'/, 'solver predicts 3D positions on GPU');
assert.match(webgpuCoreSource, /lambda:\s*'compute_density_lambda'/, 'solver computes PBF density lambdas');
assert.match(webgpuCoreSource, /delta:\s*'solve_position_delta'/, 'solver computes density position corrections');
assert.match(webgpuCoreSource, /applyDelta:\s*'apply_position_delta'/, 'solver applies density position corrections');
assert.match(webgpuCoreSource, /vorticity:\s*'compute_vorticity'/, 'solver computes neighbor-derived curl after projection');
assert.match(webgpuCoreSource, /confinement:\s*'apply_vorticity_confinement'/, 'solver applies a separate vorticity confinement pass');
assert.match(webgpuCoreSource, /classifySurface:\s*'classify_free_surface'/, 'solver classifies the neighbor-derived free surface');
assert.match(webgpuCoreSource, /cohesion:\s*'apply_surface_cohesion'/, 'solver applies bounded cohesion to classified interface particles');
...
assert.deepEqual(webgpuMod.KAMINOS_FINGER_FLUID_TRUTH_SCENES, ['multi_regime_playground', 'deep_pool_rest', 'dam_break', 'laminar_inlets', 'live_hand_inlets']);
