...
assert.match(witness, /--material-modes/, 'clay witness can render multiple geometry material modes in one contact sheet');
assert.match(witness, /glb_material=clay/, 'clay witness loads each GLB through the clay material override route');
assert.match(witness, /clay-front/, 'clay witness can request a front-face-only material diagnostic');
assert.match(witness, /clay-back/, 'clay witness can request a back-face-only material diagnostic');
assert.match(witness, /normal/, 'clay witness can request a normal-material diagnostic');
...
assert.match(witness, /direct-glb-geometry-material-load/, 'clay witness asks the browser witness to verify non-clay geometry material override state');
...
assert.match(witness, /contactSheetHeight/, 'clay witness sizes screenshot capture to include every contact-sheet row');
assert.match(witness, /--window-size=1600,\$\{contactSheetHeight\}/, 'clay witness passes the computed contact-sheet height into headless Chrome');
