...
assert.match(core, /let requestedBudget = boundarySplatDrawGroups\[groupIndex\]\.requestedCandidateBudget;\s*let effectiveBudget = select\(candidateCount,\s*min\(candidateCount,\s*requestedBudget\),\s*requestedBudget > 0u\)/, 'finalize pass preserves each tier source unless it clamps an explicit positive budget');
