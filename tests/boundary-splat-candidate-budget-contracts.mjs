...
assert.match(core, /BOUNDARY_SPLAT_SELECTOR_POLICY_IDENTITY\s*=\s*'boundary-splat-nested-permutation-prefix-v0'/, 'nested permutation selector policy identity is explicit');
assert.match(core, /BOUNDARY_SPLAT_SELECTOR_BUDGETS\s*=\s*\[0,\s*12800,\s*6400,\s*3200,\s*1600,\s*800\]/, 'runtime preserves an uncapped default alongside the exact adaptive budget ladder');
...
assert.match(core, /fn boundarySplatNestedSourceIndex\(rank:\s*u32,\s*sourceCount:\s*u32\)/, 'WGSL owns deterministic nested permutation source-index selection');
assert.match(core, /let requestedBudget = boundarySplatDrawGroups\[groupIndex\]\.requestedCandidateBudget;\s*let requestedEffectiveBudget = select\(candidateCount,\s*min\(candidateCount,\s*requestedBudget\),\s*requestedBudget > 0u\);\s*let effectiveBudget = select\(0u,\s*requestedEffectiveBudget,\s*hasDescriptors\)/, 'finalize pass preserves each populated tier source unless it clamps an explicit positive budget, while empty groups remain zero');
assert.match(core, /boundarySplatDraw\.selectedCandidateCount = archiveCandidateCount/, 'finalize pass records the largest shared archive prefix separately from global raster work');
assert.match(core, /atomicStore\(&boundarySplatDraw\.instanceCount,\s*totalRenderedInstanceCount\)/, 'indirect accounting is the sum of effective tier budgets times their descriptor counts');
assert.match(core, /archiveBoundarySplatHistory[\s\S]*let selectedSourceCandidateIndex = boundarySplatNestedSourceIndex\(candidateIndex,\s*boundarySplatDraw\.sourceCandidateCount\)[\s\S]*boundarySplatHistory\[targetIndex\] = boundarySplats\[selectedSourceCandidateIndex\]/, 'archive pass maps nested ranks onto deterministic source candidates without CPU readback');
...
assert.match(witness, /boundarySplatBudgetVisualEvidence[\s\S]*boundary-splat-nested-permutation-prefix-v0[\s\S]*budgeted learned-splat volume signal/, 'witness has a narrow nonblank visual gate for sparse budgeted learned-splat rows');
