...
assert.match(core, /BOUNDARY_SPLAT_SELECTOR_BUDGETS\s*=\s*\[6400,\s*3200,\s*1600,\s*800\]/, 'first-slice budget ladder is fixed to the requested exact counts');
...
assert.match(core, /let effectiveBudget = min\(candidateCount,\s*boundarySplatDraw\.requestedCandidateBudget\)/, 'finalize pass clamps requested budget into an explicit effective budget');
