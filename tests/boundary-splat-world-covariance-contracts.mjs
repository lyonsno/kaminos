...
assert.match(coreSource, /coefficientPayloadSha256/, 'GPU audit must separately hash frozen color and opacity coefficients');
assert.match(coreSource, /covariancePayloadSha256/, 'GPU audit must separately hash frozen covariance attributes');
assert.match(
  coreSource,
...
assert.deepEqual(canonicalForward.coefficients, canonicalPermuted.coefficients, 'coefficients must remain paired with their canonical candidate row');
assert.deepEqual(canonicalForward.covariance, canonicalPermuted.covariance, 'covariance must remain paired with its canonical candidate row');
assert.deepEqual([...canonicalForward.coefficients], [21, 22, 23, 24, 1, 2, 3, 4]);
assert.deepEqual([...canonicalForward.covariance], [25, 26, 27, 28, 29, 30, 31, 32, 5, 6, 7, 8, 9, 10, 11, 12]);
