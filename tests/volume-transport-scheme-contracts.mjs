...
test('runtime receipt, predictor dispatch, and cost ledger name the transport scheme', () => {
  ...
  assert.match(source, /label: `kaminos first fluid sim compute pipeline \$\{gridShapeLabel\(gridSize\)\}`,\s*layout: transportPipelineLayout,/, 'main sim pipeline uses the transport layout');
});
