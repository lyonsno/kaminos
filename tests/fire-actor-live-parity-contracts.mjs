...
const workbenchPath = join(root, 'fire-actor-live-parity.html');
assert.ok(existsSync(workbenchPath), 'two-surface live parity workbench must exist');
const workbench = readFileSync(workbenchPath, 'utf8');
assert.match(workbench, /data-arm="splats"/);
assert.match(workbench, /data-arm="smoke"/);
assert.match(workbench, /data-arm="composite"/);
assert.match(workbench, /data-command="play"/);
assert.match(workbench, /data-command="pause"/);
assert.match(workbench, /Sync cockpit camera/);
assert.doesNotMatch(workbench, /searchParams\.set\('settings_preset'/, 'workbench must not require producer-local preset installation');
assert.match(workbench, /window\.kaminosFireActorParityWorkbench\s*=/, 'browser witness can exercise the same workbench commands as the operator');

const witnessPath = join(root, 'fire-actor-live-parity-witness.mjs');
assert.ok(existsSync(witnessPath), 'live parity workbench requires a reusable browser witness');
const witness = readFileSync(witnessPath, 'utf8');
assert.match(witness, /Page\.captureScreenshot/, 'live parity witness captures operator-visible pixels');
assert.match(witness, /\['splats', 'smoke', 'composite'\]/, 'live parity witness exercises all three presentation arms');
assert.match(witness, /finally\s*\{[\s\S]*writeReport/, 'live parity witness preserves a report after early failure');
