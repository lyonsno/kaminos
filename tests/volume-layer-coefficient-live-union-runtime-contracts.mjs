...
const selectiveSplatGroup = core.match(/splat: device\.createBindGroup\(\{[\s\S]*?label: `kaminos \$\{SELECTIVE_HEAD_LIVE_ROUTE\} \$\{role\} splat`[\s\S]*?entries: \[([\s\S]*?)\]\s*,?\n\s*\}\),/)?.[1] || '';
assert.match(selectiveSplatGroup, /binding: 12, resource: \{ buffer: boundarySplatBilinearIndirectBuffer \}/, 'selective-head splat compute groups bind the bilinear indirect draw state');
