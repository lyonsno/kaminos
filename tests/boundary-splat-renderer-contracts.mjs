...
assert.match(core, /fn boundarySplatEnergyCompensation\(footprintRadius: f32, sharpness: f32\)[\s\S]*let energyRatio = \(sharpness \/ 3\.4\) \/ max\(footprintRadius \* footprintRadius,[\s\S]*clamp\(sqrt\(energyRatio\), 0\.5, 2\.5\)/, 'radius and kernel compensation preserves the bounded legacy energy law in one shared shader helper');
assert.match(core, /let footprintRadius = clamp\(boundarySplatCamera\.controls\.x,[\s\S]*boundarySplatEnergyCompensation\(footprintRadius, kernelSharpness\)[\s\S]*in\.colorOpacity\.a \* gaussian \* energyCompensation/, 'the fragment kernel applies the shared radius and sharpness energy compensation');
...
assert.match(core, /sampleFrame[\s\S]*encodeBoundarySidecar\(encoder,\s*\{\s*readBindGroup:[\s\S]*encodeBoundarySplats\(encoder,\s*\{\s*computeBindGroup:[\s\S]*if \(composition\.definition\.splat\)[\s\S]*encodeBoundarySplatPresentation\(\s*encoder,\s*frameTexture\.createView\(\),\s*boundarySplatReadbackPipeline,\s*boundarySplatPresentationReadbackPipeline,[\s\S]*loadOp:\s*raymarchApplied\s*\?\s*'load'\s*:\s*'clear'/, 'frozen witness renders the requested selective-head splat composition through the selected presentation arm instead of substituting raymarch');
assert.match(core, /renderFrozenScaleToCanvas[\s\S]*encodeBoundarySidecar\(encoder\)[\s\S]*encodeBoundarySplats\(encoder\)[\s\S]*if \(compositionDefinition\.splat\)[\s\S]*encodeBoundarySplatPresentation\(\s*encoder,\s*currentTexture\.createView\(\),\s*boundarySplatRenderPipeline,\s*boundarySplatPresentationRenderPipeline,[\s\S]*loadOp:\s*raymarchEncoded\s*\?\s*'load'\s*:\s*'clear'/, 'controlled canvas capture renders the requested selective-head splat composition through the selected presentation arm instead of substituting raymarch');
...
assert.match(frozenRenderFunction, /compositionDefinition\.splat[\s\S]*encodeBoundarySplatPresentation\([\s\S]*loadOp:\s*raymarchEncoded\s*\?\s*'load'\s*:\s*'clear'/, 'frozen capture composites splats over raymarch only for compositions that request both passes');
