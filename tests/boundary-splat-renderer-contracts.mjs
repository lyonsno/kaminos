...
assert.match(core, /let footprintRadius = clamp\(boundarySplatCamera\.controls\.x,[\s\S]*boundarySplatEnergyCompensation\(footprintRadius, kernelSharpness\)[\s\S]*in\.colorOpacity\.a \* boundarySplatCamera\.appearance\.y \* gaussian \* energyCompensation/, 'the fragment kernel applies the explicit opacity gain and shared radius and sharpness energy compensation');
...
assert.match(core, /sampleFrame[\s\S]*encodeBoundarySidecar\(encoder,\s*\{\s*readBindGroup:[\s\S]*encodeBoundarySplats\(encoder,\s*\{\s*computeBindGroup:[\s\S]*if \(composition\.definition\.splat\)[\s\S]*encodeBoundarySplatDraw\(\s*encoder,\s*frameTexture\.createView\(\),\s*boundarySplatReadbackPipeline,[\s\S]*loadOp:\s*raymarchApplied\s*\?\s*'load'\s*:\s*'clear'/, 'frozen witness renders the requested selective-head splat composition instead of substituting raymarch');
assert.match(core, /renderFrozenScaleToCanvas[\s\S]*encodeBoundarySidecar\(encoder\)[\s\S]*encodeBoundarySplats\(encoder\)[\s\S]*if \(compositionDefinition\.splat\)[\s\S]*encodeBoundarySplatDraw\(\s*encoder,\s*currentTexture\.createView\(\),\s*boundarySplatRenderPipeline,[\s\S]*loadOp:\s*raymarchEncoded\s*\?\s*'load'\s*:\s*'clear'/, 'controlled canvas capture renders the requested selective-head splat composition instead of substituting raymarch');
