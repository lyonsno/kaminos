...
assert.match(core, /sampleFrame[\s\S]*encodeBoundarySidecar\(encoder[\s\S]*encodeBoundarySplats\(encoder[\s\S]*encodeSelectiveHeadBoundarySplatComposition\(\s*encoder,\s*frameTexture\.createView\(\),\s*boundarySplatReadbackPipeline,[\s\S]*targetPbrPipeline:\s*boundarySplatPbrSceneReadbackPipeline[\s\S]*\)/, 'frozen witness renders the requested splat route through its format-matched PBR-aware composition path instead of substituting raymarch');
const sampleFrameFunction = core.match(/async function sampleFrame\(options = \{\}\) \{[\s\S]*?\n  \}\n\n  function compactRenderScaleSample/)?.[0] || '';
assert.match(sampleFrameFunction, /options\.boundarySplatComposition[\s\S]*selectiveHeadLiveRenderCompositionRequest/, 'sampleFrame accepts an explicit public splat composition when no selective-head role owns the live route');
assert.match(sampleFrameFunction, /boundarySplatReadbackCompositionEffective/, 'sampleFrame reports the effective explicit readback composition');
assert.match(sampleFrameFunction, /boundarySplatReadbackPassReceipt/, 'sampleFrame reports which raymarch and splat passes were actually submitted');
assert.match(sampleFrameFunction, /exactStateCadenceReadbackRequested[\s\S]*encodeExactStateCadencePresentation\(encoder, sampleNow\)[\s\S]*exactStateCadencePresentationBindGroups/, 'render-only cadence readback consumes the delayed interpolated presentation buffers instead of current simulator buffers');
assert.match(sampleFrameFunction, /exactStateCadenceReadbackApplied[\s\S]*exactStateCadenceReadbackReceipt/, 'sampleFrame reports exact cadence presentation authority for its readback');
