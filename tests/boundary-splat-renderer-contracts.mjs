...
assert.match(core, /renderFrozenScaleToCanvas[\s\S]*encodeBoundarySidecar\(encoder\)[\s\S]*encodeBoundarySplats\(encoder\)[\s\S]*encodeBoundarySplatPbrScene\(encoder,\s*currentTexture\.createView\(\),\s*boundarySplatPbrDepthTexture\.createView\(\)\)[\s\S]*encodeBoundarySplatDraw\(encoder,\s*currentTexture\.createView\(\),\s*boundarySplatRenderPipeline,\s*\{[\s\S]*depthView:\s*boundarySplatPbrDepthTexture\.createView\(\)[\s\S]*loadColor:\s*pbrSceneApplied[\s\S]*loadDepth:\s*pbrSceneApplied/, 'controlled canvas capture renders the requested PBR scene and splats through one shared color/depth target instead of substituting raymarch');
