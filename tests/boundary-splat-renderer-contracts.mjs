...
assert.match(core, /const phaseSourceCount = Math\.max\(1, Math\.min\(BOUNDARY_SPLAT_HISTORY_SLOTS, state\.boundarySplatPhaseSourceCount \|\| 1\)\)[\s\S]*new Uint32Array\(\[\s*6,\s*0,\s*0,\s*0,\s*0,\s*0,\s*boundarySplatCapacity,\s*requestedInstanceCount,\s*0,\s*phaseSourceCount,\s*historyWriteSlot,\s*BOUNDARY_SPLAT_HISTORY_SLOTS,\s*\]\)/, 'each compaction pass publishes effective capacity, requested instance count, descriptor-derived phase-source count, and history-ring identity to WGSL');
