...
assert.match(core, /new Uint32Array\(\[\s*6,\s*0,\s*0,\s*0,\s*0,\s*0,\s*boundarySplatCapacity,\s*requestedInstanceCount,\s*0,\s*Math\.min\(requestedInstanceCount,\s*BOUNDARY_SPLAT_HISTORY_SLOTS\),\s*historyWriteSlot,\s*BOUNDARY_SPLAT_HISTORY_SLOTS,\s*\]\)/, 'each compaction pass publishes effective capacity, requested instance count, phase-source count, and history-ring identity to WGSL');
