...
assert.match(core, /sampleFrame[\s\S]*encodeBoundarySidecar\(encoder(?:,\s*\{[\s\S]*?\})?\)[\s\S]*encodeBoundarySplats\(encoder(?:,\s*\{[\s\S]*?\})?\)[\s\S]*encodeBoundarySplatDraw\(\s*encoder,\s*frameTexture\.createView\(\),\s*boundarySplatReadbackPipeline[\s\S]*?\)/, 'frozen witness renders the requested splat route instead of substituting raymarch');
assert.match(core, /renderFrozenScaleToCanvas[\s\S]*encodeBoundarySidecar\(encoder(?:,\s*\{[\s\S]*?\})?\)[\s\S]*encodeBoundarySplats\(encoder(?:,\s*\{[\s\S]*?\})?\)[\s\S]*encodeBoundarySplatDraw\(\s*encoder,\s*currentTexture\.createView\(\)[\s\S]*?\)/, 'controlled canvas capture renders the requested splat route instead of substituting raymarch');
