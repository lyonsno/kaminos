...
assert.match(index, /function applyMotionPanelHillTerrainGroundPlaneSuppression/, 'Hill terrain smoke explicitly suppresses the default scene ground plane instead of visually fighting it');
...
assert.match(index, /smoothedCarrierRoot/, 'carrier path follower records the smoothed rendered root separately from terrain contact authority');
assert.match(index, /smoothedCarrierHeading/, 'carrier path follower records damped heading evidence separately from raw route tangent');
assert.match(index, /carrierSmoothing/, 'actor/debug evidence exposes carrier smoothing state');
assert.match(index, /carrierCadence/, 'actor/debug evidence exposes bounded generated-motion cadence state');
assert.match(index, /cadenceSource/, 'carrier cadence evidence names the generated temporal source used as local affect');
assert.match(index, /contactPolicy:\s*'terrain-clamped-local-affect'/, 'carrier cadence evidence states that cadence is terrain-clamped local affect, not travel authority');
assert.match(index, /airborneGrant:\s*false/, 'carrier path follower forbids generated motion from granting airborne travel');
assert.match(index, /airborneSuppressed:\s*true/, 'carrier cadence evidence explicitly suppresses airborne authority from generated motion');
...
assert.match(liveWitness, /carrierSmoothing/, 'live witness records Hill carrier smoothing evidence per frame');
