...
assert.match(route, /nativeGrid:\s*96/, 'manual route reports the effective native grid');
...
assert.match(route, /roles:\s*\{[\s\S]*native96Control[\s\S]*baseline128Trained[\s\S]*candidate96Trained/, 'manual receipt exposes normalized three-role identities');
