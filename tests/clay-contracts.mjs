...
assert.match(index, /clay_sculpt/, 'clay route accepts the first particle sculpting route');
assert.match(index, /clay_sculpt_particles/, 'clay route accepts an explicit particle sculpting preset');
...
assert.match(core, /export function normalizeClaySculptConfig/, 'clay module exports sculpt particle config normalization');
assert.match(core, /export function seedClaySculptParticles/, 'clay module exports deterministic sculpt particle seeding');
...
assert.match(core, /CLAY_CUBE_CELL_VOLUME_PRESERVATION_POLICY/, 'clay core names real cube cell-volume preservation policy');
assert.match(core, /measureClayCubeCellVolumeMetrics/, 'clay core exposes structured cube cell-volume metrics');
assert.match(core, /applyClayCubeCellVolumeConstraints/, 'clay core exposes structured cube cell-volume projection');
assert.match(core, /structured-hexa-cell-volume-projection-js-postprocess-v0/, 'clay core labels first real volume-cell projection honestly');
...
assert.match(core, /clayCubeCellVolumeEvidenceKind/, 'clay debug state records cell-volume metric evidence kind');
assert.match(core, /clayCubeCellVolumeMeanRelativeErrorBefore/, 'clay debug state records pre-projection mean cell-volume error');
assert.match(core, /clayCubeCellVolumeMeanRelativeErrorAfter/, 'clay debug state records post-projection mean cell-volume error');
assert.match(core, /clayCubeCellVolumeMaxRelativeErrorBefore/, 'clay debug state records pre-projection max cell-volume error');
assert.match(core, /clayCubeCellVolumeMaxRelativeErrorAfter/, 'clay debug state records post-projection max cell-volume error');
assert.match(core, /clayCubeVolumeCellConstraintIterationCount/, 'clay debug state records volume-cell projection iterations');
assert.match(core, /clayCubeVolumeCellConstrainedCellCount/, 'clay debug state records constrained volume-cell count');
