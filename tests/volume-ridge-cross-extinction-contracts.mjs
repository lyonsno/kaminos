...
assert.match(wrapper, /const rendererSettled\s*=\s*warmupComplete[\s\S]*&&\s*state\?\.active[\s\S]*&&\s*\(frozenAnchorSettled\s*\|\|\s*\(effectiveRole\s*===\s*requestedRole[\s\S]*presentationSettled\)\)/, 'frozen-anchor admission does not depend on unrendered effective role state');
