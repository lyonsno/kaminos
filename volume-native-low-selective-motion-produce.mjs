...
const MODEL_IDENTITY = 'exact-basin-selective-carrier-heads-160-to-128-v0';
const ROLES = ['nativeLowControl', 'nativeLowSelectivePredicted'];
...
try {
  ...
  if (routeGrid !== 128 || controlGrid !== 128) {
    throw new Error(`native-low motion witness requires a 128-grid source capture, got route=${routeGrid} controls=${controlGrid}`);
  }

  failurePhase = 'model-validation';
  const modelRaw = readFileSync(modelManifestPath);
  const model = JSON.parse(modelRaw);
  if (model.identity !== MODEL_IDENTITY || model.status !== 'captured' || model.failurePhase !== null) {
  ...
  const baseManifest = {
    ...
    frameCount,
    roles: ROLES,
    renderCompositionRequested: RENDER_COMPOSITION,
    viewportSize,
    renderCanvasSize,
    browserSession: {
  ...
  if (planOnly) {
    ...
    const witness = assembleWitness(frames, sequenceIdentity);
    activeManifest = {
      ...baseManifest,
      status: 'captured',
...
function buildFramePlan(context) {
  ...
  return {
    ...
    roles: ROLES,
    paths: { scratchRoot, captureRoot, nativeDir, predictedDir, renderDir },
    commands: {
    ...
    argv: { nativeExport: nativeArgs, compose: composeArgs, render: renderArgs },
  };
}
...
function executeFrame(frame, identity) {
  ...
  if (native.status !== 'captured' || native.failurePhase !== null || native.grid !== 128) {
  ...
  if (rendered.status !== 'captured' || rendered.failurePhase !== null
    ...
    || rendered.renderer?.requested !== RENDER_COMPOSITION
    || rendered.renderer?.controlEffective !== RENDER_COMPOSITION
    || rendered.renderer?.treatmentEffective !== RENDER_COMPOSITION
    || rendered.renderer?.raymarchExcludedFromDiscriminant !== true) {
    throw new Error('native frame render route or state identity mismatch');
  }

  const retainedRoles = {};
  for (const role of ROLES) {
    const sourceImage = resolve(String(rendered.roles?.[role]?.image?.path || ''));
    ...
    if (imageSha256 !== rendered.roles[role].image.sha256) throw new Error(`${role} retained image checksum mismatch`);
    retainedRoles[role] = {
      ...rendered.roles[role],
      image: { ...rendered.roles[role].image, path: retainedPath, sha256: imageSha256, byteLength: bytes.byteLength },
    };
  }
  ...
  const frameManifest = {
    ...
    identity: 'matched-native-low-control-selective-predicted-splat-frame-v0',
    status: 'captured',
    failurePhase: null,
...
function assembleWitness(frames, sequenceIdentity) {
  ...
  writeFileSync(htmlPath, htmlPage(records, sequenceIdentity));
  return {
    ...
    requestedComposition: RENDER_COMPOSITION,
    effectiveComposition: RENDER_COMPOSITION,
    raymarchExcludedFromDiscriminant: true,
  };
}

function htmlPage(records, sequenceIdentity) {
...
:root{color-scheme:dark;font-family:Inter,system-ui,sans-serif;background:#07090a;color:#eef2f3}*{box-sizing:border-box}body{margin:0;background:#07090a}header{height:58px;padding:10px 16px;display:flex;align-items:center;gap:16px;border-bottom:1px solid #30383a;background:#111516}h1{font-size:16px;margin:0;white-space:nowrap}.controls{display:flex;align-items:center;gap:8px;margin-left:auto}button,select,input{accent-color:#e06c43}button,select{height:34px;border:1px solid #414b4e;background:#1b2224;color:#edf2f3;padding:0 11px}button{cursor:pointer}.readout{font:12px ui-monospace,monospace;color:#acb8bb;min-width:180px}.grid{height:calc(100vh - 58px);display:grid;grid-template-columns:1fr 1fr;gap:2px;background:#30383a}.panel{min-width:0;min-height:0;background:#000;display:grid;grid-template-rows:42px 1fr}.label{padding:8px 12px;background:#101415}.label strong{font-size:14px}.label span{display:block;font-size:10px;color:#9daaad;margin-top:2px}.image-wrap{min-height:0;display:flex;align-items:center;justify-content:center;overflow:hidden}.image-wrap img{display:block;max-width:100%;max-height:100%;width:auto;height:auto;image-rendering:auto}@media(max-width:850px){header{height:auto;flex-wrap:wrap}.controls{margin-left:0}.grid{height:auto;grid-template-columns:1fr}.panel{height:70vh}}
</style></head><body><header><h1>Native 128 vs frozen 128-to-160 reconstruction</h1><div class="readout" id="readout"></div><div class="controls"><button id="play" type="button">Pause</button><label>Speed <select id="speed"><option value="16.6667">1x</option><option value="33.3333" selected>0.5x</option><option value="66.6667">0.25x</option></select></label><input id="scrub" type="range" min="0" max="${records.length - 1}" value="0" step="1"></div></header><main class="grid"><section class="panel"><div class="label"><strong>Native-low control</strong><span>Untouched native 128^3 state, splat-only</span></div><div class="image-wrap"><img id="control" alt="Native-low control"></div></section><section class="panel"><div class="label"><strong>Native-low selective predicted</strong><span>Frozen heads applied to the same native state, 160^3 splat-only</span></div><div class="image-wrap"><img id="treatment" alt="Native-low selective predicted"></div></section></main><script>
const frames=${JSON.stringify(frames)};const sequence=${JSON.stringify(sequenceIdentity)};let index=0,playing=true,timer=null;const control=document.querySelector('#control'),treatment=document.querySelector('#treatment'),readout=document.querySelector('#readout'),scrub=document.querySelector('#scrub'),play=document.querySelector('#play'),speed=document.querySelector('#speed');function show(next){index=(next+frames.length)%frames.length;const frame=frames[index];control.src=frame.control;treatment.src=frame.treatment;scrub.value=String(index);readout.textContent='frame '+(index+1)+'/'+frames.length+' | step '+frame.step+' | support '+(100*frame.prevalence).toFixed(2)+'%';}function schedule(){clearInterval(timer);if(playing)timer=setInterval(()=>show(index+1),Number(speed.value));}play.addEventListener('click',()=>{playing=!playing;play.textContent=playing?'Pause':'Play';schedule();});speed.addEventListener('change',schedule);scrub.addEventListener('input',()=>show(Number(scrub.value)));show(0);schedule();console.info({sequence,frameCount:frames.length,composition:'splat-only-v0',raymarchApplied:false});
</script></body></html>`;
}
