...
const rows={rows},bandwidths={bandwidth_json},fields=[...new Set(rows.map(r=>r.field))],$=id=>document.getElementById(id);for(const value of fields)$('field').add(new Option(value,value));for(const value of bandwidths)$('bandwidth').add(new Option(String(value),String(value)));function render(){{const field=$('field').value,bw=+$('bandwidth').value,axis=$('axis').value,mode=$('mode').value,row=rows.find(r=>r.field===field&&r.bandwidthCells===bw&&r.axis===axis&&r.mode===mode);$('image').src=row?row.image:''}}for(const id of ['field','bandwidth','axis','mode'])$(id).addEventListener('input',render);render();</script></body></html>"""
