...
function hasVerifiedDecoderKernelProof(verification, expectedChunkItems) {
  const assertion = (verification?.boundaryAssertions || []).find(candidate =>
    candidate?.field === 'decoderKernelChunkItems'
  );
  return assertion?.status === 'verified'
    && assertion.requested === expectedChunkItems
    && assertion.effective === expectedChunkItems
    && Number.isInteger(assertion.observedCount)
    && assertion.observedCount >= 2
    && Number.isInteger(assertion.observedKernel?.tileTotal)
    && assertion.observedKernel.tileTotal >= 2;
}
...
export function createSharpBreathingRoomComparison({
  ...
  if (cooperativeVerification?.status === 'verified'
    && !hasVerifiedDecoderKernelProof(cooperativeVerification, expectedDecoderChunkItems)) {
