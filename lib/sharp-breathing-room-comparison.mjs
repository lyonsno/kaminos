...
} from './decoder-kernel-tiling-evidence.mjs';
    ...
    profiles: [
      ...
      },
      {
        ...
        proofExpectation: {
          ...
          requiredBoundaries: ['phaseChunkSize.spnFusionOutputItems', 'decoderKernelChunkItems'],
...
export function createSharpBreathingRoomComparison({
  ...
  const state = {
    ...
    falseClosureChecks: {
      ...
      decoderKernelProofMissing: false,
    },
  };
  ...
  const expectedDecoderScheduler = sharpBreathingRoomComparisonProfiles().profiles
    .find(profile => profile.id === 'cooperative-spn-gaussian')
    ?.scheduler;
  const cooperativeDecoderKernelTileEvents = decoderKernelTileEventsFromSchedulerEvents(
    cooperativeVerification?.eventTrace?.events || [],
  );
  const cooperativeDecoderKernelTileFailures = validateDecoderKernelTileEvidence({
    expectedChunkItems: expectedDecoderScheduler?.decoderKernelChunkItems,
    expectedAdaptivePolicy: {
      minChunkItems: expectedDecoderScheduler?.decoderKernelMinChunkItems,
      maxChunkItems: expectedDecoderScheduler?.decoderKernelMaxChunkItems,
      targetDurationMs: expectedDecoderScheduler?.decoderKernelTargetDurationMs,
      adjustmentGain: expectedDecoderScheduler?.decoderKernelAdjustmentGain,
    },
    boundaryAssertions: cooperativeVerification?.boundaryAssertions || [],
    tileEvents: cooperativeDecoderKernelTileEvents,
  });
  if (cooperativeVerification?.status === 'verified'
    && cooperativeDecoderKernelTileFailures.length) {
    state.falseClosureChecks.decoderKernelProofMissing = true;
    addDowngrade(state, 'cooperative-decoder-kernel-proof-missing');
  }
  ...
  const invalidDowngrades = new Set([
    ...
    'cooperative-decoder-kernel-proof-missing',
  ]);
  ...
  const report = {
    ...
    schedulerComparison: {
      ...
      cooperative: {
        ...
        decoderKernelTileEvents: cooperativeDecoderKernelTileEvents,
        decoderKernelTileFailures: cooperativeDecoderKernelTileFailures,
  ...
  const effectiveStatus = hasInvalidDowngrade ? 'invalid' : validation.status;
  const effectiveDowngrades = [...new Set([...state.downgrades, ...(validation.downgrades || [])])];
  const effectiveCanClaim = hasInvalidDowngrade
    ? { ...validation.canClaim, breathingRoomSmoke: false }
    : validation.canClaim;
  return {
    ...report,
    ...
    downgrades: effectiveDowngrades,
    validation: {
      ...validation,
      status: effectiveStatus,
      canClaim: effectiveCanClaim,
