...
function validateForegroundHeartbeat(heartbeat, firingId, expectedHookIdentity) {
  ...
  for (let index = 0; index < samples.length; index += 1) {
    ...
    assertEqual(hooks?.identity, expectedHookIdentity.identity, `promoted FireActor hook identity at sample ${index}`);
    assertEqual(hooks?.evidenceSource, expectedHookIdentity.evidenceSource, `promoted FireActor hook source at sample ${index}`);
    assertEqual(hooks?.authority, expectedHookIdentity.authority, `promoted FireActor hook authority at sample ${index}`);
    assertEqual(hooks?.generation, expectedHookIdentity.generation, `promoted FireActor hook generation at sample ${index}`);
    exactObjectMatch(
      hooks?.routeIdentity,
      expectedHookIdentity.routeIdentity,
      `promoted FireActor hook route at sample ${index}`,
    );
    assertEqual(sample.prototypeIdentity, expectedHookIdentity.routeIdentity.prototypeIdentity, `promoted FireActor prototype at sample ${index}`);
...
function validateSchedulerArchive({ report, runDebug, schedulerArchive, firingId }) {
  ...
  const outputRoot = requiredString(report?.outputRoot, 'SHARP pipeline report output root');
  const reportPath = requiredString(report?.reportPath, 'SHARP pipeline report path');
  assertEqual(reportPath, `${outputRoot}/sharp-inline-report.json`, 'SHARP pipeline report run identity');
  const artifactPath = requiredString(artifact?.path, 'scheduler trace artifact path');
  assertEqual(artifactPath, `${outputRoot}/traces/scheduler-events.ndjson`, 'scheduler trace artifact run path');
  const artifactReadUrl = new URL(requiredString(artifact?.readUrl, 'scheduler trace artifact read URL'), 'http://kaminos.invalid');
  if (artifactReadUrl.pathname !== '/api/read'
    || artifactReadUrl.searchParams.get('root') !== 'pipeline-runs') {
    throw new Error('scheduler trace artifact read URL is outside the pipeline-run route');
  }
  const pipelineRunsMarker = '/pipeline-runs/';
  const markerIndex = outputRoot.lastIndexOf(pipelineRunsMarker);
  if (markerIndex < 0) throw new Error('SHARP pipeline report output root is outside pipeline-runs');
  const expectedReadPath = `${outputRoot.slice(markerIndex + pipelineRunsMarker.length)}/traces/scheduler-events.ndjson`;
  assertEqual(artifactReadUrl.searchParams.get('path'), expectedReadPath, 'scheduler trace artifact read URL run identity');
  assertEqual(
    artifact?.jsonPointer,
    '#/authoritativeTrace/sharpRunDebug/schedulerTelemetry/eventTrace/events',
    'scheduler trace artifact JSON pointer',
  );
  ...
  const archiveIdentity = runDebug?.route?.receipt?.metadataPayload?.schedulerTrace?.archiveIdentity;
  assertEqual(archiveIdentity?.schema, 'sharp.webgpu.scheduler-event-archive-identity.v0', 'authenticated scheduler archive identity schema');
  assertEqual(archiveIdentity?.runId, firingId, 'authenticated scheduler archive run identity');
  assertEqual(archiveIdentity?.canonicalization, 'json-stringify-rows-utf8-ndjson-v1', 'authenticated scheduler archive canonicalization');
  assertEqual(archiveIdentity?.encoding, 'utf-8', 'authenticated scheduler archive encoding');
  assertEqual(archiveIdentity?.jsonPointer, artifact.jsonPointer, 'authenticated scheduler archive JSON pointer');
  assertEqual(archiveIdentity?.eventCount, rows.length, 'authenticated scheduler archive event count');
  assertEqual(archiveIdentity?.bytes, bytes, 'authenticated scheduler archive byte count');
  assertEqual(archiveIdentity?.sha256, sha256, 'authenticated scheduler archive SHA-256');
...
function validateFireActorReceipt(receipt, firingId) {
  ...
  const hook = receipt?.foregroundHookIdentity;
  assertEqual(hook?.identity, FIRE_EPISODE_HOOK_IDENTITY, 'promoted FireActor hook identity');
  assertEqual(hook?.evidenceSource, FIRE_EPISODE_HOOK_EVIDENCE_SOURCE, 'promoted FireActor hook evidence source');
  assertEqual(hook?.authority, FIRE_EPISODE_HOOK_AUTHORITY, 'promoted FireActor hook authority');
  if (!Number.isSafeInteger(hook?.generation) || hook.generation <= 0) {
    throw new Error('promoted FireActor hook generation is required');
  }
  for (const field of [
    'effectiveRoute',
    'prototypeIdentity',
  ...
  ]) requiredString(hook?.routeIdentity?.[field], `promoted FireActor hook ${field}`);
  if (hook.routeIdentity.compositionRequested !== hook.routeIdentity.compositionEffective
    || hook.routeIdentity.fallbackReason
    || hook.routeIdentity.compositionFallbackReason) {
    throw new Error('promoted FireActor hook composition identity mismatch');
  }
  return {
    requestedRevision,
    effectiveRevision,
    foregroundHookIdentity: structuredClone(hook),
  };
}
...
function validateBrowserIdentity(value) {
  ...
  if (requestSource === 'independent-default') {
    ...
    assertEqual(
      requiredString(resolution.requestedRealPath, 'browser requested real path'),
      effectiveRealPath,
      'browser requested/effective real path',
    );
  }
...
export function createComposedWorldFireSharpEvidence(input = {}) {
  ...
  const foreground = validateForegroundHeartbeat(
    input.foregroundHeartbeat,
    firingId,
    fireActorSharp.foregroundHookIdentity,
  );
