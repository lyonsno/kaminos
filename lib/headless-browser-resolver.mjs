...
function inspectExecutable(executable, descriptor = {}) {
  ...
  const resolvedKind = executableKind(realPath);
  const kind = resolvedKind === 'installed-stable-chrome'
    ? resolvedKind
    : (descriptor.kind || resolvedKind);
  return {
    executable,
    realPath,
    kind,
...
export function resolveHeadlessBrowser({
  ...
  if (request.executable) {
    const effective = inspectExecutable(request.executable);
    return {
      schema: HEADLESS_BROWSER_RESOLUTION_SCHEMA,
      request,
      requestedRealPath: effective.realPath,
      effective,
  ...
  for (const candidate of independentCandidates) {
    try {
      const effective = inspectExecutable(candidate.executable, candidate);
      if (effective.installedStableChrome) {
        rejectedCandidates.push({
          ...candidate,
          reason: 'installed stable Chrome is forbidden as an implicit headless default',
        });
        continue;
      }
      return {
  ...
  throw new Error(
    ...
    + `Set --chrome or KAMINOS_CHROME explicitly. Candidates: ${JSON.stringify(rejectedCandidates)}`,
  );
}
