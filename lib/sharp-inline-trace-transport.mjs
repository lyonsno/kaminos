...
export async function startSharpInlineLiveTelemetrySession({
  ...
  const pendingRows = [];
  const drain = async () => {
    while (pendingRows.length > 0 && !transportFailure) {
      const rows = pendingRows.splice(0);
      const expectedStart = durableCount;
      try {
        ...
        if (
          receipt.collectionId !== 'progress-events'
          || receipt.receivedCount !== expectedStart + rows.length
        ) {
          throw new Error(
            `SHARP live telemetry receipt mismatch: expected ${expectedStart + rows.length}, `
            + `received ${receipt.receivedCount}`,
          );
        }
        durableCount = receipt.receivedCount;
      } catch (error) {
  ...
  return {
    ...
    async finish({
      ...
      try {
        ...
        try {
          await postJson(fetchImpl, '/api/sharp-inline-run-report/abort', {
            ...
            lastTrustworthyCounts: { 'progress-events': durableCount },
          });
        } catch {
      ...
      return postJson(fetchImpl, '/api/sharp-inline-run-report/finish', {
        sessionId: startReceipt.sessionId,
        expectedCounts: { 'progress-events': durableCount },
        documentPatch: { status, phase },
      });
    },
    async abort({
      phase = 'sharp-inference',
