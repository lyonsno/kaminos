...
export function compactSharpInlineReportDocument(document, {
  ...
  const externalize = (id, jsonPointer, values) => {
    ...
    collections.push({ id, jsonPointer, values, ref });
...
export async function startSharpInlineLiveTelemetrySession({
  ...
  if (!isRecord(routeIdentity)) {
    throw new TypeError('SHARP live telemetry routeIdentity must be an object');
  }
  const gateBMode = routeIdentity.schema === 'kaminos.sharp-gate-b-route-identity.v0';
  if (gateBMode) {
    const identityFailures = validateGateBRouteIdentity(routeIdentity);
    if (identityFailures.length) {
      throw new Error(`SHARP Gate B route identity rejected: ${identityFailures.join('; ')}`);
    }
  }
  if (!Number.isFinite(flushIntervalMs) || flushIntervalMs <= 0) {
    throw new TypeError('SHARP live telemetry flushIntervalMs must be finite and positive');
  }
  if (typeof setTimeoutImpl !== 'function' || typeof clearTimeoutImpl !== 'function') {
    throw new TypeError('SHARP live telemetry requires timer functions');
  }
  const collectionDefinitions = gateBMode
    ? GATE_B_COLLECTIONS
    : [{
        id: 'progress-events',
  ...
  const collectionStates = new Map(collectionDefinitions.map(collection => [
    collection.id,
    {
      queuedCount: 0,
      durableCount: 0,
  ...
  const batchingSnapshot = () => createGateBBatchingState({
    flushIntervalMs,
    collectionCounts: Object.fromEntries(
      [...collectionStates].map(([collectionId, state]) => [
        collectionId,
        {
          queued: state.queuedCount,
          flushed: state.durableCount,
  ...
  const startReceipt = await postJson(fetchImpl, '/api/sharp-inline-run-report/start', {
    ...
    document: {
      ...
      sourceIdentity,
      liveTelemetry: {
        schema: 'kaminos.sharp-inline-live-progress.v0',
        retention: 'uncapped',
        progressEventsRef: '#/traceArtifacts/progress-events',
        collectionRefs: Object.fromEntries(
          collectionDefinitions.map(collection => [
            collection.id,
            `#/traceArtifacts/${collection.id}`,
          ]),
        ),
      },
      ...(gateBMode ? {
        gateB: {
          ...
          routeIdentity,
          batching: batchingSnapshot(),
        },
      } : {}),
    },
    collections: collectionDefinitions,
  });
  if (!startReceipt.sessionId || !startReceipt.statePath || !startReceipt.stateReadUrl) {
    throw new Error('SHARP live telemetry start omitted its durable state identity');
  }
  ...
  let flushTimer = null;
  const hasPendingRows = () => [...collectionStates.values()]
    .some(state => state.pendingRows.length > 0);
  ...
  const scheduleFlush = () => {
    if (flushTimer !== null || drainPromise || transportFailure || settled) return;
  ...
  const drain = async () => {
    ...
    try {
      const batches = [...collectionStates]
        .map(([collectionId, state]) => {
          const rows = state.pendingRows.splice(0);
          state.inFlightCount = rows.length;
          return { collectionId, state, rows };
        })
        .filter(batch => batch.rows.length > 0);
      for (const { collectionId, state, rows } of batches) {
        if (transportFailure) break;
        const expectedStart = state.durableCount;
        try {
          const receipt = await postJson(fetchImpl, '/api/sharp-inline-run-report/chunk', {
          ...
          if (
            ...
            || receipt.receivedCount !== expectedStart + rows.length
          ) {
            throw new Error(
              `SHARP live telemetry receipt mismatch for ${collectionId}: `
              + `expected ${expectedStart + rows.length}, received ${receipt.receivedCount}`,
            );
          }
          state.durableCount = receipt.receivedCount;
      ...
      drainPromise = null;
      if (hasPendingRows() && !transportFailure && !settled) scheduleFlush();
    }
  };
  const append = (collectionIdOrRow, maybeRow) => {
    if (settled) throw new Error('SHARP live telemetry session is already settled');
    const collectionId = typeof collectionIdOrRow === 'string'
      ? collectionIdOrRow
      : 'progress-events';
    const row = typeof collectionIdOrRow === 'string' ? maybeRow : collectionIdOrRow;
    const state = collectionStates.get(collectionId);
    if (!state) throw new RangeError(`unknown SHARP live telemetry collection ${collectionId}`);
    if (!isRecord(row)) throw new TypeError('SHARP live telemetry row must be an object');
    if (transportFailure) return Promise.resolve();
  ...
  const flush = async () => {
    cancelScheduledFlush();
    while (drainPromise || hasPendingRows()) {
      if (!drainPromise) drainPromise = drain();
      await drainPromise;
    }
  ...
  return {
    status: startReceipt.status || 'receiving',
    sessionId: startReceipt.sessionId,
    outputRoot: startReceipt.outputRoot,
    statePath: startReceipt.statePath,
    stateReadUrl: startReceipt.stateReadUrl,
    append,
    flush,
    batchingSnapshot,
    async finish({
      ...
      try {
        ...
        try {
          await postJson(fetchImpl, '/api/sharp-inline-run-report/abort', {
            ...
            lastTrustworthyCounts: lastTrustworthyCounts(),
            ...(gateBMode ? { batching: batchingSnapshot() } : {}),
          });
        } catch {
      ...
      if (gateBMode) {
        const collections = Object.fromEntries(
          [...collectionStates].map(([collectionId, state]) => [
            ...
            {
              retention: 'uncapped',
              receivedCount: state.durableCount,
              expectedCount: state.queuedCount,
              partialWrite: state.durableCount !== state.queuedCount,
            },
          ]),
        );
        validationFailures = validateGateBCompletion({
          status,
          routeIdentity,
          collections,
          artifact,
        ...
        if (validationFailures.length) {
          throw new Error(`SHARP Gate B completion rejected: ${validationFailures.join('; ')}`);
        }
      }
      const receipt = await postJson(fetchImpl, '/api/sharp-inline-run-report/finish', {
        sessionId: startReceipt.sessionId,
        expectedCounts: lastTrustworthyCounts(),
        documentPatch: {
          status,
          phase,
          ...(artifact ? { artifact } : {}),
    ...
    async abort({
      phase = 'sharp-inference',
      error = 'SHARP inline inference aborted',
    } = {}) {
      if (settled) throw new Error('SHARP live telemetry session is already settled');
      settled = true;
      cancelScheduledFlush();
      if (drainPromise) await drainPromise;
      return postJson(fetchImpl, '/api/sharp-inline-run-report/abort', {
        sessionId: startReceipt.sessionId,
        phase,
        error: transportFailure?.message || error,
        lastTrustworthyCounts: lastTrustworthyCounts(),
        ...(gateBMode ? { batching: batchingSnapshot() } : {}),
      });
    },
