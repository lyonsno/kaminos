...
const NO_RENDER_RECEIPT_SCHEMA = 'kaminos.volume-foreground-no-render-receipt.v0';
...
function validateNoRenderReceipt(receipt, expected) {
  const serviceAuthorityValid = (
    receipt?.serviceMode === 'presented-raf'
    && receipt.presentationObserved === true
    && receipt.serviceAuthority === 'browser-request-animation-frame'
    && Number.isFinite(receipt.rafTimestampMs)
    && receipt.fallbackReason === null
  ) || (
    receipt?.serviceMode === 'non-present-fallback'
    && receipt.presentationObserved === false
    && receipt.serviceAuthority === 'browser-task-fallback-no-presentation'
    && receipt.rafTimestampMs === null
    && receipt.fallbackReason === 'raf-suspended-or-delayed'
  );
  const valid = receipt?.schema === NO_RENDER_RECEIPT_SCHEMA
    && receipt.status === 'opportunity-served'
    && receipt.firingId === expected.firingId
    && receipt.frameId === expected.frameId
    && receipt.requestId === expected.requestId
    && receipt.commandBufferCount === 0
    && receipt.simulationQuiesced === true
    && receipt.raymarchSubmissionQuiesced === true
    && serviceAuthorityValid;
  if (!valid) {
    throw new Error('foreground no-render service did not prove a quiesced opportunity');
  }
  return receipt;
}
...
export function createSharpSameDeviceKilnOpportunityHook({
  ...
  firingId,
  nextFrameId,
  presentationIsolation = 'render',
} = {}) {
  const exactFiringId = requireNonEmptyString(firingId, 'firingId');
  if (!['render', 'no-render'].includes(presentationIsolation)) {
    throw new Error(`unsupported same-device kiln presentation isolation: ${presentationIsolation}`);
  }
  ...
  return function sharpSameDeviceKilnOpportunity(context = {}) {
    ...
    return {
      requestId,
      metadata: {
        workloadIdentity: presentationIsolation === 'no-render'
          ? 'foreground-opportunity-no-render-v0'
      ...
      async run(service = {}) {
        ...
        if (presentationIsolation === 'no-render') {
          if (typeof volume.serveForegroundNoRenderOpportunity !== 'function') {
            throw new Error('same-device kiln hook requires serveForegroundNoRenderOpportunity()');
          }
          const receipt = await volume.serveForegroundNoRenderOpportunity({
            firingId: exactFiringId,
            frameId,
            requestId,
            signal: service.signal,
          });
          return validateNoRenderReceipt(receipt, {
            firingId: exactFiringId,
            frameId,
            requestId,
          });
