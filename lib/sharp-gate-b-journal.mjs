...
export function normalizeGateBAdaptiveRange(event) {
  ...
  const rangeIndexSuffix = `:${event.rangeIndex}`;
  if (typeof event.rangeId !== 'string' || !event.rangeId.endsWith(rangeIndexSuffix)) {
    throw new TypeError('Gate B adaptive range requires a planner-scoped rangeId');
  }
  let plannerId = event.rangeId.slice(0, -rangeIndexSuffix.length);
  // Retain compatibility with the journal fixture's `plannerId:range:index`
  // spelling while accepting the kit's effective `plannerId:index` contract.
  if (plannerId.endsWith(':range')) plannerId = plannerId.slice(0, -':range'.length);
  ...
  const adaptiveRangeStatus = event.adjustment === 'complete' ? 'complete' : 'continuing';
  if (adaptiveRangeStatus === 'complete') {
    if (event.outputEnd !== event.totalOutputItems
        || event.completedItems !== event.totalOutputItems
        || event.progress !== 1
        || event.rangeCountAuthority !== 'actual'
        || !positiveSafeInteger(event.actualRangeCount)
        || event.actualRangeCount !== event.rangeIndex + 1) {
      throw new TypeError('Gate B terminal adaptive range completion identity is inconsistent');
    }
    for (const field of [
      'rawNextChunkItems',
      ...
      if (event[field] !== null) {
        throw new TypeError(`Gate B terminal adaptive range ${field} must be null`);
      }
    }
  } else {
    for (const field of [
  ...
  const plannedChunkItems = event.plannedChunkItems == null
    ? null
    : event.plannedChunkItems;
  if (plannedChunkItems !== null
      && (!Number.isFinite(plannedChunkItems) || plannedChunkItems <= 0)) {
    throw new TypeError('Gate B adaptive range plannedChunkItems must be finite and positive when emitted');
  }
  for (const field of ['observedDurationMs']) {
    if (!nonNegativeFinite(event[field])) {
      throw new TypeError(`Gate B adaptive range ${field} must be finite and non-negative`);
    }
  }
  ...
  const bounds = event.bounds == null ? null : event.bounds;
  if (bounds !== null
      && (!isRecord(bounds)
        || !positiveSafeInteger(bounds.minChunkItems)
        || !positiveSafeInteger(bounds.maxChunkItems)
        || bounds.minChunkItems > bounds.maxChunkItems)) {
  ...
  return Object.freeze({
    ...event,
    plannedChunkItems,
    plannedChunkItemsAuthority: plannedChunkItems === null
      ? 'not-emitted-by-effective-producer'
      : 'producer-explicit',
    bounds,
    boundsAuthority: bounds === null
      ? 'not-emitted-by-effective-producer'
      : 'producer-explicit',
    adaptiveRangeStatus,
    plannerId,
...
export function appendGateBSchedulerTelemetry({
  event,
  append,
  observedAtMs = null,
} = {}) {
  if (typeof append !== 'function') {
    return Object.freeze({
      status: 'unreported',
      error: 'Gate B scheduler telemetry append callback is unavailable',
    });
  }
  let row = event;
  try {
    if (
      ...
      row = normalizeGateBAdaptiveRange(event);
    }
  } catch (error) {
    const diagnostic = {
      schema: 'kaminos.sharp-gate-b-runtime-error.v0',
      kind: 'scheduler-telemetry-validation-error',
      authority: 'kaminos-gate-b-observer',
      observedAtMs: Number.isFinite(observedAtMs) ? observedAtMs : null,
      validator: 'normalizeGateBAdaptiveRange',
      message: error?.message || String(error),
      event,
    };
    try {
      append('runtime-errors', diagnostic);
      return Object.freeze({
        status: 'quarantined',
        diagnostic,
      });
      ...
      return Object.freeze({
        status: 'unreported',
        error: reportingError?.message || String(reportingError),
        diagnostic,
      });
    }
  }
  try {
    append('scheduler-events', row);
    return Object.freeze({
      status: 'accepted',
      row,
    });
