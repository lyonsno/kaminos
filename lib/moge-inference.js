...
function createWebGpuDeviceRequest(adapter, options = {}) {
...
function requestBrowserWebGpuDevice(gpu, options = {}) {
  ...
  return (async () => {
    ...
    if (isNonEmptyString$3(options.label)) descriptor.label = options.label;
...
function validateWebGpuBackendIdentity(identity) {
  ...
  if (!isNonEmptyString$3(identity.adapterName)) errors.push('adapterName must be a non-empty string');
...
const WEBGPU_INFERENCE_KIT_VERSION = '0.1.49';
...
function createMogeDepthNormalRouteDefinition(input = {}) {
  ...
  return defineWebGpuRoute({
    routeId: MOGE_DEPTH_NORMAL_ROUTE_ID$3,
...
function retainResource(device, resource) {
...
function createResourceScope(device) {
 ...
 */
function buildMogeDeviceRequest(adapter, options = {}) {
 ...
 */
function borrowedDeviceBackendIdentity({ adapter, device, browser, requestedFeatures } = {}) {
 ...
 * pooled site here.
 */
function createBufferPool(device) {
...
function acquirePooledBuffer(device, size, usage = 0) {
...
function releaseRunBuffers(device) {
...
async function warmUpPipelines(device) {
  const pipelineCache = cacheFor(device, pipelineCaches);
  const pending = [];
...
async function coopAdmit(coop, phase, chunk, extra = {}) {
...
function createMogeSchedulerVerificationReceipt({ routeRequest, scheduler, backpressure, stagedStages, observedEvents }) {
  ...
  return {
    ...
    route: {
      requestedRouteId: routeRequest?.routeId || MOGE_DEPTH_NORMAL_ROUTE_ID$2,
      effectiveRouteId: MOGE_DEPTH_NORMAL_ROUTE_ID$2,
...
export { INFERENCE_LIMIT_KEYS, MoGeInference, borrowedDeviceBackendIdentity, buildMogeDeviceRequest, coopAdmit, createMogeSchedulerVerificationReceipt, inferenceLimits, initGPU, resolveCooperativeScheduler };
