...
export function createForegroundHostEventCorrelation({ foregroundHeartbeat, foregroundGaps = [] } = {}) {
  const events = Array.isArray(foregroundHeartbeat?.hostEvents) ? foregroundHeartbeat.hostEvents : [];
  const gaps = foregroundGaps.filter(gap => Number.isFinite(gap?.startEpochMs) && Number.isFinite(gap?.endEpochMs));
  const correlatedGaps = gaps.map(gap => {
    const overlaps = events.map(event => {
      const start = Math.max(gap.startEpochMs, event.startEpochMs);
      const end = Math.min(gap.endEpochMs, event.endEpochMs);
      return end > start ? { ...event, overlapStartEpochMs: start, overlapEndEpochMs: end, overlapDurationMs: end - start } : null;
    }).filter(Boolean);
    return { ...gap, hostOverlaps: overlaps };
  });
  const coveredDurationMs = correlatedGaps.reduce((sum, gap) => sum + gap.hostOverlaps.reduce((inner, event) => inner + event.overlapDurationMs, 0), 0);
  return {
    ...
    correlatedGaps,
    coveredDurationMs,
    uncoveredDurationMs: Math.max(0, correlatedGaps.reduce((sum, gap) => sum + gap.durationMs, 0) - coveredDurationMs),
  };
}
...
export function createForegroundKilnHeartbeatEpisode({
  ...
  function recordEvent({ kind, phase, startMs, endMs, detail = null } = {}) {
