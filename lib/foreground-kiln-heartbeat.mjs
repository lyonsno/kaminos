...
export function createForegroundHostEventCorrelation({
  foregroundHeartbeat,
  foregroundGaps = [],
  unexplainedThresholdMs = 50,
} = {}) {
  const failures = [];
  const events = Array.isArray(foregroundHeartbeat?.hostEvents) ? foregroundHeartbeat.hostEvents : [];
  const clock = foregroundHeartbeat?.clock;
  if (foregroundHeartbeat?.schema !== FOREGROUND_KILN_HEARTBEAT_SCHEMA) {
    pushFailure(failures, 'foreground-heartbeat-missing');
  }
  if (!foregroundHeartbeat?.firingId) pushFailure(failures, 'foreground-firing-id-missing');
  if (foregroundHeartbeat?.hostEventRetention !== 'uncapped'
    || foregroundHeartbeat?.hostEventCount !== events.length) {
    pushFailure(failures, 'host-events-capped-or-partial');
  }
  if (clock?.schema !== CROSS_PAGE_CLOCK_SCHEMA
    || clock?.timingAuthority !== 'performance-time-origin-plus-now'
  ...
  for (const event of events) {
    if (!event?.eventId || !event?.kind || !event?.source || !event?.phase
      || !Number.isFinite(event?.startMs) || !Number.isFinite(event?.endMs)
      || !Number.isFinite(event?.startEpochMs) || !Number.isFinite(event?.endEpochMs)
      || !Number.isFinite(event?.durationMs) || event.endMs <= event.startMs
      || event.endEpochMs <= event.startEpochMs
      || Math.abs(event.durationMs - (event.endMs - event.startMs)) > 1) {
      pushFailure(failures, 'host-event-interval-invalid');
      continue;
    }
    if (event.firingId !== foregroundHeartbeat?.firingId) {
      pushFailure(failures, 'host-event-firing-mismatch');
      continue;
    }
    if (Number.isFinite(clock?.timeOriginEpochMs)
      && (Math.abs((clock.timeOriginEpochMs + event.startMs) - event.startEpochMs) > 1
        || Math.abs((clock.timeOriginEpochMs + event.endMs) - event.endEpochMs) > 1)) {
      pushFailure(failures, 'host-event-clock-mismatch');
      continue;
    }
  ...
  const validGaps = [];
  for (const gap of Array.isArray(foregroundGaps) ? foregroundGaps : []) {
    if (!Number.isFinite(gap?.sampleIndex)
      || !Number.isFinite(gap?.startEpochMs) || !Number.isFinite(gap?.endEpochMs)
      || !Number.isFinite(gap?.durationMs) || gap.endEpochMs <= gap.startEpochMs
      || Math.abs(gap.durationMs - (gap.endEpochMs - gap.startEpochMs)) > 1
      || !Array.isArray(gap?.overlaps)) {
      pushFailure(failures, 'foreground-gap-invalid');
      continue;
    }
    validGaps.push(gap);
  }
  if (!validGaps.length) pushFailure(failures, 'foreground-gaps-missing-or-partial');
  ...
  function mergedIntervals(intervals) {
    ...
    for (const interval of ordered) {
      ...
      if (previous && interval.startEpochMs <= previous.endEpochMs) {
        previous.endEpochMs = Math.max(previous.endEpochMs, interval.endEpochMs);
      } else {
  ...
  function intervalDuration(intervals) {
    return intervals.reduce((sum, interval) => sum + interval.endEpochMs - interval.startEpochMs, 0);
  }
  ...
  function intersectionDuration(left, right) {
    ...
    while (leftIndex < left.length && rightIndex < right.length) {
      const start = Math.max(left[leftIndex].startEpochMs, right[rightIndex].startEpochMs);
      const end = Math.min(left[leftIndex].endEpochMs, right[rightIndex].endEpochMs);
      if (end > start) total += end - start;
      if (left[leftIndex].endEpochMs <= right[rightIndex].endEpochMs) leftIndex += 1;
      else rightIndex += 1;
    }
    return total;
  }

  function uncoveredSegments(gap, coverage) {
    const uncovered = [];
    let cursor = gap.startEpochMs;
    for (const interval of coverage) {
      if (interval.startEpochMs > cursor) {
        uncovered.push({
          startEpochMs: rounded(cursor),
          endEpochMs: rounded(interval.startEpochMs),
          durationMs: rounded(interval.startEpochMs - cursor),
        });
      }
      cursor = Math.max(cursor, interval.endEpochMs);
    }
    if (cursor < gap.endEpochMs) {
      uncovered.push({
        startEpochMs: rounded(cursor),
        endEpochMs: rounded(gap.endEpochMs),
        durationMs: rounded(gap.endEpochMs - cursor),
      });
    }
    return uncovered;
  }

  const correlatedGaps = validGaps.map(gap => {
    const clippedSharpIntervals = gap.overlaps.map(overlap => ({
      startEpochMs: Math.max(gap.startEpochMs, overlap.startEpochMs),
      endEpochMs: Math.min(gap.endEpochMs, overlap.endEpochMs),
    })).filter(interval => interval.endEpochMs > interval.startEpochMs);
    const hostOverlaps = validEvents.map(event => {
      const overlapStartEpochMs = Math.max(gap.startEpochMs, event.startEpochMs);
      const overlapEndEpochMs = Math.min(gap.endEpochMs, event.endEpochMs);
      return overlapEndEpochMs > overlapStartEpochMs ? {
        ...event,
        overlapStartEpochMs: rounded(overlapStartEpochMs),
        overlapEndEpochMs: rounded(overlapEndEpochMs),
        overlapDurationMs: rounded(overlapEndEpochMs - overlapStartEpochMs),
      } : null;
    }).filter(Boolean);
    const sharpIntervals = mergedIntervals(clippedSharpIntervals);
    const hostIntervals = mergedIntervals(hostOverlaps.map(overlap => ({
      startEpochMs: overlap.overlapStartEpochMs,
      endEpochMs: overlap.overlapEndEpochMs,
    })));
    const combinedIntervals = mergedIntervals([...sharpIntervals, ...hostIntervals]);
    const sharpCoveredDurationMs = intervalDuration(sharpIntervals);
    const hostCoveredDurationMs = intervalDuration(hostIntervals);
    const sharedSharpHostDurationMs = intersectionDuration(sharpIntervals, hostIntervals);
    const combinedCoveredDurationMs = intervalDuration(combinedIntervals);
    return {
      ...gap,
      hostOverlaps,
      sharpCoveredDurationMs: rounded(sharpCoveredDurationMs),
      hostCoveredDurationMs: rounded(hostCoveredDurationMs),
      sharedSharpHostDurationMs: rounded(sharedSharpHostDurationMs),
      hostOnlyDurationMs: rounded(hostCoveredDurationMs - sharedSharpHostDurationMs),
      combinedCoveredDurationMs: rounded(combinedCoveredDurationMs),
      uncoveredDurationMs: rounded(Math.max(0, gap.durationMs - combinedCoveredDurationMs)),
      uncoveredSegments: uncoveredSegments(gap, combinedIntervals),
    };
  });

  const unexplainedGapsAtOrAboveThreshold = correlatedGaps.flatMap(gap => gap.uncoveredSegments
    .filter(segment => segment.durationMs >= unexplainedThresholdMs)
    .map(segment => ({ sampleIndex: gap.sampleIndex, ...segment })));
  const foregroundGapDurationMs = correlatedGaps.reduce((sum, gap) => sum + gap.durationMs, 0);
  const sharpCoveredDurationMs = correlatedGaps.reduce((sum, gap) => sum + gap.sharpCoveredDurationMs, 0);
  const hostCoveredDurationMs = correlatedGaps.reduce((sum, gap) => sum + gap.hostCoveredDurationMs, 0);
  const sharedSharpHostDurationMs = correlatedGaps.reduce((sum, gap) => sum + gap.sharedSharpHostDurationMs, 0);
  const combinedCoveredDurationMs = correlatedGaps.reduce((sum, gap) => sum + gap.combinedCoveredDurationMs, 0);
  const uncoveredDurationMs = correlatedGaps.reduce((sum, gap) => sum + gap.uncoveredDurationMs, 0);
  const hostRanking = field => {
    const grouped = new Map();
    for (const gap of correlatedGaps) {
      const perGap = new Map();
      for (const overlap of gap.hostOverlaps) {
        const key = overlap[field] || 'unknown';
        const intervals = perGap.get(key) || [];
        intervals.push({ startEpochMs: overlap.overlapStartEpochMs, endEpochMs: overlap.overlapEndEpochMs });
        perGap.set(key, intervals);
      }
      for (const [key, intervals] of perGap) {
        grouped.set(key, (grouped.get(key) || 0) + intervalDuration(mergedIntervals(intervals)));
      }
    }
    return [...grouped.entries()]
      .map(([key, overlapDurationMs]) => ({ [field]: key, overlapDurationMs: rounded(overlapDurationMs) }))
  ...
  return {
    ...
    status,
    cadenceStatus: status === 'verified'
      ? (unexplainedGapsAtOrAboveThreshold.length ? 'failed' : 'passed')
      : 'invalid',
    evidenceSource: 'epoch-intersection-of-foreground-raf-gaps-and-browser-host-events',
    timingAuthority: 'performance-time-origin-plus-now-cross-page-join',
    disclaimer: 'host-and-submitted-work-overlap-not-causal-gpu-exclusive-compositor-or-display-present-latency',
    firingId: foregroundHeartbeat?.firingId || null,
    unexplainedThresholdMs,
    foregroundGapCount: correlatedGaps.length,
    hostEventCount: events.length,
    correlatedGaps,
    phaseRankings: hostRanking('phase'),
    sourceRankings: hostRanking('source'),
    totals: {
      foregroundGapDurationMs: rounded(foregroundGapDurationMs),
      sharpCoveredDurationMs: rounded(sharpCoveredDurationMs),
      hostCoveredDurationMs: rounded(hostCoveredDurationMs),
      ...
      hostOnlyDurationMs: rounded(hostCoveredDurationMs - sharedSharpHostDurationMs),
      combinedCoveredDurationMs: rounded(combinedCoveredDurationMs),
      uncoveredDurationMs: rounded(uncoveredDurationMs),
      combinedCoveredFraction: foregroundGapDurationMs > 0
        ? rounded(combinedCoveredDurationMs / foregroundGapDurationMs)
        : null,
    },
    unexplainedGapsAtOrAboveThreshold,
    failures,
  };
}
...
export function createForegroundKilnHeartbeatEpisode({
  ...
  function recordEvent({ kind, source = 'runtime-explicit', phase, startMs, endMs, detail = null } = {}) {
    ...
    hostEvents.push({
      eventId: `${firingId || 'unbound-firing'}:host:${hostEventSequence}`,
  ...
  function startHostTelemetry() {
    ...
    try {
      performanceObserver = new globalThis.PerformanceObserver(entries => {
  ...
  function report({ phase = finished ? 'complete' : 'running', sharpHeartbeat = null } = {}) {
    ...
    if (requireSharpDutyCorrelation && finished) {
      ...
      heartbeat.hostEventCorrelation = createForegroundHostEventCorrelation({
        foregroundHeartbeat: heartbeat,
        foregroundGaps: heartbeat.sharpDutyCorrelation.foregroundGaps,
      });
      if (heartbeat.sharpDutyCorrelation.status !== 'verified') {
        pushFailure(failures, 'sharp-duty-correlation-invalid');
      }
      if (heartbeat.hostEventCorrelation.status !== 'verified') {
        pushFailure(failures, 'host-event-correlation-invalid');
      }
    } else {
      heartbeat.sharpDutyCorrelation = null;
      heartbeat.hostEventCorrelation = null;
    }
