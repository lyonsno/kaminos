...
export const FOREGROUND_HOST_TELEMETRY_SCHEMA = 'kaminos.foreground-host-telemetry.v0';
...
function summarizeSharpHeartbeat(sharpHeartbeat) {
  if (!sharpHeartbeat || typeof sharpHeartbeat !== 'object') return null;
  const dutyEnvelope = sharpHeartbeat.gpuDutyIntervals;
  return {
    schema: sharpHeartbeat.schema || null,
    status: sharpHeartbeat.status || null,
    evidenceSource: sharpHeartbeat.evidenceSource || null,
    timingAuthority: sharpHeartbeat.timingAuthority || null,
    crossPageClock: sharpHeartbeat.crossPageClock ? { ...sharpHeartbeat.crossPageClock } : null,
    inferenceWindow: sharpHeartbeat.inferenceWindow ? { ...sharpHeartbeat.inferenceWindow } : null,
    gpuDutyIntervals: dutyEnvelope ? {
      schema: dutyEnvelope.schema || null,
      timingAuthority: dutyEnvelope.timingAuthority || null,
      runId: dutyEnvelope.runId || null,
      count: Number.isSafeInteger(dutyEnvelope.count) ? dutyEnvelope.count : null,
      intervalsRef: dutyEnvelope.intervalsRef ? { ...dutyEnvelope.intervalsRef } : null,
      pairingFailureCount: Array.isArray(dutyEnvelope.pairingFailures)
        ? dutyEnvelope.pairingFailures.length
        : 0,
    } : null,
  };
}
...
function normalizedFirePresentation(value) {
  ...
  return {
    ...
    flameContinuityRequested: value.flameContinuityRequested || 'live-every-frame',
    flameContinuityEffective: value.flameContinuityEffective || 'live-every-frame',
    flameContinuityEffectiveReason: value.flameContinuityEffectiveReason || 'unspecified',
    flameContinuityEvidence: value.flameContinuityEvidence && typeof value.flameContinuityEvidence === 'object'
      ? structuredClone(value.flameContinuityEvidence)
      : null,
    hybridSplatSmokeCompositorIdentity: value.hybridSplatSmokeCompositorIdentity || null,
    hybridSplatSmokeApproximation: value.hybridSplatSmokeApproximation || null,
    splatDepthConditionedSmokeSplit: value.splatDepthConditionedSmokeSplit || null,
    hybridSmokePhaseAuthority: value.hybridSmokePhaseAuthority || null,
    hybridSplatLayer: value.hybridSplatLayer && typeof value.hybridSplatLayer === 'object'
      ? { ...value.hybridSplatLayer }
      : null,
    hybridSmokeLayer: value.hybridSmokeLayer && typeof value.hybridSmokeLayer === 'object'
      ? {
          ...value.hybridSmokeLayer,
          intervals: Array.isArray(value.hybridSmokeLayer.intervals)
            ? [...value.hybridSmokeLayer.intervals]
            : value.hybridSmokeLayer.intervals,
        }
      : null,
    raster: {
...
function cloneFirePresentation(value) {
  if (!value || typeof value !== 'object') return null;
  return {
    ...value,
    hybridSplatLayer: value.hybridSplatLayer && typeof value.hybridSplatLayer === 'object'
    ...
    hybridSmokeLayer: value.hybridSmokeLayer && typeof value.hybridSmokeLayer === 'object'
      ...
      : value.hybridSmokeLayer,
    raster: value.raster && typeof value.raster === 'object'
      ? { ...value.raster }
      : value.raster,
    timing: value.timing && typeof value.timing === 'object'
      ? { ...value.timing }
      : value.timing,
    flameContinuityEvidence: value.flameContinuityEvidence && typeof value.flameContinuityEvidence === 'object'
      ? structuredClone(value.flameContinuityEvidence)
      : value.flameContinuityEvidence,
    fireEpisodeHooks: value.fireEpisodeHooks && typeof value.fireEpisodeHooks === 'object'
      ? {
          ...value.fireEpisodeHooks,
          rawRafGapSamplesMs: Array.isArray(value.fireEpisodeHooks.rawRafGapSamplesMs)
            ? [...value.fireEpisodeHooks.rawRafGapSamplesMs]
            : value.fireEpisodeHooks.rawRafGapSamplesMs,
          routeIdentity: value.fireEpisodeHooks.routeIdentity && typeof value.fireEpisodeHooks.routeIdentity === 'object'
            ? { ...value.fireEpisodeHooks.routeIdentity }
            : value.fireEpisodeHooks.routeIdentity,
        }
      : value.fireEpisodeHooks,
  };
...
export function firePresentationMismatchReasons(expected, effective) {
  if (!expected) return [];
  ...
  for (const field of [
    ...
    'splatDepthConditionedSmokeSplit',
    'hybridSmokePhaseAuthority',
  ]) {
  ...
  if (expected.hybridSplatLayer?.identity !== undefined
    && expected.hybridSplatLayer.identity !== effective.hybridSplatLayer?.identity) {
    reasons.push('hybrid-splat-layer-identity-mismatch');
  }
  if (expected.hybridSmokeLayer?.identity !== undefined
    && expected.hybridSmokeLayer.identity !== effective.hybridSmokeLayer?.identity) {
    reasons.push('hybrid-smoke-layer-identity-mismatch');
  }
  if (expected.hybridSmokeLayer?.opticalComposition !== undefined
    && expected.hybridSmokeLayer.opticalComposition !== effective.hybridSmokeLayer?.opticalComposition) {
    reasons.push('hybrid-smoke-optical-composition-mismatch');
  }
  if (Array.isArray(expected.hybridSmokeLayer?.intervals)
    && (!Array.isArray(effective.hybridSmokeLayer?.intervals)
      || expected.hybridSmokeLayer.intervals.length !== effective.hybridSmokeLayer.intervals.length
      || expected.hybridSmokeLayer.intervals.some((interval, index) => interval !== effective.hybridSmokeLayer.intervals[index]))) {
    reasons.push('hybrid-smoke-intervals-mismatch');
  }
  if (expected.requireNoFallback && effective.fallbackReason !== null) reasons.push('fallback-present');
  if (expected.flameContinuityRequested
    && effective.flameContinuityRequested !== expected.flameContinuityRequested) {
    reasons.push('flame-continuity-request-mismatch');
  }
  if (expected.requireFlameContinuityEvidence) {
    ...
    if (continuity?.schema !== 'kaminos.single-flame-continuity-runtime.v0'
      || continuity?.firingId !== effective.firingId
      || continuity?.requested !== expected.flameContinuityRequested
      || continuity?.effective !== effective.flameContinuityEffective
      || !['live', 'holdover'].includes(continuity?.mode)
      || !Number.isFinite(continuity?.presentationOrdinal)
      || !Number.isFinite(counts?.live)
      || !Number.isFinite(counts?.holdover)
      || !Number.isFinite(counts?.fallback)) {
      reasons.push('flame-continuity-evidence-missing-or-mismatched');
    } else if (continuity.mode === 'holdover') {
      const slot = continuity.selectedHistorySlot;
      if (!Number.isFinite(slot?.slotIndex)
        || !Number.isFinite(slot?.historyAllocationGeneration)
        || !Number.isFinite(slot?.archiveWriteSequence)
        || !Number.isFinite(slot?.sourceCandidateGeneration)
        || continuity.renderFrameAdvanced !== true
        || continuity.sourceRenderFrameAdvanced !== false
        || continuity.simulatorStepAdvanced !== false) {
        reasons.push('holdover-evidence-incomplete');
      }
    } else {
      if (continuity.renderFrameAdvanced !== true
        || continuity.sourceRenderFrameAdvanced !== true
        || continuity.simulatorStepAdvanced !== true) {
        reasons.push('live-continuity-evidence-incomplete');
      }
      if (counts.fallback > 0 && !continuity.fallbackReason) {
        reasons.push('fallback-evidence-incomplete');
      }
    }
  }
  if (expected.requireZeroOverflow && effective.candidateOverflow !== 0) reasons.push('candidate-overflow-present');
  if (expected.requireZeroCandidateCopy && effective.candidateCopyBytes !== 0) {
    reasons.push('candidate-copy-present');
  }
  if (expected.requireNonEmptyCandidateSet
    && (!Number.isFinite(effective.candidateCount) || effective.candidateCount <= 0)) {
...
export function createForegroundKilnHeartbeatEpisode({
  ...
  const expectedFirePresentationSnapshot = cloneFirePresentation(expectedFirePresentation);
  ...
  let hostEpisodeId = null;
  let longTaskSourceStatus = 'not-started';
  let longTaskSourceReason = null;
  let longTaskSourceError = null;
  let startedAtMs = null;
  let finishedAtMs = null;
  let preEpisodeFrameCallbackCount = 0;
  ...
  function recordHostEvent({
    ...
    if (!started || !hostEpisodeId) {
      throw new Error('foreground telemetry events require a started episode');
    }
    if (typeof kind !== 'string' || !kind || typeof phase !== 'string' || !phase) {
      throw new TypeError('foreground telemetry events require kind and phase');
    }
    if (typeof source !== 'string' || !source) {
      throw new TypeError('foreground telemetry events require source identity');
    }
    const start = finite(startMs);
    const end = finite(endMs);
    if (start === null || end === null || end < start) {
      throw new TypeError('foreground telemetry events require an ordered finite interval');
    }
    hostEvents.push({
      kind,
      phase,
      ...
      clockSchema: CROSS_PAGE_CLOCK_SCHEMA,
    ...
    return hostEvents.at(-1);
  }

  function recordEvent(event = {}) {
    return recordHostEvent({ ...event, source: 'explicit-record-event' });
  }
  ...
  function schedule() {
    frameHandle = requestFrame(timestampMs => {
      frameHandle = null;
      if (finished) return;
      const frameTimestampMs = finite(timestampMs, now());
      if (samples.length === 1 && frameTimestampMs < startedAtMs) {
        preEpisodeFrameCallbackCount += 1;
      } else {
  ...
  function startHostTelemetry() {
    if (typeof PerformanceObserverClass !== 'function') {
      longTaskSourceStatus = 'unavailable';
      longTaskSourceReason = 'performance-observer-unavailable';
      return;
    }
    try {
      performanceObserver = new PerformanceObserverClass(entries => {
        for (const entry of entries.getEntries()) {
          recordHostEvent({
            kind: 'browser-performance',
            phase: entry.entryType || 'performance-entry',
            source: 'performance-observer-longtask',
            startMs: entry.startTime,
            endMs: entry.startTime + entry.duration,
            detail: { name: entry.name || null, entryType: entry.entryType || null },
          });
        }
      });
      performanceObserver.observe({ type: 'longtask', buffered: true });
      longTaskSourceStatus = 'recording';
    } catch (error) {
      performanceObserver?.disconnect?.();
      performanceObserver = null;
      longTaskSourceStatus = 'unavailable';
      longTaskSourceReason = 'longtask-observe-failed';
      longTaskSourceError = error?.message || String(error);
    }
  }
  ...
  function start() {
    ...
    started = true;
    startedAtMs = now();
    hostEpisodeId = `${firingId || 'unbound-firing'}:${effectiveTimeOriginEpochMs}:${startedAtMs}`;
    capture(startedAtMs);
    startHostTelemetry();
    schedule();
    return api;
  ...
  function finish({ phase = 'complete', sharpHeartbeat = null } = {}) {
    ...
    performanceObserver?.disconnect();
    performanceObserver = null;
    if (longTaskSourceStatus === 'recording') longTaskSourceStatus = 'complete';
  ...
  function report({ phase = finished ? 'complete' : 'running', sharpHeartbeat = null } = {}) {
    ...
    for (const gap of gaps) {
      maxFrameGapMs = maxFrameGapMs === null ? gap : Math.max(maxFrameGapMs, gap);
    }
    ...
    const firePresentationMismatchSamples = samples
      .map((sample, sampleIndex) => ({
        sampleIndex,
        timestampMs: sample.timestampMs,
        firePresentation: cloneFirePresentation(sample.firePresentation),
        reasons: firePresentationMismatchReasons(expectedFirePresentationSnapshot, sample.firePresentation),
      }))
      ...
      ? samples
          .map((sample, sampleIndex) => ({
            ...
            reasons: fireEpisodeMismatchReasons({
              ...
              expectedFirePresentation: expectedFirePresentationSnapshot,
            }),
          }))
          .filter(sample => sample.reasons.length > 0)
      : [];
    ...
    if (finished) {
      ...
      if (expectedFirePresentationSnapshot?.requireFireEpisodeHooks) {
    ...
    const heartbeat = {
      ...
      expectedFirePresentation: cloneFirePresentation(expectedFirePresentationSnapshot),
      requireExactFireEpisode: requireExactFireEpisode === true,
      effectiveFirePresentation: cloneFirePresentation(last?.firePresentation || first?.firePresentation),
      ...
      hostEvents: hostEvents.map(event => ({ ...event })),
      hostTelemetry: {
        schema: FOREGROUND_HOST_TELEMETRY_SCHEMA,
        status: ['recording', 'complete'].includes(longTaskSourceStatus)
          ? longTaskSourceStatus
          : 'unavailable',
        firingId: firingId || null,
        episodeId: hostEpisodeId,
        clock: {
        ...
        explicitRecordEventAvailable: true,
        explicitEventSource: {
          identity: 'explicit-record-event',
          status: 'available',
        },
        longTaskSource: {
          identity: 'performance-observer-longtask',
          status: longTaskSourceStatus,
          reason: longTaskSourceReason,
          error: longTaskSourceError,
        },
      },
      budgetMismatchSamples,
      firePresentationMismatchSamples,
      fireEpisodeMismatchSamples,
      sharpHeartbeat: summarizeSharpHeartbeat(sharpHeartbeat),
      failures,
    };
  ...
  const api = { start, finish, report, recordEvent };
  return api;
}
