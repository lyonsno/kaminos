...
class GTAOComputeNode extends TempNode {
	...
	/**
	 * Returns true when an object should not contribute to the AO depth pass.
	 * The pass renders with an override material, so preserve source material
	 * "no depth" intent explicitly before the override can erase it.
	 *
	 * @private
	 * @param {Object3D} object - The object being tested.
	 * @return {boolean}
	 */
	_shouldExcludeFromDepthPass( object ) {

		if ( object.userData?.kaminosExcludeFromAODepth === true ) return true;

		const material = object.material;
		if ( material === undefined ) return false;

		const materials = Array.isArray( material ) ? material : [ material ];
		return materials.some( mat => mat?.depthWrite === false );

	}

	/**
	 * Temporarily hides objects that should not write AO depth.
	 *
	 * @private
	 * @param {Scene} scene - Scene traversed for non-occluders.
	 * @return {Array<Object3D>} Objects hidden for the pass.
	 */
	_hideDepthPassExcludedObjects( scene ) {

		const hiddenObjects = [];

		scene.traverse( object => {

			if ( object.visible === true && this._shouldExcludeFromDepthPass( object ) ) {
	...
	/**
	 * Restores objects hidden by `_hideDepthPassExcludedObjects()`.
	 *
	 * @private
	 * @param {Array<Object3D>} hiddenObjects - Objects hidden for the pass.
	 */
	_restoreDepthPassExcludedObjects( hiddenObjects ) {
		...
		if ( this._computeNode ) {
			...
			if ( this._depthRT && this._depthMaterial && this._scene ) {
				...
				try {
					...
					this._restoreDepthPassExcludedObjects( hiddenDepthObjects );
