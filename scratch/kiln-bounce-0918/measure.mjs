...
const [output,origin,sceneName,frameArg='240',pitchArg='0.1,0.2,0.4',timing='on']=process.argv.slice(2);
...
assert.ok(['on','off'].includes(timing));
const out=path.resolve(output),sha=b=>createHash('sha256').update(b).digest('hex');
const report={status:'running',phase:'identity',origin,sceneName,frames,pitches,timing,errors:[],limitations:[
...
try{
 ...
 const url=compositionRestoreUrl(scene.composition,sceneName,origin)+`&volume_light_field_shadows=1&volume_light_field_timing=${timing==='on'?1:0}`;report.requested=url;
 ...
 if(timing==='on')for(let i=0;i<5;i++)report.gpuProfiles.push(await page.evaluate(()=>window.kaminosFireLightFieldGpuProfile()));
 ...
 const geometry=await page.evaluate(()=>{
  ...
  scene.traverseVisible(o=>{
   ...
   meshes.push({name:o.name,uuid:o.uuid,floatOffset:offset,floatCount:values.length-offset,matrixWorld:o.matrixWorld.toArray(),groups:g.groups,
    materials:(Array.isArray(o.material)?o.material:[o.material]).map(m=>({type:m.type,side:m.side,opacity:m.opacity,metalness:m.metalness,metalnessMap:!!m.metalnessMap,roughness:m.roughness,roughnessMap:!!m.roughnessMap,color:m.color?.toArray(),map:!!m.map,mapColorSpace:m.map?.colorSpace,mapSize:m.map?.image?{width:m.map.image.width,height:m.map.image.height}:null}))});
  });
