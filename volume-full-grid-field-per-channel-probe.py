...
def write_scalar_head_application(
    ...
    for z0 in range(0, high_grid, chunk_z):
        ...
        for channel_index, state in channel_states.items():
            scalar_residual = predict_scalar_mlp(features_std, state)
            values = low_chunk[:, channel_index] + scalar_residual
            if args.application_prediction_source == "refinement":
                ...
                values = values + predict_scalar_mlp(refinement_features_std, refinement_state["model"])
            values = clamp_channel_values(values, channel_index, channel_max)
    ...
    manifest = {
        ...
        "model": {
            ...
            "limitation": "Same-pair fitted scalar/refinement head assembly; incoherent render output is route-local evidence, not a field-residual-program disproof.",
        },
        "roles": {
            "lowUpsampled": {
                "role": "lowUpsampled",
