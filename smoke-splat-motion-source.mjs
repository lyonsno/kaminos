...
export const HELD_SMOKE_ASSAY_MANIFEST_SCHEMA = 'kaminos.held-smoke-assay-source.v0';
export const HELD_SMOKE_ASSAY_ROUTE_IDENTITY = 'webgpu-held-smoke-assay-v0';
export const HELD_SMOKE_ASSAY_TEMPORAL_AUTHORITY = 'held-current-state-only-v0';
...
function requireFiniteArray(value, length, label) {
  if (!Array.isArray(value) || value.length !== length) throw new TypeError(`${label} must contain ${length} finite values`);
  return value.map((component, index) => requireFinite(component, `${label}[${index}]`));
}
...
export function validateSmokeSplatMotionManifest(candidate) {
  ...
  const heldAssay = manifest.schema === HELD_SMOKE_ASSAY_MANIFEST_SCHEMA;
  if (!heldAssay && manifest.schema !== SMOKE_SPLAT_MOTION_MANIFEST_SCHEMA) throw new Error('motion manifest schema mismatch');
  ...
  const expectedRoute = heldAssay ? HELD_SMOKE_ASSAY_ROUTE_IDENTITY : SMOKE_SPLAT_MOTION_ROUTE_IDENTITY;
  if (manifest.effectiveRoute !== expectedRoute) throw new Error(`effective route is not ${expectedRoute}`);
  if (manifest.fallbackReason !== null) throw new Error(`motion manifest declares fallback: ${manifest.fallbackReason}`);
  const expectedTemporalAuthority = heldAssay ? HELD_SMOKE_ASSAY_TEMPORAL_AUTHORITY : SMOKE_SPLAT_MOTION_TEMPORAL_AUTHORITY;
  if (manifest.temporalAuthority !== expectedTemporalAuthority) throw new Error('temporal authority mismatch');
  if (manifest.producerAuthority !== PRODUCER_AUTHORITY) throw new Error('manifest producer authority mismatch');
  if (heldAssay) {
    const source = requireObject(manifest.source, 'held assay source');
    requireShaIdentity(source.manifestIdentity, 'held assay manifest identity');
    requireShaIdentity(source.fluidIdentity, 'held assay fluid identity');
    requireShaIdentity(source.cameraIdentity, 'held assay camera identity');
    requireInteger(source.simStepCount, 'held assay sim step');
    const camera = requireObject(manifest.camera, 'held assay camera');
    requireVector3(camera.position, 'held assay camera position');
    requireVector3(camera.target, 'held assay camera target');
    requireFiniteArray(camera.projectionMatrix, 16, 'held assay projectionMatrix');
    requireFiniteArray(camera.matrixWorldInverse, 16, 'held assay matrixWorldInverse');
  }
