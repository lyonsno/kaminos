...
async function waitForPageResult(session, chrome, server) {
  while (chrome.exitCode === null && server.exitCode === null) {
    const state = await evaluate(session, 'window.__TRANSFER_GPU_BENCHMARK__ ?? null');
...
async function run(args) {
  ...
  try {
    ...
    report.sources = {
      ...
      page: { path: join(ROOT, 'view-conditioned-transfer-gpu-benchmark.html'), sha256: await sha256File(join(ROOT, 'view-conditioned-transfer-gpu-benchmark.html')) },
    ...
    await session.send('Emulation.setDeviceMetricsOverride', {
      width: 1180, height: 920, deviceScaleFactor: 1, mobile: false,
    });
    ...
    if (pageReport.effectiveSamples?.dense !== args.samples || pageReport.effectiveSamples?.reduced !== args.samples) throw new Error('browser silently changed the sample count');
    ...
    report.outputValidation = pageReport.outputValidation;
