...
import {
  ...
  STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_INTERACTION_BYTES,
  STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_ROUTE,
  STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_SHADER,
  STRUCTURAL_MATERIAL_3D_SOLVED_FRACTURE_SHADER,
  buildLayeredStructuralResidentSolverInteraction,
  deriveLayeredStructuralAliveComponentLabels,
  packLayeredStructuralResidentSolverInteraction,
  parseLayeredStructuralResidentSolverNodes,
  summarizeLayeredStructuralSolvedNodes,
...
function applyGpuBondMutationState(state, mutationState) {
  const livenessChanged = state.bonds.some((bond, index) =>
    bond.alive !== mutationState[index]?.alive);
  return {
    ...state,
...
export function validateLayeredStructuralHotSidecarReceipt(state, receipt) {
  ...
  const solver = receipt?.solver;
  const nodeDisplacements = receipt?.gpuStructuralState?.nodeDisplacements;
  ...
  if (lifecycle.pipelineCreateCount !== 4) reasons.push('pipeline-lifecycle');
  if (lifecycle.bufferAllocationCount !== 13) reasons.push('buffer-lifecycle');
  ...
  if (lifecycle.solverDispatchCount < 2 || lifecycle.solverNodeReadbackCount < 1) reasons.push('solver-lifecycle');
  ...
  if (!Array.isArray(nodeDisplacements) || nodeDisplacements.length !== state.nodes.length ||
      nodeDisplacements.some(displacement =>
        !Number.isFinite(displacement?.x) ||
        !Number.isFinite(displacement?.y) ||
        !Number.isFinite(displacement?.z))) {
    reasons.push('solver-node-displacements');
  }
  if (solver?.route !== STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_ROUTE ||
      solver?.authority !== STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_AUTHORITY) {
    reasons.push('solver-authority');
  }
  if (!Number.isInteger(solver?.iterationCount) || solver.iterationCount < 2 ||
      solver.iterationCount % 2 !== 0 || solver.dispatchCount !== solver.iterationCount) {
    reasons.push('solver-iterations');
  }
  if (!Number.isInteger(solver?.generation?.before) ||
      solver?.generation?.after !== solver.generation.before + 1) {
    reasons.push('solver-generation');
  }
  if (!Number.isFinite(solver?.metrics?.maxConstraintResidual) ||
      !Number.isFinite(solver?.metrics?.meanConstraintResidual) ||
      solver?.metrics?.contactTargetError > 0.000001 ||
      solver?.metrics?.maxPinnedDisplacement > 0.000001 ||
      solver?.metrics?.nonPrimaryCurrentResponse > 0.000001) {
    reasons.push('solver-metrics');
  }
...
export async function createLayeredStructuralHotWebGpuSidecar(options = {}) {
  ...
  let solverGesture = null;
  let lastAcceptedExecution = null;
  ...
  let solverInteractionBuffer;
  let responseBuffer;
  let eventHeaderBuffer;
  let eventBuffer;
  let componentLabelBuffer;
  let solverComponentLabelBuffer;
  let bondReadback;
  ...
  let solverForwardBindGroup;
  let solverBackwardBindGroup;
  ...
  try {
    ...
    nodePingBuffer = makeBuffer({
      label: 'hot-structural-solver-node-ping',
      size: packed.nodeData.byteLength,
      usage: usage.STORAGE | usage.COPY_DST | usage.COPY_SRC,
    });
    ...
    solverInteractionBuffer = makeBuffer({
      label: 'hot-structural-solver-interaction-uniform',
      size: STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_INTERACTION_BYTES,
      usage: usage.UNIFORM | usage.COPY_DST,
    });
    ...
    solverComponentLabelBuffer = makeBuffer({
      label: 'hot-structural-solver-component-label-storage',
      size: packed.nodeCount * Uint32Array.BYTES_PER_ELEMENT,
      usage: usage.STORAGE | usage.COPY_DST,
    });
    ...
    nodeReadback = makeBuffer({
      label: 'hot-structural-solver-node-readback',
      size: packed.nodeData.byteLength,
      usage: usage.COPY_DST | usage.MAP_READ,
    });

    device.queue.writeBuffer(nodeBuffer, 0, packed.nodeData);
    device.queue.writeBuffer(nodePingBuffer, 0, packed.nodeData);
    ...
    device.queue.writeBuffer(
      solverComponentLabelBuffer,
      0,
      Uint32Array.from(deriveLayeredStructuralAliveComponentLabels(state)),
    );
    ...
    const fractureModule = device.createShaderModule({
      ...
      code: STRUCTURAL_MATERIAL_3D_SOLVED_FRACTURE_SHADER,
    });
    ...
    const solverPipelineStart = layeredStructuralGpuNow();
    const solverModule = device.createShaderModule({
      label: STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_ROUTE,
      code: STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_SHADER,
    });
    lifecycle.pipelineCreateCount += 1;
    solverPipeline = await device.createComputePipelineAsync({
      label: `${STRUCTURAL_MATERIAL_3D_WEBGPU_HOT_SIDECAR_ROUTE}:solver`,
      layout: 'auto',
      compute: { module: solverModule, entryPoint: 'main' },
    });
    coldTimingsMs.solverPipelineCompile = layeredStructuralGpuNow() - solverPipelineStart;
    ...
    solverForwardBindGroup = device.createBindGroup({
      label: 'hot-structural-solver-forward-bind-group',
      layout: solverPipeline.getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: nodeBuffer } },
        { binding: 1, resource: { buffer: nodePingBuffer } },
        { binding: 2, resource: { buffer: bondBuffer } },
        { binding: 3, resource: { buffer: solverComponentLabelBuffer } },
        { binding: 4, resource: { buffer: solverInteractionBuffer } },
      ],
    });
    solverBackwardBindGroup = device.createBindGroup({
      label: 'hot-structural-solver-backward-bind-group',
      layout: solverPipeline.getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: nodePingBuffer } },
  ...
  const restoreAcceptedGpuState = async acceptedState => {
    try {
      ...
      device.queue.writeBuffer(nodePingBuffer, 0, accepted.nodeData);
      ...
      device.queue.writeBuffer(
        solverComponentLabelBuffer,
        0,
        Uint32Array.from(deriveLayeredStructuralAliveComponentLabels(acceptedState)),
      );
  ...
  const execute = interaction => {
    ...
    return enqueue(async () => {
      ...
      const priorSolverGesture = solverGesture;
      try {
        ...
        if (lastAcceptedExecution?.eventEpoch === eventEpoch &&
            lastAcceptedExecution.sequenceIdentity === sequenceIdentity) {
          return { ...lastAcceptedExecution.receipt, replayed: true };
        }
        eventEpoch += 1;
        const gestureId = interaction?.gestureId == null
          ? sequenceIdentity
          : String(interaction.gestureId);
        if (solverGesture?.gestureId !== gestureId) {
          solverGesture = {
            gestureId,
            baselineDisplacements: sourceState.nodes.map(node => ({ ...node.displacement })),
          };
        }
        const solverInteraction = buildLayeredStructuralResidentSolverInteraction(
          sourceState,
          interaction,
          { baselineDisplacements: solverGesture.baselineDisplacements },
        );
        const nextSolverGeneration = solverGeneration + 1;
        const solverData = packLayeredStructuralResidentSolverInteraction(
          sourceState,
          solverInteraction,
          { eventEpoch, solverGeneration: nextSolverGeneration },
        );
        ...
        device.queue.writeBuffer(solverInteractionBuffer, 0, solverData);
        device.queue.writeBuffer(eventHeaderBuffer, 0, new Uint32Array(4));
        ...
        device.queue.writeBuffer(
          solverComponentLabelBuffer,
          0,
          Uint32Array.from(deriveLayeredStructuralAliveComponentLabels(sourceState)),
        );
        ...
        for (let iteration = 0; iteration < solverInteraction.config.iterationCount; iteration += 1) {
          const solverPass = encoder.beginComputePass({
            label: `hot-structural-solver:${eventEpoch}:${iteration + 1}`,
          });
          solverPass.setPipeline(solverPipeline);
          solverPass.setBindGroup(0, iteration % 2 === 0
            ? solverForwardBindGroup
            : solverBackwardBindGroup);
          solverPass.dispatchWorkgroups(
            Math.ceil(packed.nodeCount / STRUCTURAL_MATERIAL_3D_WEBGPU_WORKGROUP_SIZE),
          );
          solverPass.end();
          lifecycle.solverDispatchCount += 1;
        }
        ...
        const solvedNodes = parseLayeredStructuralResidentSolverNodes(nodeBytes, sourceState);
        const solverMetrics = summarizeLayeredStructuralSolvedNodes(
          sourceState,
          solvedNodes,
          solverInteraction,
        );
        ...
        Object.assign(receipt, {
          ...
          gpuStructuralState: {
            finalBondLiveness,
            componentLabels,
            nodeDisplacements: solvedNodes.map(node => ({ ...node.displacement })),
          },
          topology,
          solver: {
            schema: 'kaminos.structural-material.resident-compliant-jacobi-receipt.v0',
            route: STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_ROUTE,
            authority: STRUCTURAL_MATERIAL_3D_RESIDENT_SOLVER_AUTHORITY,
            iterationCount: solverInteraction.config.iterationCount,
            dispatchCount: solverInteraction.config.iterationCount,
            config: { ...solverInteraction.config },
            gestureId,
            contact: { ...solverInteraction.contact },
            contactTargetDisplacements: solverInteraction.contactTargetDisplacements
              .map(displacement => ({ ...displacement })),
            generation: { before: solverGeneration, after: nextSolverGeneration },
            topologyEpoch: sourceState.topologyEpoch,
            connectivityEpoch: sourceState.connectivityEpoch,
        ...
        if (!validation.ok) {
          solverGesture = priorSolverGesture;
          ...
          const mutated = applyGpuBondMutationState(sourceState, bondMutationState);
          state = { ...mutated, nodes: solvedNodes };
          solverGeneration = nextSolverGeneration;
          lastAcceptedExecution = { sequenceIdentity, eventEpoch, receipt };
        }
      } catch (error) {
        solverGesture = priorSolverGesture;
        ...
        receipt.lifecycle = cloneLifecycle(lifecycle);
        for (const readback of [bondReadback, componentReadback, nodeReadback]) {
          try {
            readback.unmap();
          } catch {
  ...
  const reinitialize = nextState => {
    ...
    return enqueue(async () => {
      ...
      solverGeneration = 0;
      solverGesture = null;
      lastAcceptedExecution = null;
      device.queue.writeBuffer(nodeBuffer, 0, nextPacked.nodeData);
      device.queue.writeBuffer(nodePingBuffer, 0, nextPacked.nodeData);
      ...
      device.queue.writeBuffer(
        solverComponentLabelBuffer,
        0,
        Uint32Array.from(deriveLayeredStructuralAliveComponentLabels(nextState)),
      );
      await device.queue.onSubmittedWorkDone();
