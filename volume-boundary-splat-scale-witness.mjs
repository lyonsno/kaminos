...
const FIELD_LAYOUT = 'boundary-splat-composed-field-v0';
...
try {
  ...
  const cameraState = await evaluate('window.kaminosBoundarySplatFieldLayoutDebugState?.()');
  ...
  const capture = await evaluate(`window.__kaminosVolumePrototype.renderFrozenScaleToCanvas(${JSON.stringify({
    ...
    controlOverrides: {
      ...
      boundarySplatFieldLayout: 'field',
  ...
  const report = {
    ...
    composedCapture: {
      ...
      fieldLayoutIdentity: capture.boundarySplatFieldLayoutIdentity || finalState.boundarySplatFieldLayoutIdentity,
...
async function waitForPrototype() {
  for (let attempt = 0; attempt < 240; attempt += 1) {
    ...
    if (state?.active && state?.backend && typeof state.boundarySplatFieldLayoutIdentity === 'string') return state;
    await delay(125);
  }
...
function validateEffectiveState(state, cameraState, pageUrl) {
  ...
  if (state?.boundarySplatFieldLayoutIdentity !== FIELD_LAYOUT) mismatches.push(['fieldLayout', FIELD_LAYOUT, state?.boundarySplatFieldLayoutIdentity]);
  ...
  if (params.get('volume_boundary_splat_layout') !== 'field') mismatches.push(['routeFieldLayout', 'field', params.get('volume_boundary_splat_layout')]);
...
function validateLadder(ladder) {
  ...
  for (const [index, row] of ladder.rows.entries()) {
    ...
    assert.equal(row.fieldLayoutIdentity, FIELD_LAYOUT, 'field layout identity changed in cost ladder');
